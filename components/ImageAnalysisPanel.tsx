      ...
      <div className="roof-er-content-scroll">
          ...
          <div style={{ float: 'right', display: 'flex', gap: '12px', alignItems: 'center' }}>
            <button
              ...
              style={{
                ...
                color: historyView ? 'var(--text-primary)' : 'var(--text-primary)',
            ...
            {assessments.length >= 2 && (
              <button
                ...
                style={{
                  ...
                  color: comparisonMode ? 'var(--text-primary)' : 'var(--text-primary)',
        ...
        >
          <input
            ref={fileInputRef}
            type="file"
            accept="image/*,.heic,.heif,.pdf,.docx,.md,.txt"
            multiple
          ...
          <div className="roof-er-upload-text">
            {isAnalyzing ? 'Analyzing...' : isDragging ? 'Drop files here' : 'Drop photos/docs here or click to upload'}
          </div>
          <div className="roof-er-upload-subtext">
            {isAnalyzing ? 'AI is processing your uploads' : 'Up to 10 files • Images (PNG/JPG/HEIC) and Docs (PDF/DOCX/MD/TXT)'}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', margin: '12px 0' }}>
          <button onClick={() => setViewKind('all')} style={{ padding: '6px 10px', background: viewKind==='all'?'var(--roof-red)':'var(--bg-hover)', border: `1px solid ${viewKind==='all'?'var(--roof-red)':'var(--border-default)'}`, borderRadius: '9999px', color: 'var(--text-primary)', fontSize: '12px' }}>All</button>
          <button onClick={() => setViewKind('images')} style={{ padding: '6px 10px', background: viewKind==='images'?'var(--roof-red)':'var(--bg-hover)', border: `1px solid ${viewKind==='images'?'var(--roof-red)':'var(--border-default)'}`, borderRadius: '9999px', color: 'var(--text-primary)', fontSize: '12px' }}>Images</button>
          <button onClick={() => setViewKind('docs')} style={{ padding: '6px 10px', background: viewKind==='docs'?'var(--roof-red)':'var(--bg-hover)', border: `1px solid ${viewKind==='docs'?'var(--roof-red)':'var(--border-default)'}`, borderRadius: '9999px', color: 'var(--text-primary)', fontSize: '12px' }}>Docs</button>
        </div>

        {/* Recent User Uploads (Docs) */}
        <div style={{ marginTop: '24px' }}>
          <div className="roof-er-page-title" style={{ fontSize: '18px', margin: 0 }}>
            Recent Uploads (Docs)
          </div>
          {viewKind !== 'images' && (userUploads.length === 0 ? (
            <div style={{ padding: '16px', color: 'var(--text-tertiary)', fontSize: '14px' }}>
              No recent document uploads. Drag and drop PDFs, DOCX, MD, or TXT here to add them.
            </div>
          ) : (
            <div className="roof-er-doc-grid" style={{ marginTop: '12px' }}>
              {userUploads.map((u) => (
                <div key={u.id} className="roof-er-doc-card" style={{ position: 'relative' }}>
                  <div className="roof-er-doc-title">{u.name}</div>
                  <div className="roof-er-doc-desc">User Uploads • DOC</div>
                  <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                    <button
                      onClick={() => { try { localStorage.setItem('chat_quick_doc', JSON.stringify({ name: u.name, path: u.path })); } catch {}; onOpenChat?.(); }}
                      style={{ padding: '6px 10px', background: 'var(--roof-red)', border: 'none', borderRadius: '6px', color: '#fff', fontSize: '12px' }}
                    >
                      Open in Chat
                    </button>
                    <button
                      onClick={() => {
                        try { localStorage.setItem('knowledge_open_doc', u.path); } catch {}
                        onOpenKnowledge?.();
                      }}
                      style={{ padding: '6px 10px', background: 'var(--bg-hover)', border: '1px solid var(--border-default)', borderRadius: '6px', color: 'var(--text-primary)', fontSize: '12px' }}
                    >
                      View
                    </button>
                  </div>
        ...
        {viewKind !== 'docs' && (historyView || !selectedAssessment) && (
          <>
            <div style={{
              display: 'flex',
              justifyContent: 'space-between',
