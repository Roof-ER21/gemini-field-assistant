...
async function main() {
    ...
    for (const e of ENTRIES) {
        const exists = await pool.query(`SELECT id FROM knowledge_documents WHERE name = $1 LIMIT 1`, [e.name]);
        if (exists.rowCount && exists.rowCount > 0) {
            console.log(`  - already exists: "${e.name}" (id=${exists.rows[0].id})`);
            already++;
            continue;
        }
        const ins = await pool.query(`INSERT INTO knowledge_documents (name, category, state, content)
       VALUES ($1, $2, $3, $4) RETURNING id`, [e.name, e.category, e.state || null, e.content]);
