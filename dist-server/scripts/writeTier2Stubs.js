...
async function main() {
    ...
    for (const s of STUBS) {
        const exists = await pool.query(`SELECT id FROM knowledge_documents WHERE name = $1 LIMIT 1`, [s.name]);
...
NEXT STEP: When a rep posts an experience, leadership can promote this stub to a real intel row. Until then, Susan should acknowledge the name and request team input rather than fabricate.`;
        const ins = await pool.query(`INSERT INTO knowledge_documents (name, category, content) VALUES ($1, 'adjuster-intel', $2) RETURNING id`, [s.name, content]);
