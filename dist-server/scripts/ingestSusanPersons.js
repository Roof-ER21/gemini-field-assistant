 ...
 */
async function fetchGroupMeRoster() {
    ...
    try {
        ...
        console.warn('[ingest] GroupMe roster fetch err:', e.message);
        return new Set();
    }
}
...
function normalizeCarrier(raw) {
    ...
    if (CARRIER_NORMALIZE[cleaned])
        return CARRIER_NORMALIZE[cleaned];
...
function isLikelyTestUser(name) {
    ...
    if (TEAMMATE_HANDLE_BLOCKLIST.has(name.toLowerCase().trim()))
        return true;
    return false;
}
function buildTeammateAltNames(name) {
    const alts = new Set();
...
function buildAdjusterAltNames(parsed) {
    const alts = new Set();
    ...
    if (parsed.raw && parsed.raw !== parsed.cleanedName) {
        ...
        if (stripped && stripped !== parsed.cleanedName)
            alts.add(stripped);
    }
...
async function main() {
    ...
    const teammateNames = new Set(); // lowercased full names
    const adjusterByKey = new Map();
    ...
    const usersResult = await pool.query(`SELECT id, name, role FROM users WHERE name IS NOT NULL ORDER BY name`);
    for (const u of usersResult.rows) {
        if (u.name && !isLikelyTestUser(u.name))
            teammateNames.add(u.name.toLowerCase());
    }
    for (const n of groupmeRoster) {
        if (n && !isLikelyTestUser(n))
            teammateNames.add(n.toLowerCase());
    }
    ...
    if (!DRY && teammateNames.size > 0) {
        ...
        const dups = await pool.query(`SELECT a.id AS adj_id, a.canonical_name,
       ...
       WHERE a.person_type='adjuster'
         AND a.canonical_lower = ANY($1::text[])`, [allTeammatesLower]);
        for (const row of dups.rows) {
            if (row.team_id) {
                // Merge: move KB links to the teammate row, delete the adjuster dup
                await pool.query(`UPDATE knowledge_documents SET person_id=$1 WHERE person_id=$2`, [row.team_id, row.adj_id]);
                ...
                await pool.query(`UPDATE susan_persons SET person_type='teammate', updated_at=now() WHERE id=$1`, [row.adj_id]);
    ...
    for (const u of usersResult.rows) {
        ...
        await pool.query(`INSERT INTO susan_persons (canonical_name, alt_names, person_type, team_user_id)
       VALUES ($1, $2, 'teammate', $3)
       ON CONFLICT (canonical_lower, person_type, COALESCE(lower(carrier), ''))
         DO UPDATE SET alt_names = EXCLUDED.alt_names, team_user_id = EXCLUDED.team_user_id, updated_at = now()`, [name, altNames, u.id]);
        stats.teammates_inserted++;
    }
    // ─── Teammates from GroupMe (not in users table) ───────────────────────────
    for (const name of groupmeRoster) {
        if (!name || isLikelyTestUser(name))
            continue;
        const lower = name.toLowerCase();
        if (teammateNames.has(lower))
            continue; // already inserted via users
       ...
       ON CONFLICT (canonical_lower, person_type, COALESCE(lower(carrier), ''))
         DO UPDATE SET alt_names = EXCLUDED.alt_names, updated_at = now()`, [name, altNames]);
        stats.teammates_groupme_only++;
    }
    // (Step 1 reclassification was already done before inserts — see top of main)
    // ─── Adjusters from knowledge_documents ────────────────────────────────────
    const kbResult = await pool.query(`SELECT id, name FROM knowledge_documents WHERE category='adjuster-intel' ORDER BY id`);
    for (const r of kbResult.rows) {
        ...
        if (existing) {
            ...
            for (const a of buildAdjusterAltNames(parsed))
                existing.alts.add(a);
        }
        else {
    ...
    for (const entry of adjusterByKey.values()) {
        if (!DRY) {
            ...
            const ins = await pool.query(`INSERT INTO susan_persons (canonical_name, alt_names, person_type, carrier)
         VALUES ($1, $2, $3, $4)
         ON CONFLICT (canonical_lower, person_type, COALESCE(lower(carrier), ''))
           DO UPDATE SET alt_names = EXCLUDED.alt_names, updated_at = now()
         RETURNING id`, [entry.name, altArr, entry.isCompany ? 'company' : 'adjuster', entry.carrier]);
            const personId = ins.rows[0]?.id;
            if (personId) {
                // Link all kb rows for this person
                await pool.query(`UPDATE knowledge_documents SET person_id = $1 WHERE id = ANY($2::int[])`, [personId, entry.kbIds]);
    ...
    const teammateFirstNames = new Map(); // first → [full names]
    for (const fullLower of teammateNames) {
        const first = fullLower.split(/\s+/)[0];
        if (!teammateFirstNames.has(first))
            teammateFirstNames.set(first, []);
    ...
    for (const entry of adjusterByKey.values()) {
        ...
        if (!teammateMatches || teammateMatches.length === 0)
            continue;
