...
export class StormDataService {
     ...
     */
    async findStormsNearLocation(params) {
        const query = `
    ...
    async searchStorms(params) {
        ...
        if (params.latitude && params.longitude && params.radius_miles) {
            whereClauses.push(`calculate_distance_miles($${paramIndex}, $${paramIndex + 1}, latitude, longitude) <= $${paramIndex + 2}`);
     ...
     * Update storm event
     */
    async updateStormEvent(id, data) {
        const setClauses = [];
        const values = [];
        let paramIndex = 1;
     ...
     */
    async getClaimOutcomesByStormId(stormEventId) {
        const query = `
     ...
     */
    async getClaimOutcomesByJobId(jobId) {
        const query = `
      SELECT * FROM storm_claim_outcomes
      WHERE job_id = $1
      ORDER BY created_at DESC
     ...
     */
    async updateClaimOutcome(id, data) {
        const setClauses = [];
        const values = [];
        let paramIndex = 1;
        Object.entries(data).forEach(([key, value]) => {
        ...
        if (setClauses.length === 0) {
            const result = await this.pool.query('SELECT * FROM storm_claim_outcomes WHERE id = $1', [id]);
            return result.rows[0] || null;
        }
        values.push(id);
        const query = `
     ...
     */
    async getAreaStrategies(params) {
        const query = `
     ...
     * Get area patterns
     */
    async getAreaPatterns(state, scopeType, scopeValue) {
        let query = `
     ...
     */
    async getRecentSuccessfulClaims(limit = 10, state) {
     ...
     * Get storm hotspots
     */
    async getStormHotspots(state, limit = 20) {
     ...
     */
    async getTopArguments(insuranceCompany, state, limit = 10) {
     ...
     */
    async getTopEvidenceTypes(insuranceCompany, state, limit = 10) {
        let whereClauses = ["claim_result IN ('won', 'partial_win')"];
        const values = [];
        let paramIndex = 1;
        if (insuranceCompany) {
    ...
    async getSusanRecommendations(params) {
        ...
        // Get top arguments
        const topArgsResult = await this.getTopArguments(params.insurance_company, params.state, 5);
        const top_arguments = topArgsResult.map((r) => r.argument);
        // Get top evidence types
        const topEvidenceResult = await this.getTopEvidenceTypes(params.insurance_company, params.state, 5);
