 ...
 */
export function createTerritoryService(pool) {
    /**
     * Get all territories for a user (owned or assigned)
     */
    async function getUserTerritories(userId) {
    ...
    async function getTerritoryById(territoryId) {
      ...
      FROM territories t
      LEFT JOIN users u ON t.owner_id = u.id
      WHERE t.id = $1 AND t.archived_at IS NULL`, [territoryId]);
     ...
     */
    async function createTerritory(input) {
        const result = await pool.query(`INSERT INTO territories (
     ...
     * Update a territory
     */
    async function updateTerritory(territoryId, updates) {
        const fields = [];
        const values = [];
        let paramCount = 1;
        ...
        await pool.query(`UPDATE territories SET ${fields.join(', ')} WHERE id = $${paramCount}`, values);
        return getTerritoryById(territoryId);
    }
    /**
    ...
    async function deleteTerritory(territoryId) {
        const result = await pool.query(`UPDATE territories SET archived_at = NOW() WHERE id = $1 AND archived_at IS NULL`, [territoryId]);
        return (result.rowCount ?? 0) > 0;
    }
    /**
     * Check in to a territory
     */
    async function checkIn(territoryId, userId, lat, lng) {
        // End any existing active check-ins for this user
        await pool.query(`UPDATE territory_checkins
       SET check_out_time = NOW()
       WHERE user_id = $1 AND check_out_time IS NULL`, [userId]);
        const result = await pool.query(`INSERT INTO territory_checkins (
        territory_id, user_id, check_in_lat, check_in_lng
      ) VALUES ($1, $2, $3, $4)
      RETURNING *`, [territoryId, userId, lat || null, lng || null]);
        // Log activity
        await logActivity({
            territoryId,
            userId,
     ...
     */
    async function checkOut(checkInId, stats, lat, lng) {
        const result = await pool.query(`UPDATE territory_checkins SET
        check_out_time = NOW(),
        check_out_lat = $2,
        check_out_lng = $3,
    ...
    async function getActiveCheckIn(userId) {
        const result = await pool.query(`SELECT * FROM territory_checkins
    ...
    async function logActivity(activity) {
        await pool.query(`INSERT INTO territory_activity (
    ...
    async function getLeaderboard() {
        const result = await pool.query(`SELECT * FROM territory_leaderboard`);
        return result.rows.map(row => ({
            territoryId: row.territory_id,
     ...
     */
    async function findTerritoryByPoint(lat, lng) {
        const result = await pool.query(`SELECT t.id FROM territories t
