...
export class ImpactedAssetService {
    ...
    async addCustomerProperty(params) {
        const result = await this.pool.query(`INSERT INTO customer_properties (
     ...
     */
    async getUserProperties(userId, activeOnly = true) {
    ...
    async getPropertyById(propertyId) {
        const result = await this.pool.query('SELECT * FROM customer_properties WHERE id = $1', [propertyId]);
    ...
    async updateProperty(propertyId, updates) {
        ...
        const result = await this.pool.query(`UPDATE customer_properties
    ...
    async deleteProperty(propertyId, userId) {
      ...
      WHERE id = $1 AND user_id = $2`, [propertyId, userId]);
     ...
     */
    async checkStormImpact(latitude, longitude, eventType, hailSize, windSpeed) {
        const result = await this.pool.query(`SELECT * FROM check_storm_impact($1, $2, $3, $4, $5)`, [latitude, longitude, eventType, hailSize || null, windSpeed || null]);
    ...
    async createImpactAlerts(params) {
        ...
        const impacted = await this.checkStormImpact(params.stormLatitude, params.stormLongitude, params.eventType, params.hailSize, params.windSpeed);
        const alerts = [];
        for (const property of impacted) {
            // Calculate severity
            const severityResult = await this.pool.query(`SELECT calculate_alert_severity($1, $2, $3, $4) as severity`, [params.eventType, property.distanceMiles, params.hailSize || null, params.windSpeed || null]);
            const severity = severityResult.rows[0].severity;
            // Create alert
            const alertResult = await this.pool.query(`INSERT INTO impact_alerts (
     ...
     */
    async sendSMSNotification(alertId, userId, propertyId, details) {
        try {
            // Check if user has SMS enabled and has a phone number
            const userResult = await this.pool.query(`SELECT phone_number, sms_alerts_enabled
         FROM users
         WHERE id = $1`, [userId]);
           ...
           SET sms_sent = TRUE,
               ...
               sms_message_sid = $1,
               updated_at = NOW()
           WHERE id = $2`, [result.messageSid, alertId]);
    ...
    async getPendingAlerts(userId) {
      ...
      ORDER BY ia.created_at DESC`, [userId]);
        return result.rows.map(row => ({
     ...
     * Update alert status
     */
    async updateAlertStatus(alertId, status, outcome, contactNotes) {
        const result = await this.pool.query(`UPDATE impact_alerts
      SET
        status = $1,
        ...
        updated_at = NOW()
      WHERE id = $4
      RETURNING *`, [status, outcome || null, contactNotes || null, alertId]);
        if (result.rows.length === 0) {
            return null;
        }
     ...
     */
    async convertAlert(alertId, jobId) {
        const result = await this.pool.query(`UPDATE impact_alerts
      SET
        status = 'converted',
        outcome = 'converted',
     ...
     */
    async getUserImpactStats(userId, daysBack = 90) {
        const result = await this.pool.query(`SELECT * FROM get_impact_stats($1, $2)`, [userId, daysBack]);
     ...
     */
    async runStormMonitoring(stormData) {
        // Log monitoring run start
        const runResult = await this.pool.query(`INSERT INTO storm_monitoring_runs (run_type, start_time)
      VALUES ('scheduled', NOW())
      RETURNING id`);
        const runId = runResult.rows[0].id;
        let totalAlerts = 0;
        const propertiesChecked = new Set();
        try {
            ...
            await this.pool.query(`UPDATE storm_monitoring_runs
        SET end_time = NOW(), properties_checked = $1, alerts_generated = $2
        WHERE id = $3`, [propertiesChecked.size, totalAlerts, runId]);
        }
        catch (error) {
            // Log error
            await this.pool.query(`UPDATE storm_monitoring_runs
        SET end_time = NOW(), errors = $1
        WHERE id = $2`, [JSON.stringify({ message: error.message }), runId]);
            throw error;
        }
        return {
 ...
 */
export const createImpactedAssetService = (pool) => {
