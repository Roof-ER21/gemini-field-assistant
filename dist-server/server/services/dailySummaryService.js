...
Powered by ROOFER - The Roof Docs
    ...
    async getAdminDailySummary(date) {
        ...
        try {
            ...
            const usersResult = await this.pool.query(`SELECT
        ...
        ORDER BY COUNT(*) DESC`, [targetDate]);
            // Get any email errors from today
            const errorsResult = await this.pool.query(`SELECT notification_type, recipient_email, error_message, sent_at
     ...
     */
    generateAdminSummaryEmail(summary) {
...
USER BREAKDOWN:
${summary.userBreakdown.map(u => `• ${u.name} (${u.state || 'N/A'}) - ${u.totalActivities} activities [Chats: ${u.chats}, Docs: ${u.documents}]`).join('\n')}
...
Powered by ROOFER - The Roof Docs
     ...
     */
    async sendAdminDailySummary(date) {
        const targetDate = date || new Date().toISOString().split('T')[0];
        ...
        try {
            ...
            // Send to admin
            const success = await this.sendEmailViaService(adminEmail, emailTemplate.subject, emailTemplate.html, emailTemplate.text);
            // Log to email_notifications
            await this.pool.query(`INSERT INTO email_notifications (notification_type, recipient_email, email_data, success)
         VALUES ($1, $2, $3, $4)`, [
                'admin_daily_summary',
                adminEmail,
                JSON.stringify({
