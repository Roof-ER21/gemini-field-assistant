        ...
        if (twilioService.isConfigured() && alerts.length > 0) {
            ...
            for (const alert of alerts) {
                try {
                    ...
                    const userResult = await pool.query(`SELECT u.id, u.phone_number, u.sms_alerts_enabled, cp.address, cp.customer_name
             FROM users u
             JOIN customer_properties cp ON cp.user_id = u.id
             WHERE u.id = $1 AND cp.id = $2`, [alert.userId, alert.customerPropertyId]);
