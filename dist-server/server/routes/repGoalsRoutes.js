...
export function createRepGoalsRoutes(pool) {
    const router = Router();
    /**
     * GET /api/rep/goals
    ...
    router.get('/goals/progress', async (req, res) => {
        try {
            ...
            const repResult = await pool.query(`SELECT
          id,
          name,
          monthly_signups,
          monthly_revenue,
         ...
         LIMIT 6`, [rep.id]);
            ...
            const yearlyRevenueProgress = yearlyRevenueGoal > 0
                ...
                : 0;
            // Determine status
            const getStatus = (progress, daysRemaining, totalDays) => {
            ...
            const rankResult = await pool.query(`WITH ranked AS (
           SELECT id, ROW_NUMBER() OVER (ORDER BY monthly_signups DESC, name ASC) as rank
           FROM sales_reps
           WHERE is_active = true
         )
         SELECT rank FROM ranked WHERE id = $1`, [rep.id]);
            const totalActiveResult = await pool.query(`SELECT COUNT(*) as total FROM sales_reps WHERE is_active = true`);
