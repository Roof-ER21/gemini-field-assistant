...
import { createStormMemoryService } from '../services/stormMemoryService.js';
...
router.post('/save', async (req, res) => {
    try {
        ...
        const { address, city, state, zipCode, latitude, longitude, stormEvents, dataSources } = req.body;
...
router.get('/nearby', async (req, res) => {
    try {
        ...
        const service = createStormMemoryService(pool);
        const nearbyStorms = await service.findNearbyStorms(latitude, longitude, radiusMiles, userOnly === 'true' ? userId : undefined);
        res.json({
            success: true,
...
router.get('/by-zip/:zipCode', async (req, res) => {
    try {
        ...
        const service = createStormMemoryService(pool);
        const lookups = await service.getStormsByZipCode(zipCode, userOnly === 'true' ? userId : undefined);
        res.json({
            success: true,
            zipCode,
...
router.get('/by-city', async (req, res) => {
    try {
        ...
        const service = createStormMemoryService(pool);
        const lookups = await service.getStormsByCity(city, state, userOnly === 'true' ? userId : undefined);
        res.json({
            success: true,
            city,
            state,
...
router.get('/stats', async (req, res) => {
    try {
        ...
        const { userOnly = 'false' } = req.query;
        const service = createStormMemoryService(pool);
        const stats = await service.getStormStats(userOnly === 'true' ? userId : undefined);
        res.json({
            success: true,
            stats
        });
 ...
 */
router.delete('/:lookupId', async (req, res) => {
    try {
        const pool = getPool(req);
        const userEmail = req.headers['x-user-email'];
        if (!userEmail) {
...
router.get('/by-address', async (req, res) => {
    try {
        ...
        const { userOnly = 'false' } = req.query;
        const result = await pool.query(userOnly === 'true'
            ? `SELECT * FROM storm_lookups
           WHERE user_id = $1 AND LOWER(address) LIKE LOWER($2)
           ORDER BY created_at DESC LIMIT 1`
            : `SELECT * FROM storm_lookups
           WHERE LOWER(address) LIKE LOWER($1)
           ORDER BY created_at DESC LIMIT 1`, userOnly === 'true' ? [userId, `%${address}%`] : [`%${address}%`]);
...
router.get('/recent', async (req, res) => {
    try {
        ...
        const result = await pool.query(userOnly === 'true'
           ...
           ORDER BY created_at DESC LIMIT $1`, userOnly === 'true' ? [userId, limitNum] : [limitNum]);
        const lookups = result.rows.map((row) => service['rowToStormLookup'](row));
        res.json({
            success: true,
            count: lookups.length,
            lookups
 ...
 * }
 */
router.put('/:lookupId/outcome', async (req, res) => {
    try {
        const pool = getPool(req);
        const userEmail = req.headers['x-user-email'];
        if (!userEmail) {
