...
let _cache = null;
let _cacheAt = 0;
export async function ensureContentTable(pool) {
...
export async function loadContent(pool) {
    if (_cache && Date.now() - _cacheAt < 10000)
        return _cache;
    const map = {};
    try {
        const r = await pool.query(`SELECT page, ckey, val FROM lead_page_content`);
        for (const row of r.rows) {
            (map[row.page] ||= {})[row.ckey] = row.val;
        }
    }
...
export function pc(content, page, key) {
...
export function sec(content, page, key) {
    const v = content?.[page]?.[key];
...
export function markEm(text) {
    return esc(text).replace(/\*([^*]+)\*/g, '<em>$1</em>');
}
export function registerLeadContent(app, pool) {
    ...
    app.get('/admin/content', async (req, res, next) => {
        try {
            ...
            if (!email)
                return res.status(401).send('<h1>401</h1><p>Pass ?email=&lt;your email&gt; to edit content.</p>');
            if (!(await canManageQR(pool, email)))
                return res.status(403).send(`<h1>403</h1><p>${esc(email)} — marketing or admin only.</p>`);
    ...
    app.post('/admin/content/save', async (req, res) => {
        try {
            ...
            const upsert = (ckey, val) => pool.query(`INSERT INTO lead_page_content (page, ckey, val, updated_by, updated_at) VALUES ($1,$2,$3,$4,now())
         ON CONFLICT (page, ckey) DO UPDATE SET val=$3, updated_by=$4, updated_at=now()`, [page, ckey, val, email]);
            for (const f of def.fields)
                await upsert(f.key, String(fields[f.key] ?? ''));
            for (const t of def.toggles || [])
                await upsert(t.key, toggles[t.key] ? '1' : '0');
            bustContentCache();
            res.json({ ok: true });
        }
        catch (err) {
...
function renderContentEditor(content, email) {
