...
import { createOAuth2Client, saveTokens, getGoogleStatus, revokeTokens, } from '../services/googleTokenService.js';
const SCOPES = [
    'https://www.googleapis.com/auth/calendar',
    'https://www.googleapis.com/auth/gmail.send',
    'https://www.googleapis.com/auth/userinfo.email',
];
export function createGoogleOAuthRoutes(pool) {
    const router = Router();
    ...
    router.get('/auth-url', async (req, res) => {
        try {
            ...
            await pool.query(`INSERT INTO google_oauth_tokens (user_id, access_token_encrypted, refresh_token_encrypted,
    ...
    router.get('/callback', async (req, res) => {
        try {
            ...
            // Validate CSRF nonce
            const nonceCheck = await pool.query(`SELECT scope FROM google_oauth_tokens
         WHERE user_id = $1 AND access_token_encrypted = 'pending' AND revoked_at IS NULL`, [stateData.userId]);
            ...
            try {
                ...
                console.warn('[GoogleOAuth] Could not fetch userinfo, using app email:', infoErr.message);
