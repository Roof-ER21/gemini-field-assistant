...
router.get('/tokens', async (req, res) => {
    try {
        ...
        if (!userId)
            return res.status(404).json({ error: 'User not found' });
        const result = await pool.query(`SELECT id, device_type, device_name, is_active, notifications_enabled, created_at,
              LEFT(device_token, 60) AS token_preview
       FROM push_tokens WHERE user_id = $1 ORDER BY created_at DESC`, [userId]);
...
router.post('/phone', async (req, res) => {
    try {
        ...
        await pool.query(`UPDATE users SET phone_number = $2, sms_alerts_enabled = $3 WHERE id = $1`, [userId, cleaned || null, sms_enabled !== false]);
...
router.get('/phone', async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT phone_number, sms_alerts_enabled FROM users WHERE id = $1`, [userId]);
