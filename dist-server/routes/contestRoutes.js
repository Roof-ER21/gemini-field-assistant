...
export function createContestRoutes(pool) {
    const router = Router();
    // ============================================================================
    ...
    router.get('/contests', async (req, res) => {
      ...
      `;
            ...
            // Filter by type
            if (type && ['company_wide', 'team_based', 'individual'].includes(type)) {
                params.push(type);
     ...
     */
    router.get('/contests/:id/my-standing', async (req, res) => {
        try {
            const contestId = parseInt(req.params.id);
     ...
     * Create new contest
     */
    router.post('/admin/contests', async (req, res) => {
        try {
            const { name, description, contest_type, metric_type, start_date, end_date, is_monthly, prize_description, rules, participants } = req.body;
            const userEmail = req.header('x-user-email')?.toLowerCase().trim();
            // Get user ID
            let userId = null;
            if (userEmail) {
                const userResult = await pool.query('SELECT id FROM users WHERE LOWER(email) = $1', [userEmail]);
        ...
        `, [
            ...
            if (pushService) {
                const participantUsers = await pool.query(`SELECT DISTINCT u.id FROM users u
           ...
           WHERE u.is_active = TRUE`);
                for (const p of participantUsers.rows) {
                    ...
                    }, 'contest_alert').catch((e) => console.error('Contest push error:', e.message));
                }
            }
            res.status(201).json({
                success: true,
     ...
     * Update contest
     */
    router.put('/admin/contests/:id', async (req, res) => {
        try {
            const contestId = parseInt(req.params.id);
            const { name, description, contest_type, metric_type, start_date, end_date, is_monthly, prize_description, rules, is_active } = req.body;
     ...
     * Delete contest
     */
    router.delete('/admin/contests/:id', async (req, res) => {
        try {
            const contestId = parseInt(req.params.id);
            const result = await pool.query('DELETE FROM contests WHERE id = $1 RETURNING id', [contestId]);
            if (result.rows.length === 0) {
                return res.status(404).json({
                    success: false,
                    error: 'Contest not found'
     ...
     */
    router.post('/admin/contests/:id/participants', async (req, res) => {
        try {
            const contestId = parseInt(req.params.id);
     ...
     */
    router.delete('/admin/contests/:id/participants/:repId', async (req, res) => {
        try {
            const contestId = parseInt(req.params.id);
            const repId = parseInt(req.params.repId);
            const result = await pool.query('DELETE FROM contest_participants WHERE contest_id = $1 AND sales_rep_id = $2 RETURNING id', [contestId, repId]);
            if (result.rows.length === 0) {
                return res.status(404).json({
                    success: false,
     ...
     */
    router.post('/admin/contests/:id/refresh-standings', async (req, res) => {
        try {
            const contestId = parseInt(req.params.id);
