...
export function createAgentTaskRoutes(pool) {
    ...
    router.get('/', async (req, res) => {
        try {
            ...
            const result = await pool.query(`SELECT * FROM agent_tasks
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            const result = await pool.query(`INSERT INTO agent_tasks (user_id, title, description, task_type, due_at, priority, job_id, job_number, metadata)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)
         RETURNING *`, [userId, title, description || null, task_type || 'followup', due_at, priority || 'medium', job_id || null, job_number || null, metadata || {}]);
    ...
    router.put('/:id', async (req, res) => {
        try {
            ...
            const result = await pool.query(`UPDATE agent_tasks SET
           status = COALESCE($3, status),
           due_at = COALESCE($4, due_at),
           completed_at = CASE WHEN $3 = 'done' THEN NOW() ELSE completed_at END
         WHERE id = $2 AND user_id = $1
         RETURNING *`, [userId, req.params.id, status || null, due_at || null]);
