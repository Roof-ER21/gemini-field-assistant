...
export function forwardLeadToCC24(lead, pool) {
    ...
    fetch(cc21Url, {
        ...
        if (!r.ok) {
            console.error('[CC24 forward] non-OK', r.status, txt.slice(0, 200));
            return;
        }
        ...
        if (pool && lead.leadId) {
            try {
                ...
                if (j?.jobId) {
                    await pool.query(`UPDATE profile_leads
                 SET cc24_job_id = $1, cc24_job_number = $2,
                     cc24_status = COALESCE(cc24_status, 'new_lead'), cc24_synced_at = NOW()
               WHERE id = $3`, [String(j.jobId), j.jobNumber != null ? String(j.jobNumber) : null, lead.leadId]);
    ...
    (async () => {
        try {
            ...
            for (const qid of Object.keys(qs)) {
                ...
                else if (t === 'control_textbox' && /first/.test(label)) {
                    body.set(`submission[${qid}]`, first);
                    gotName = true;
                }
                else if (t === 'control_textbox' && /last/.test(label)) {
                    body.set(`submission[${qid}]`, last);
                }
                else if (t === 'control_textbox' && /name/.test(label) && !gotName) {
                    body.set(`submission[${qid}]`, lead.homeownerName);
                    gotName = true;
                }
                else if (t === 'control_phone' && lead.homeownerPhone) {
                    body.set(`submission[${qid}_full]`, lead.homeownerPhone);
                    gotPhone = true;
                }
                else if (t === 'control_email' && lead.homeownerEmail) {
                    body.set(`submission[${qid}]`, lead.homeownerEmail);
                }
                else if (t === 'control_address' && lead.address) {
                    body.set(`submission[${qid}_addr_line1]`, lead.address);
                }
                else if ((t === 'control_hidden' || t === 'control_textbox') && /rep\s?slug|repslug/.test(label) && lead.rep) {
                    body.set(`submission[${qid}]`, lead.rep);
                }
                else if (t === 'control_textarea' && /comment/.test(label) && lead.message) {
                    body.set(`submission[${qid}]`, lead.message);
                }
            }
            if (!gotName || !gotPhone) {
            ...
            if (j.responseCode !== 200)
                console.error('[jotform push] api error', j.responseCode, String(j.message || '').slice(0, 120));
            else
                console.log(`[jotform push] submitted "${lead.homeownerName}" -> form ${formId} (${(j.content || {}).submissionID || '?'})`);
        }
        catch (e) {
...
export function forwardCompanyLeadToJotForm(jf) {
    if (!jf || !jf.firstName || !jf.lastName || !jf.email)
        return;
    const FORM = '251003812776049';
    const slotTime = { Morning: '09:00:00', Midday: '12:00:00', Afternoon: '14:00:00', Evening: '17:00:00' };
    (async () => {
        try {
            const b = new URLSearchParams();
            b.append('formID', FORM);
            b.append('simple_spc', FORM);
            b.append('website', '');
            b.append('q8_typeA', jf.firstName);
            b.append('q19_lastName', jf.lastName);
            b.append('q10_email', jf.email);
            if (jf.phone)
                b.append('q4_phoneNumber[full]', jf.phone);
            if (jf.addr1)
                b.append('q11_address[addr_line1]', jf.addr1);
            if (jf.addr2)
                b.append('q11_address[addr_line2]', jf.addr2);
            if (jf.city)
                b.append('q11_address[city]', jf.city);
            if (jf.state)
                b.append('q11_address[state]', jf.state);
            if (jf.zip)
                b.append('q11_address[postal]', jf.zip);
            (jf.areas || []).forEach((a) => { if (a)
                b.append('q16_selectThe[]', a); });
            if (jf.howHeard)
                b.append('q23_howDid', jf.howHeard);
            if (jf.howMore) {
                const h = jf.howHeard || '';
                if (h === 'Referral')
                    b.append('q25_provideName', jf.howMore);
                else if (h === 'Doorhanger')
                    b.append('q29_provideDoorhanger', jf.howMore);
                else if (h === 'Spoke to a Rep')
                    b.append('q30_provideName30', jf.howMore);
                else
                    b.append('q31_tellUs', jf.howMore);
            }
            if (jf.comments)
                b.append('q26_provideComments', jf.comments);
            ...
            if (date) {
                b.append('q5_appointment[date]', `${date} ${time}`);
                b.append('q5_appointment[duration]', '15');
                b.append('q5_appointment[timezone]', 'America/New_York');
            }
            ...
            console.error('[jotform company] failed:', e?.message);
        }
    })();
}
 ...
 */
async function provisionOwnProfile(pool, userId, userEmail, userName) {
    ...
    try {
        ...
        const linked = await client.query('SELECT * FROM employee_profiles WHERE user_id = $1 LIMIT 1', [userId]);
        if (linked.rows.length > 0) {
            await client.query('COMMIT');
            return linked.rows[0];
        }
        const adopted = await client.query(`UPDATE employee_profiles
       ...
       RETURNING *`, [userId, userEmail]);
        ...
        for (let attempt = 0; attempt < SELF_PROVISION_SLUG_ATTEMPTS; attempt++) {
            ...
            const inserted = await client.query(`INSERT INTO employee_profiles (name, email, slug, user_id, is_claimed)
         VALUES ($1, $2, $3, $4, TRUE)
         ON CONFLICT (slug) DO NOTHING
         RETURNING *`, [name, userEmail, slug, userId]);
 ...
 */
export async function resolveOwnProfile(pool, userId, userEmail, userName) {
 ...
 */
async function bindProfileEmail(pool, rawEmail, excludeProfileId) {
    if (rawEmail === undefined)
        return { email: undefined, userId: null, warning: null };
    if (typeof rawEmail !== 'string')
        return { email: null, userId: null, warning: null };
    const email = rawEmail.trim().toLowerCase();
    // Preserve "clear this field" semantics for an explicitly blank string.
    if (!email)
        return { email: rawEmail, userId: null, warning: null };
    const match = await pool.query(`SELECT u.id,
            (SELECT COUNT(*)::int FROM employee_profiles ep
              WHERE ep.user_id = u.id AND ($2::uuid IS NULL OR ep.id <> $2::uuid)) AS other_profiles
       FROM users u
      WHERE LOWER(u.email) = $1
      LIMIT 1`, [email, excludeProfileId || null]);
...
export function createProfileRoutes(pool) {
    ...
    async function isStrictAdmin(email) {
        return isAdminRole(pool, email);
    }
    // Helper: Check if feature is enabled
    async function isFeatureEnabled(key) {
        try {
            const result = await pool.query('SELECT enabled FROM feature_flags WHERE key = $1 LIMIT 1', [key]);
                ...
                if (repUserId && leadData.preferredDate) {
                    ...
                    if (calResult.success) {
                        ...
                        if (adminUserId) {
                            ...
                            if (repEmail)
                                fallbackAttendees.push(repEmail);
                            if (leadData.homeownerEmail)
                                fallbackAttendees.push(leadData.homeownerEmail);
    ...
    function runCompanyLeadIntegrations(leadId, leadData) {
        ...
        (async () => {
            try {
                ...
                const adminUserId = adminUserRow.rows[0]?.id || null;
                if (!adminUserId) {
                    console.error('[Company lead] no admin sender available — lead saved but not emailed');
                    return;
                }
                const serviceLabel = leadData.serviceType
                    ? SERVICE_LABELS[leadData.serviceType] || leadData.serviceType.replace(/_/g, ' ')
    ...
    router.get('/slug/:slug', async (req, res) => {
        try {
            ...
            const result = await pool.query(`SELECT * FROM employee_profiles WHERE slug = $1 AND is_active = true LIMIT 1`, [slug]);
         ...
         FROM profile_videos
         WHERE profile_id = $1
         ORDER BY display_order ASC`, [profile.id]);
            res.json({
                success: true,
                profile: {
                    ...profile,
     ...
     * calendar events.
     */
    router.post('/jotform-webhook', jotformParser, async (req, res) => {
        try {
            ...
            for (const [k, v] of Object.entries(fields)) {
                ...
                if (['selectthe', 'service', 'servicetype', 'typeofservice'].includes(suffix)) {
                    ...
                    if (list.length) {
                        servicesReadable = list.map((s) => String(s).trim().toLowerCase().replace(/\b\w/g, (c) => c.toUpperCase())).join(', ');
                        break;
                    }
                }
            }
    ...
    router.post('/track-scan', async (req, res) => {
        try {
            ...
            // Get profile ID
            const profileResult = await pool.query('SELECT id FROM employee_profiles WHERE slug = $1 LIMIT 1', [profileSlug]);
    ...
    router.post('/track-share', async (req, res) => {
        try {
            ...
            if (!profileSlug)
                return res.status(400).json({ success: false, error: 'Profile slug required' });
            const profileResult = await pool.query('SELECT id FROM employee_profiles WHERE slug = $1 LIMIT 1', [profileSlug]);
            const profileId = profileResult.rows[0]?.id || null;
            const userAgent = req.headers['user-agent'] || '';
            const ip = req.ip || (req.headers['x-forwarded-for'] || '');
            await pool.query(`INSERT INTO profile_shares (profile_id, profile_slug, share_type, user_agent, ip_hash)
         VALUES ($1, $2, $3, $4, $5)`, [profileId, profileSlug, shareType, userAgent.substring(0, 500), hashIP(ip)]);
            res.json({ success: true });
        }
        catch (error) {
    ...
    router.get('/me', async (req, res) => {
        try {
            ...
            const userResult = await pool.query('SELECT id, name FROM users WHERE LOWER(email) = LOWER($1) LIMIT 1', [userEmail]);
    ...
    router.put('/me', async (req, res) => {
        try {
         ...
         SET name = COALESCE($1, name),
             ...
             updated_at = NOW()
         WHERE user_id = $7
         RETURNING *`, [name, title, bio, phone_number, image_url, start_year, userId]);
            if (result.rows.length === 0) {
                return res.status(404).json({
                    success: false,
    ...
    router.post('/claim/:id', async (req, res) => {
        try {
            ...
            // Get user
            const userResult = await pool.query('SELECT id, email FROM users WHERE LOWER(email) = LOWER($1) LIMIT 1', [userEmail]);
            if (userResult.rows.length === 0) {
                return res.status(404).json({
                    success: false,
                    error: 'User not found'
            ...
            const existingResult = await pool.query('SELECT id FROM employee_profiles WHERE user_id = $1 LIMIT 1', [user.id]);
            ...
            const profileResult = await pool.query('SELECT * FROM employee_profiles WHERE id = $1 LIMIT 1', [id]);
    ...
    router.post('/', async (req, res) => {
        try {
            ...
            const { name, title, email, phone_number, bio, image_url, role_type, start_year, slug: customSlug, user_id } = req.body;
            if (!name) {
                return res.status(400).json({
                    success: false,
                    error: 'Name is required'
            ...
            const slugCheck = await pool.query('SELECT id FROM employee_profiles WHERE slug = $1', [slug]);
    ...
    router.put('/:id', async (req, res) => {
        try {
            ...
            const { name, title, email, phone_number, bio, image_url, role_type, start_year, slug, is_active, user_id } = req.body;
         ...
         SET name = COALESCE($1, name),
             ...
             updated_at = NOW()
         WHERE id = $12
         RETURNING *`, [name, title, binding.email, phone_number, bio, image_url, role_type, start_year, slug, is_active, linkedUserId, id, userEmail || null]);
            if (result.rows.length === 0) {
                return res.status(404).json({
                    success: false,
                    error: 'Profile not found'
     ...
     */
    router.post('/:id/reset-claim', async (req, res) => {
        try {
            const userEmail = req.headers['x-user-email'];
            const { id } = req.params;
            if (!await canManageQRUser(userEmail)) {
      ...
      `);
            ...
            for (const user of users) {
             ...
             VALUES ($1, $2, $3, $4, TRUE, $5, $5)`, [name, user.email, slug, user.id, userEmail || null]);
                    ...
                    errors.push(`Failed to create profile for ${user.email}: ${err.message}`);
                }
            }
            res.json({
                success: true,
    ...
    router.post('/toggle-feature', async (req, res) => {
        try {
            ...
            await pool.query(`UPDATE feature_flags SET enabled = $1, updated_at = NOW() WHERE key = 'qr_profiles_enabled'`, [enabled === true]);
            res.json({
                success: true,
    ...
    router.get('/:id/videos', async (req, res) => {
        try {
            const { id } = req.params;
            const result = await pool.query(`SELECT * FROM profile_videos WHERE profile_id = $1 ORDER BY display_order ASC, created_at DESC`, [id]);
     ...
     * Remove a video
     */
    router.delete('/:profileId/videos/:videoId', async (req, res) => {
        try {
            const userEmail = req.headers['x-user-email'];
     ...
     */
    router.get('/reviews/global', async (_req, res) => {
        try {
            const r = await pool.query(`SELECT id, profile_id, text, author, date_label, source, source_url, rating, display_order, is_active, created_at
    ...
    router.put('/reviews/:reviewId', async (req, res) => {
        try {
            ...
            const r = await pool.query(`UPDATE profile_reviews SET
    ...
    router.delete('/reviews/:reviewId', async (req, res) => {
        try {
            ...
            const r = await pool.query('DELETE FROM profile_reviews WHERE id = $1 RETURNING id', [req.params.reviewId]);
            if (r.rowCount === 0)
                return res.status(404).json({ success: false, error: 'Review not found' });
            res.json({ success: true });
        }
