 ...
 */
export function ensureVeteranSchema(pool) {
    if (!schemaReady) {
        schemaReady = (async () => {
            await pool.query(`
 ...
 * not to email again.
 */
export async function recordVeteranNomination(pool, n) {
    await ensureVeteranSchema(pool);
    const result = await pool.query(`INSERT INTO veteran_nominations
...
export async function recordVeteranNotifyResult(pool, reference, result) {
    try {
        await pool.query(`UPDATE veteran_nominations
          SET email_status = $2, email_error = $3, notified_at = NOW()
        WHERE reference = $1`, [reference, result.success ? 'sent' : 'failed', result.success ? null : (result.error ?? 'unknown').slice(0, 500)]);
    }
    catch (e) {
        // The nomination itself is already safe; a missing status stamp is not
        // worth failing the webhook over.
        console.error('[veterans] notify-status update failed:', e?.message);
    }
}
export function createVeteransRoutes(pool) {
    const router = Router();
    /**
    ...
    router.get('/nominations', async (req, res) => {
        ...
        `),
            ...
            console.error('[veterans] nominations fetch error:', error?.message);
