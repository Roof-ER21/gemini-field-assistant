...
export function createLiveKitRoutes(pool) {
    ...
    const notifyTeamGoLive = async (req, hostUserId, sessionTitle) => {
        ...
        try {
            ...
            const usersResult = await pool.query(`SELECT id FROM users WHERE id != $1 AND is_active = true`, [hostUserId]);
    ...
    const notifyIncomingCall = async (req, calleeId, callerName, inviteId) => {
        const push = getPushService(req);
        if (!push)
            return;
        try {
    ...
    router.post('/api/livekit/sessions', async (req, res) => {
        try {
            ...
            if (type === 'broadcast') {
                const existing = await pool.query(`SELECT id FROM live_sessions WHERE host_user_id = $1 AND status = 'active' AND session_type = 'broadcast'`, [userId]);
            ...
            const result = await pool.query(`INSERT INTO live_sessions (room_name, title, host_user_id, status, participant_count, session_type, job_id)
         VALUES ($1, $2, $3, 'active', 1, $4, $5)
         RETURNING *`, [roomName, sessionTitle, userId, type, jobId || null]);
    ...
    // SESSION HISTORY
    // =====================================================
    router.get('/api/livekit/sessions/history', async (req, res) => {
        try {
      ...
      `, [limit, offset]);
            ...
            const countResult = await pool.query(`SELECT COUNT(*) FROM live_sessions WHERE status = 'ended'`);
    ...
    router.post('/api/livekit/sessions/force-end', async (req, res) => {
        try {
         ...
         WHERE host_user_id = $1 AND status = 'active' RETURNING id`, [userId]);
    ...
    // IN-ROOM CHAT
    // =====================================================
    router.post('/api/livekit/sessions/:id/messages', async (req, res) => {
        try {
            const { id } = req.params;
            ...
            const session = await pool.query(`SELECT id FROM live_sessions WHERE id = $1 AND status = 'active'`, [id]);
            if (session.rows.length === 0) {
                return res.status(404).json({ error: 'Session not found or ended' });
            }
            const result = await pool.query(`INSERT INTO live_session_messages (session_id, user_id, message)
         VALUES ($1, $2, $3)
         RETURNING *`, [id, userId, message.trim()]);
            // Get sender name
    ...
    router.get('/api/livekit/sessions/:id/messages', async (req, res) => {
        try {
            const { id } = req.params;
    ...
    // 1:1 CALLING
    // =====================================================
    router.post('/api/livekit/call', async (req, res) => {
        try {
            ...
            await pool.query(`UPDATE live_call_invites SET status = 'missed'
         WHERE caller_id = $1 AND callee_id = $2 AND status = 'ringing'
         AND created_at < NOW() - INTERVAL '60 seconds'`, [callerId, calleeId]);
            // Check if there's already a ringing call
            const existing = await pool.query(`SELECT id FROM live_call_invites
         WHERE caller_id = $1 AND callee_id = $2 AND status = 'ringing'`, [callerId, calleeId]);
            if (existing.rows.length > 0) {
            ...
            // Create session
            const sessionResult = await pool.query(`INSERT INTO live_sessions (room_name, title, host_user_id, status, participant_count, session_type)
         VALUES ($1, $2, $3, 'active', 1, 'call')
         RETURNING *`, [roomName, title, callerId]);
            await pool.query(`INSERT INTO live_session_participants (session_id, user_id, role) VALUES ($1, $2, 'host')`, [sessionResult.rows[0].id, callerId]);
            // Create invite
            const inviteResult = await pool.query(`INSERT INTO live_call_invites (caller_id, callee_id, session_id, status)
         VALUES ($1, $2, $3, 'ringing')
         RETURNING *`, [callerId, calleeId, sessionResult.rows[0].id]);
    ...
    router.get('/api/livekit/call/pending', async (req, res) => {
        try {
    ...
    router.post('/api/livekit/call/:id/accept', async (req, res) => {
        try {
            const { id } = req.params;
            const { userId } = req.body;
            if (!userId)
                return res.status(400).json({ error: 'userId is required' });
            const invite = await pool.query(`SELECT * FROM live_call_invites WHERE id = $1 AND callee_id = $2 AND status = 'ringing'`, [id, userId]);
            if (invite.rows.length === 0) {
                return res.status(404).json({ error: 'Call invite not found or expired' });
            }
            // Update invite
            await pool.query(`UPDATE live_call_invites SET status = 'accepted', answered_at = NOW() WHERE id = $1`, [id]);
      ...
      `, [sessionId, userId]);
            await pool.query(`UPDATE live_sessions SET participant_count = 2, max_participants = GREATEST(max_participants, 2) WHERE id = $1`, [sessionId]);
            // Get session info
    ...
    router.post('/api/livekit/call/:id/decline', async (req, res) => {
        try {
            const { id } = req.params;
            const { userId } = req.body;
            await pool.query(`UPDATE live_call_invites SET status = 'declined', ended_at = NOW()
         WHERE id = $1 AND callee_id = $2 AND status = 'ringing'`, [id, userId]);
            ...
            if (invite.rows[0]?.session_id) {
                await pool.query(`UPDATE live_sessions SET status = 'ended', ended_at = NOW() WHERE id = $1 AND status = 'active'`, [invite.rows[0].session_id]);
            }
            res.json({ success: true });
        }
        catch (error) {
    ...
    router.post('/api/livekit/call/:id/cancel', async (req, res) => {
        try {
            const { id } = req.params;
            const { userId } = req.body;
            await pool.query(`UPDATE live_call_invites SET status = 'cancelled', ended_at = NOW()
         WHERE id = $1 AND caller_id = $2 AND status = 'ringing'`, [id, userId]);
    ...
    // =====================================================
    router.post('/api/livekit/sessions/:id/record/start', async (req, res) => {
        try {
            const { id } = req.params;
            const { userId } = req.body;
            if (!userId)
            ...
            const activeRecording = await pool.query(`SELECT id FROM live_session_recordings WHERE session_id = $1 AND status = 'recording'`, [id]);
            ...
            try {
                ...
                const egress = await egressClient.startRoomCompositeEgress(session.rows[0].room_name, { file: output });
                const egressId = egress.egressId;
                const recording = await pool.query(`INSERT INTO live_session_recordings (session_id, egress_id, status, started_by)
           VALUES ($1, $2, 'recording', $3)
           RETURNING *`, [id, egressId, userId]);
                res.json({ recording: recording.rows[0] });
            }
            catch (egressError) {
                console.error('[LiveKit] Egress start error:', egressError);
                // Store failed recording attempt
                await pool.query(`INSERT INTO live_session_recordings (session_id, status, started_by, error_message)
           VALUES ($1, 'failed', $2, $3)`, [id, userId, egressError.message || 'Egress API error']);
                res.status(500).json({
    ...
    router.post('/api/livekit/sessions/:id/record/stop', async (req, res) => {
        try {
            const { id } = req.params;
            const recording = await pool.query(`SELECT * FROM live_session_recordings WHERE session_id = $1 AND status = 'recording' LIMIT 1`, [id]);
            ...
            await pool.query(`UPDATE live_session_recordings SET status = 'processing', completed_at = NOW() WHERE id = $1`, [rec.id]);
    ...
    router.get('/api/livekit/sessions/:id/recordings', async (req, res) => {
        try {
            const { id } = req.params;
            const result = await pool.query(`SELECT r.*, u.name AS started_by_name
         FROM live_session_recordings r
         LEFT JOIN users u ON r.started_by = u.id
         WHERE r.session_id = $1
         ORDER BY r.started_at DESC`, [id]);
