...
const validatePhotoUpload = (photo_data, mime_type) => {
    if (!photo_data) {
 ...
 */
router.get('/:token([a-z0-9]{20,})', async (req, res) => {
    try {
        const pool = getPool(req);
 ...
 */
router.post('/:token([a-z0-9]{20,})/analytics', async (req, res) => {
    try {
        const pool = getPool(req);
        const { token } = req.params;
...
router.post('/', async (req, res) => {
    try {
        ...
        const { job_id, property_address, customer_name, inspection_date, inspector_notes, weather_conditions, roof_type, roof_age, } = req.body;
        // Validation
...
router.post('/:id/photos', async (req, res) => {
    try {
        ...
        const { photo_data, // base64 string
        file_name, file_size, mime_type, category = 'other', notes, } = req.body;
        ...
        // Store photo
        const result = await pool.query(`INSERT INTO inspection_photos (
      ...
      RETURNING *`, [
        ...
        await pool.query(`UPDATE inspections
...
router.post('/:id/analyze', async (req, res) => {
    try {
        ...
        // Check access
        const inspectionCheck = await pool.query('SELECT user_id, property_address, customer_name, roof_type, roof_age FROM inspections WHERE id = $1', [id]);
        if (inspectionCheck.rows.length === 0) {
            return res.status(404).json({ error: 'Inspection not found' });
        }
        const userCheck = await pool.query('SELECT role FROM users WHERE id = $1', [userId]);
        ...
        const photosResult = await pool.query(`SELECT id, photo_data, file_name, mime_type, category
       FROM inspection_photos
...
FORMAT YOUR RESPONSE AS JSON:
{
  ...
  "detailedAnalysis": "Insurance adjuster-focused analysis"
                ...
                await pool.query(`UPDATE inspection_photos
           SET ai_analysis = $1, analyzed_at = NOW()
           WHERE id = $2`, [JSON.stringify(analysis), photo.id]);
                analyzedCount++;
            }
            catch (photoError) {
...
router.post('/presentations', async (req, res) => {
    try {
        ...
        // Get inspection details
        const inspectionResult = await pool.query(`SELECT * FROM inspections WHERE id = $1`, [inspection_id]);
...
router.put('/presentations/:id', async (req, res) => {
    try {
        ...
        // Check ownership
        const existing = await pool.query('SELECT user_id FROM presentations WHERE id = $1', [id]);
        ...
        const { title, presentation_type, slides, branding, status, } = req.body;
        const result = await pool.query(`UPDATE presentations SET
 ...
 */
router.post('/presentations/:id/share', async (req, res) => {
    try {
        const pool = getPool(req);
        const { id } = req.params;
        const userEmail = req.headers['x-user-email'];
        ...
        // Check ownership
        const existing = await pool.query('SELECT user_id, share_token FROM presentations WHERE id = $1', [id]);
        if (existing.rows.length === 0) {
            return res.status(404).json({ error: 'Presentation not found' });
        }
        const userCheck = await pool.query('SELECT role FROM users WHERE id = $1', [userId]);
        ...
        if (!shareToken) {
            ...
            await pool.query(`UPDATE presentations
         SET share_token = $1, is_public = true, status = 'shared', updated_at = NOW()
         WHERE id = $2`, [shareToken, id]);
        }
        else {
