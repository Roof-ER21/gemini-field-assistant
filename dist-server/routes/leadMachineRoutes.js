...
export function createLeadMachineRoutes(pool) {
    const router = Router();
    // =========================================================================
    // Auth helper
    // =========================================================================
     ...
     */
    router.post('/storm-blast', async (req, res) => {
        ...
        try {
            ...
            const existingZone = await pool.query(`SELECT id FROM storm_zones WHERE zip_code = $1 AND is_active = true
         AND event_date >= NOW() - INTERVAL '7 days' LIMIT 1`, [zipCode]);
            if (existingZone.rows.length > 0) {
                stormZoneId = existingZone.rows[0].id;
            }
            else {
                const newZone = await pool.query(`INSERT INTO storm_zones (zip_code, city, state, event_type, event_date, severity, source, is_active)
           VALUES ($1, $2, $3, $4, NOW(), $5, 'blast', true)
           RETURNING id`, [zipCode, city || null, state || 'MD', eventType, severity || 'moderate']);
            ...
            const campaign = await pool.query(`INSERT INTO storm_blast_campaigns
         (storm_zone_id, zip_code, city, state, event_type, severity, status, landing_page_url, started_at)
         VALUES ($1, $2, $3, $4, $5, $6, 'active', $7, NOW())
         RETURNING *`, [stormZoneId, zipCode, city || null, state || 'MD', eventType, severity || 'moderate', landingPageUrl]);
            ...
            const pastCustomers = await pool.query(`SELECT DISTINCT
         ...
         LIMIT 200`, [zipCode]);
            ...
            for (const contact of allContacts) {
                const contactResult = await pool.query(`INSERT INTO storm_blast_contacts (campaign_id, name, phone, email, address, zip_code, source)
           ...
           RETURNING id`, [
                ...
                if (contact.phone && twilioService.isConfigured()) {
                    const smsMessage = `${firstName}, a ${eventType} event was reported near ${zipCode}. ` +
                    ...
                    try {
                        ...
                        if (result.success) {
                            smsSent++;
                            await pool.query('UPDATE storm_blast_contacts SET sms_sent = true WHERE id = $1', [contactId]);
                        }
                    }
                    catch {
                        // Non-fatal
              ...
              `,
                        ...
                        emailsSent++;
                        await pool.query('UPDATE storm_blast_contacts SET email_sent = true WHERE id = $1', [contactId]);
                    }
                    catch {
                        // Non-fatal
                    }
            ...
            await pool.query(`UPDATE storm_blast_campaigns
         SET sms_sent = $1, emails_sent = $2, total_contacts = $3, status = 'completed', completed_at = NOW()
         WHERE id = $4`, [smsSent, emailsSent, allContacts.length, campaignId]);
            notifyTelegram(`🌩️ Storm Blast Complete\nZIP: ${zipCode} | ${eventType}\n` +
     ...
     */
    router.get('/storm-blasts', async (req, res) => {
        if (!(await requireAdmin(req, res)))
            return;
        try {
            const result = await pool.query(`SELECT * FROM storm_blast_campaigns ORDER BY created_at DESC LIMIT 50`);
     ...
     */
    router.post('/door-knock', async (req, res) => {
        if (!(await requireAdmin(req, res)))
            return;
        try {
            ...
            const campaign = await pool.query(`INSERT INTO door_knock_campaigns
         ...
         RETURNING *`, [
            ...
            const neighborAddresses = addresses || [];
            let contacted = 0;
            for (const addr of neighborAddresses) {
                await pool.query(`INSERT INTO door_knock_addresses
           ...
           VALUES ($1, $2, $3, $4, $5, $6, $7)`, [
                ...
                if (addr.phone && twilioService.isConfigured()) {
                    ...
                    const baseUrl = process.env.PUBLIC_URL || `https://${req.get('host')}`;
                    const smsMessage = `Hi ${firstName}! The Roof Docs just completed a roof project on your street (${jobAddress}). ` +
                    ...
                    try {
                        ...
                        if (result.success) {
                            contacted++;
                            await pool.query(`UPDATE door_knock_addresses SET contacted = true, contacted_at = NOW()
                 WHERE campaign_id = $1 AND address = $2`, [campaignId, addr.address]);
                        }
                    }
                    catch {
                        // Non-fatal
            ...
            await pool.query(`UPDATE door_knock_campaigns
         SET total_addresses = $1, contacted = $2
         WHERE id = $3`, [neighborAddresses.length, contacted, campaignId]);
            notifyTelegram(`🚪 Door Knock Campaign\nOrigin: ${jobAddress}\n` +
     ...
     */
    router.get('/door-knocks', async (req, res) => {
        if (!(await requireAdmin(req, res)))
            return;
        try {
            const result = await pool.query(`SELECT * FROM door_knock_campaigns ORDER BY created_at DESC LIMIT 50`);
            res.json({ campaigns: result.rows });
        }
        catch (error) {
     ...
     */
    router.post('/door-knock/auto-trigger', async (req, res) => {
        try {
            ...
            const existing = await pool.query(`SELECT id FROM door_knock_campaigns WHERE job_id = $1 LIMIT 1`, [jobId]);
         ...
         RETURNING id`, [jobId, prop.address || 'Unknown', prop.zip || null, prop.city || null, prop.state || 'MD']);
            notifyTelegram(`🚪 Door Knock Ready\nJob completed at: ${prop.address || 'Unknown'}\n` +
    ...
    router.post('/gbp-posts', async (req, res) => {
        if (!(await requireAdmin(req, res)))
            return;
        try {
            const { postType, title, content, imageUrl, callToAction, ctaUrl, scheduledFor, autoGenerated, generationSource, sourceId, } = req.body;
            if (!content) {
                return res.status(400).json({ error: 'content is required' });
            }
            const status = scheduledFor ? 'scheduled' : 'draft';
            const result = await pool.query(`INSERT INTO gbp_posts
     ...
     */
    router.post('/gbp-posts/auto-generate', async (req, res) => {
        if (!(await requireAdmin(req, res)))
            return;
        try {
            const postsCreated = [];
            // 1. Storm-based posts — recent storms in service area
            const recentStorms = await pool.query(`SELECT * FROM storm_zones
         ...
         ORDER BY event_date DESC LIMIT 3`);
            for (const storm of recentStorms.rows) {
                // Don't duplicate
                const exists = await pool.query(`SELECT id FROM gbp_posts WHERE generation_source = 'storm_alert' AND source_id = $1`, [storm.id]);
                if (exists.rows.length > 0)
                    continue;
                const content = `⚠️ Recent ${storm.event_type} damage reported in ${storm.city || storm.zip_code}!\n\n` +
                ...
                const post = await pool.query(`INSERT INTO gbp_posts
           ...
           RETURNING *`, [`Storm Alert: ${storm.event_type} in ${storm.zip_code}`, content, storm.id]);
                postsCreated.push(post.rows[0]);
            }
            // 2. Job completion posts — celebrate recent work
            const recentJobs = await pool.query(`SELECT j.id, j.title, j.property->>'city' as city, j.property->>'state' as state
         FROM jobs j
         WHERE j.status = 'completed'
           AND j.updated_at >= NOW() - INTERVAL '7 days'
         ORDER BY j.updated_at DESC LIMIT 3`);
            for (const job of recentJobs.rows) {
                const exists = await pool.query(`SELECT id FROM gbp_posts WHERE generation_source = 'job_complete' AND source_id = $1`, [job.id]);
                if (exists.rows.length > 0)
                    continue;
                const content = `✅ Another successful project completed in ${job.city || 'the area'}!\n\n` +
           ...
           RETURNING *`, [`Project Complete in ${job.city || 'Your Area'}`, content, job.id]);
                postsCreated.push(post.rows[0]);
            }
...
The Roof Docs — Protecting Your Home Since Day One`,
     ...
     */
    router.post('/craigslist-posts', async (req, res) => {
        if (!(await requireAdmin(req, res)))
            return;
        try {
            ...
            const status = scheduledFor ? 'scheduled' : 'draft';
            const result = await pool.query(`INSERT INTO craigslist_posts
         (market, title, body, zip_code, template_name, status, scheduled_for, auto_generated)
         VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
         RETURNING *`, [market, postTitle, postBody, zipCode || null, templateName || null, status, scheduledFor || null, !!templateName]);
            res.json({ success: true, post: result.rows[0] });
        }
        catch (error) {
     ...
     */
    router.get('/craigslist-posts', async (req, res) => {
        if (!(await requireAdmin(req, res)))
            return;
        try {
     ...
     */
    router.get('/craigslist-templates', (_req, res) => {
     ...
     */
    router.post('/craigslist-posts/batch', async (req, res) => {
        if (!(await requireAdmin(req, res)))
            return;
        try {
           ...
           RETURNING *`, [market, template.title, template.body, templateName, scheduledFor ? 'scheduled' : 'draft', scheduledFor || null]);
                posts.push(result.rows[0]);
            }
            notifyTelegram(`📋 Craigslist Batch Created\nTemplate: ${templateName}\n` +
...
The Roof Docs Team`;
     ...
     */
    router.post('/hoa-contacts', async (req, res) => {
        if (!(await requireAdmin(req, res)))
            return;
        try {
            const { companyName, contactName, email, phone, website, city, state, zipCodes, estimatedUnits, source, notes, } = req.body;
            if (!companyName) {
                return res.status(400).json({ error: 'companyName is required' });
            }
            const result = await pool.query(`INSERT INTO hoa_contacts
     ...
     */
    router.get('/hoa-contacts', async (req, res) => {
        if (!(await requireAdmin(req, res)))
            return;
        try {
     ...
     */
    router.post('/hoa-contacts/:id/send-email', async (req, res) => {
        if (!(await requireAdmin(req, res)))
            return;
        try {
            const { id } = req.params;
            ...
            try {
                ...
                await pool.query(`INSERT INTO hoa_email_log (hoa_contact_id, email_type, subject, body_preview, error_message)
           VALUES ($1, $2, $3, $4, $5)`, [id, templateType, template.subject, emailBody.substring(0, 200), String(emailErr)]);
                return res.status(500).json({ error: 'Failed to send email' });
            }
            // Log successful send
            await pool.query(`INSERT INTO hoa_email_log (hoa_contact_id, email_type, subject, body_preview)
         VALUES ($1, $2, $3, $4)`, [id, templateType, template.subject, emailBody.substring(0, 200)]);
            // Update contact status
            const stepMap = { intro: 1, followup: 2, value_add: 3 };
            const nextStep = stepMap[templateType] || 1;
            const nextEmailDays = templateType === 'intro' ? 3 : templateType === 'followup' ? 5 : 14;
            await pool.query(`UPDATE hoa_contacts
         SET status = CASE WHEN status = 'new' THEN 'contacted' ELSE status END,
             ...
             updated_at = NOW()
         WHERE id = $2`, [nextStep, id]);
            notifyTelegram(`📧 HOA Outreach Sent\nTo: ${hoa.company_name} (${hoa.email})\nType: ${templateType}`);
     ...
     */
    router.post('/hoa-contacts/batch-import', async (req, res) => {
        if (!(await requireAdmin(req, res)))
            return;
        try {
            ...
            for (const c of contacts) {
                ...
                if (c.email) {
                    const exists = await pool.query(`SELECT id FROM hoa_contacts WHERE LOWER(company_name) = LOWER($1) AND LOWER(email) = LOWER($2) LIMIT 1`, [c.companyName, c.email]);
                    if (exists.rows.length > 0) {
                        skipped++;
                        continue;
                    }
                }
                await pool.query(`INSERT INTO hoa_contacts
     ...
     */
    router.patch('/hoa-contacts/:id', async (req, res) => {
        if (!(await requireAdmin(req, res)))
            return;
        try {
            const { id } = req.params;
            ...
            values.push(id);
            const result = await pool.query(`UPDATE hoa_contacts SET ${sets.join(', ')} WHERE id = $${idx} RETURNING *`, values);
