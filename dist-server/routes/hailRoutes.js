    ...
    try {
        ...
        if (lat && lng) {
            ...
            try {
                ...
                const localAlerts = await pool.query(`SELECT event_type, event_date, magnitude, magnitude_unit, latitude, longitude, location, county, state, narrative, alert_phase
           FROM storm_alerts
           ...
           LIMIT 200`);
                ...
                const existingDates = new Set(noaaData.map((e) => `${e.date}-${(e.latitude || 0).toFixed?.(2) || e.latitude}`));
                let merged = 0;
...
router.get('/address-history', async (req, res) => {
    try {
        ...
        let recentAlerts = [];
        try {
            const alertResult = await pool.query(`SELECT * FROM (
           SELECT *,
             (3959 * acos(LEAST(1.0,
         ...
         ORDER BY event_date DESC`, [searchLat, searchLng]);
