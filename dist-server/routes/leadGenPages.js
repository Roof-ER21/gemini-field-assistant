...
const NO_REP = { slug: '', name: '', title: '', phone: ROOFCHECK_PHONE, image: '', isRep: false };
...
async function resolveRep(pool, slug) {
    const safe = slug.replace(/[^a-zA-Z0-9-]/g, '').slice(0, 64);
    if (!safe)
        return null;
    try {
        const r = await pool.query(`SELECT slug, name, phone_number AS phone, image_url AS image, title
         FROM employee_profiles WHERE slug = $1 LIMIT 1`, [safe]);
        const row = r.rows[0];
        if (!row)
            return null;
        return {
...
async function repContext(req, res, pool) {
    ...
    if (rep && qslug) {
        try {
            res.cookie('ler', rep.slug, { maxAge: 30 * 864e5, sameSite: 'lax', httpOnly: false });
        }
        catch { /* noop */ }
    }
    return rep || NO_REP;
...
// ---------------------------------------------------------------------------
function htmlHead(title, description, canonical) {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
...
function formScript(buttonLabel = 'Get Free Inspection') {
...
function navBar(phoneNumber = ROOFCHECK_PHONE) {
...
// ---------------------------------------------------------------------------
function renderStormPage(storm, zip) {
...
// ---------------------------------------------------------------------------
function renderReferralPage(rep, code) {
    const hasRep = !!rep;
    const repName = rep?.rep_name || '';
    const repTitle = rep?.rep_title || 'Roofing Specialist';
    const repPhone = rep?.rep_phone || '';
    const repImage = rep?.image_url || '';
...
// ---------------------------------------------------------------------------
function renderFreeInspectionPage() {
...
// Route registration
// ---------------------------------------------------------------------------
export function registerLeadGenPages(app, pool) {
    // ── Page 1: Storm Landing ────────────────────────────────────────────────
    app.get('/storm/:zip', async (req, res) => {
        const { zip } = req.params;
        // Basic ZIP validation — accept 5-digit US ZIPs only
        if (!/^\d{5}$/.test(zip)) {
            res.status(400).set('Content-Type', 'text/html').send(renderStormPage(null, zip.slice(0, 10)));
            return;
        }
        let storm = null;
        try {
            const result = await pool.query(`SELECT * FROM storm_zones WHERE zip_code = $1 AND is_active = true ORDER BY event_date DESC LIMIT 1`, [zip]);
    ...
    app.get('/refer/:code', async (req, res) => {
        ...
        if (safeCode) {
            try {
                const result = await pool.query(`SELECT rc.*, ep.name as rep_name, ep.image_url, ep.phone_number as rep_phone, ep.title as rep_title
...
// ---------------------------------------------------------------------------
function renderStormChecklistPage() {
    return `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
