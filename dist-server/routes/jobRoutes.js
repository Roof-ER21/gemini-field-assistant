...
router.get('/:id', async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT j.*, u.email as user_email, COALESCE(u.name, SPLIT_PART(u.email, '@', 1)) as user_name
       FROM jobs j
       JOIN users u ON j.user_id = u.id
       WHERE j.id = $1`, [id]);
...
router.post('/', async (req, res) => {
    try {
        ...
        const { title, status = 'new_lead', priority = 'medium', leadSource, customer, property, roofDetails, damage, insurance, financials, notes = [], attachments = [], actions = [], tags = [], inspectionDate, contractSignedDate, scheduledInstallDate, } = req.body;
        // Validation
...
router.put('/:id', async (req, res) => {
    try {
        ...
        const { title, status, priority, leadSource, customer, property, roofDetails, damage, insurance, financials, notes, attachments, actions, tags, inspectionDate, contractSignedDate, scheduledInstallDate, completedDate, linkedChatSessionId, linkedTranscriptIds, linkedEmailIds, linkedImageAnalysisIds, } = req.body;
...
router.post('/:id/notes', async (req, res) => {
    try {
        ...
        await pool.query('UPDATE jobs SET notes = $1 WHERE id = $2', [JSON.stringify(notes), id]);
...
router.post('/:id/actions', async (req, res) => {
    try {
        ...
        await pool.query('UPDATE jobs SET actions = $1 WHERE id = $2', [JSON.stringify(actions), id]);
...
router.patch('/:id/status', async (req, res) => {
    try {
        ...
        const result = await pool.query('UPDATE jobs SET status = $1 WHERE id = $2 RETURNING job_number, status', [status, id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Job not found' });
        }
