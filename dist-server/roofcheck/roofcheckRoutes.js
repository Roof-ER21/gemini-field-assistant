...
try {
    ...
    try {
        JOBS = JSON.parse(readFileSync(new URL('./completed-jobs.json', import.meta.url), 'utf8'));
    }
    catch {
        JOBS = [];
    }
...
function hailLabel(t) {
    if (t.maxHailInches && t.maxHailInches > 0)
        return `${Math.round(t.maxHailInches * 4) / 4}″ hail`;
    if (t.sizeLabel)
        return t.sizeLabel;
    return 'hail';
}
...
async function geocode(address) {
    try {
        ...
        if (m?.coordinates)
            return { lat: Number(m.coordinates.y), lng: Number(m.coordinates.x), normalized: m.matchedAddress || address };
    }
    catch { /* */ }
    try {
        ...
        const j = await r.json();
        if (j?.[0])
            return { lat: Number(j[0].lat), lng: Number(j[0].lon), normalized: j[0].display_name || address };
    }
    catch { /* */ }
    return null;
}
...
export function createRoofCheckRoutes(pool) {
    ...
    router.get('/admin/ihm-raw', async (req, res) => {
        const email = String(req.query.email || '').trim();
        if (!email || !(await canManageQR(pool, email)))
            return res.status(403).json({ ok: false, error: 'marketing/admin only — pass ?email=<your email>' });
        if (!ihmConfigured())
            return res.json({ ok: false, configured: false, note: 'Set IHM_API_USER/IHM_API_PASS (or IHM_BASIC_AUTH) in Railway first.' });
    ...
    router.post('/api/roofcheck/lookup', async (req, res) => {
        try {
            ...
            if (!isFinite(lat) || !isFinite(lng)) {
                ...
                if (address.length < 5) {
                    res.json({ ok: false, error: 'Please enter your full address.' });
                    return;
                }
                const geo = await geocode(address);
                if (!geo) {
                    res.json({ ok: false, error: "We couldn't find that address — add city and ZIP." });
                    return;
                }
                lat = geo.lat;
                lng = geo.lng;
    ...
    router.post('/api/roofcheck/lead', async (req, res) => {
        try {
            const b = req.body || {};
            ...
            if (!name || phone.replace(/\D/g, '').length < 7) {
                res.json({ ok: false, error: 'Please enter your name and phone.' });
                return;
            }
            if (email && !email.includes('@')) {
                res.json({ ok: false, error: 'That email looks off — leave it blank or fix it.' });
                return;
            }
            ...
            const tiers = report
                ? [...report.directHits.map((t) => ({ ...t, _dh: true })), ...report.nearMiss.map((t) => ({ ...t, _dh: false }))]
                    .sort((a, c) => String(c.date).localeCompare(String(a.date)))
                    .slice(0, 8)
                : [];
            const events = tiers.map((t) => ({
                date: fmtDate(t.date) || t.date,
                mag: hailLabel(t),
                where: t._dh ? 'At your home' : (t.nearestMiles != null ? `${Math.round(t.nearestMiles * 10) / 10} mi away` : 'Nearby'),
                direct: !!t._dh,
            }));
            const qualifying = !!report && (report.summary.directHitCount > 0 || tiers.some((t) => (t.maxHailInches || 0) >= 1));
            let within = 0, nearest = null, nd = 9e9;
            if (lat && lng)
                for (const j of JOBS) {
            ...
            const ins = await pool.query(`INSERT INTO profile_leads
