 ...
 */
export function getDamageScoreDescription(score) {
    if (score >= 80)
        return REPORT_CONTENT.damageScoreDescriptions.critical;
    if (score >= 60)
        return REPORT_CONTENT.damageScoreDescriptions.high;
    if (score >= 40)
        return REPORT_CONTENT.damageScoreDescriptions.moderate;
    if (score >= 20)
        return REPORT_CONTENT.damageScoreDescriptions.low;
    return REPORT_CONTENT.damageScoreDescriptions.minimal;
}
/**
 * Get summary text based on damage score and event data
 */
export function getSummaryText(score, eventCount, maxSize, severeCount) {
 ...
 */
export function formatHailSize(size) {
    if (size === 0)
        return 'Unknown';
    if (size < 0.5)
        return `${size.toFixed(2)}" (pea-sized)`;
    if (size < 0.75)
        return `${size.toFixed(2)}" (penny-sized)`;
    if (size < 1.0)
        return `${size.toFixed(2)}" (quarter-sized)`;
    if (size < 1.5)
        return `${size.toFixed(2)}" (half-dollar)`;
    if (size < 1.75)
        return `${size.toFixed(2)}" (golf ball)`;
    if (size < 2.0)
        return `${size.toFixed(2)}" (lime-sized)`;
    if (size < 2.75)
        return `${size.toFixed(2)}" (baseball-sized)`;
    if (size < 3.0)
        return `${size.toFixed(2)}" (tea cup)`;
    if (size < 4.0)
        return `${size.toFixed(2)}" (softball)`;
    return `${size.toFixed(2)}" (EXTREME)`;
}
/**
 * Get severity classification based on hail size
 */
export function getHailSeverity(size) {
    if (size < 0.75)
        return 'minimal';
    if (size < 1.0)
        return 'minor';
 ...
 */
export function getUrgencyMessage(score) {
    if (score >= 80)
        return REPORT_CONTENT.callToAction.urgencyMessages.critical;
    if (score >= 60)
        return REPORT_CONTENT.callToAction.urgencyMessages.high;
    if (score >= 40)
        return REPORT_CONTENT.callToAction.urgencyMessages.moderate;
    return REPORT_CONTENT.callToAction.urgencyMessages.low;
}
/**
 * Generate report title with property address
 */
export function generateReportTitle(address) {
 ...
 */
export function getDamageScoreColor(score) {
    return getDamageScoreDescription(score).color;
}
/**
 * Get icon for damage score
 */
export function getDamageScoreIcon(score) {
