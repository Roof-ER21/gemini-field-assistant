 ...
 */
async function clientProfileFields(userId) {
    if (!userId)
        return {};
    try {
        ...
        console.warn('[AUTH] could not read division/created_at:', err.message);
        return {};
    }
}
    ...
    try {
        ...
        if (groqKey) {
            try {
                ...
                console.warn('[AI] Groq failed, trying next provider:', groqError.message);
            }
        }
        // Try Together next
        if (!content && togetherKey) {
            try {
                ...
                console.warn('[AI] Together failed, trying next provider:', togetherError.message);
            }
        }
        // Try Gemini
        ...
        if (!content && hfKey) {
            try {
                const hfResponse = await fetch('https://api-inference.huggingface.co/models/meta-llama/Llama-3.2-3B-Instruct', {
                    ...
                    body: JSON.stringify({
                        inputs: formattedMessages.map((m) => m.content).join('\n'),
                ...
                console.warn('[AI] HuggingFace failed:', hfError.message);
            }
        }
        if (!content) {
...
app.post('/api/users/phone', async (req, res) => {
    try {
       ...
       SET phone_number = $1,
           updated_at = CURRENT_TIMESTAMP
       WHERE email = $2
       RETURNING id, email, name, phone_number, sms_alerts_enabled`, [phoneNumber, email]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        res.json(result.rows[0]);
...
app.put('/api/users/sms-alerts', async (req, res) => {
    try {
       ...
       SET sms_alerts_enabled = $1,
           updated_at = CURRENT_TIMESTAMP
       WHERE email = $2
       RETURNING id, email, name, phone_number, sms_alerts_enabled`, [enabled, email]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        res.json(result.rows[0]);
...
app.post('/api/users/test-sms', async (req, res) => {
    try {
        ...
        const userResult = await pool.query('SELECT id, phone_number FROM users WHERE email = $1', [email]);
       ...
       RETURNING *`, [
        ...
        if (rating === -1 && result.rows[0]?.id) {
            try {
                ...
                const prevMsg = await pool.query(`SELECT content FROM chat_history
           WHERE session_id = $1 AND sender = 'user'
           ORDER BY created_at DESC LIMIT 1`, [session_id]);
                const userQuery = prevMsg.rows[0]?.content || '';
                await createLearningFromFeedback(pool, result.rows[0].id, userQuery, response_excerpt || '', comment || undefined, tags || [], normalizedState, normalizedInsurer);
            }
            catch (learnErr) {
...
app.get('/api/chat/learning', async (req, res) => {
    try {
        ...
        const [positiveTags, negativeTags, recentWins, recentIssues, totals] = await Promise.all([
         ...
         ORDER BY count DESC
         LIMIT 6`, [windowStart]),
            pool.query(`SELECT comment, response_excerpt, created_at
         FROM chat_feedback
         ...
         FROM chat_feedback`, [weekStart, prevWeekStart, windowStart])
        ]);
...
const DEMO_EMAILS = []; // demo login removed (Ahmed, 2026-06-27)
...
app.post('/api/auth/check-email', async (req, res) => {
    try {
        ...
        const result = await pool.query('SELECT id, name, email FROM users WHERE LOWER(email) = $1', [normalizedEmail]);
        if (result.rows.length > 0) {
...
app.post('/api/auth/direct-login', async (req, res) => {
    try {
        ...
        const existingUser = await pool.query('SELECT id, name, email, role FROM users WHERE LOWER(email) = $1', [normalizedEmail]);
        let user;
        let isNew = false;
...
app.get('/api/auth/google/callback', async (req, res) => {
    try {
        ...
        if (!user)
            return res.redirect('/?gl_error=notapproved'); // REQUIRE_ADMIN_APPROVAL on, no account yet
...
app.post('/api/auth/google/exchange', async (req, res) => {
    ...
    try {
        ...
        console.error('[AUTH] session mint failed on google/exchange:', err.message);
...
app.post('/api/auth/logout', async (req, res) => {
    try {
        if (req.session)
            await revokeSession(pool, req.session.token);
        res.json({ success: true });
    }
    catch (err) {
...
app.get('/api/auth/session-adoption', (req, res) => {
    res.json({
        ...sessionAdoption(),
...
app.post('/api/auth/verify-code', async (req, res) => {
    try {
        ...
        try {
            ...
            console.error('[AUTH] session mint failed on verify-code:', err.message);
        }
        res.json({
            success: true,
            message: isNew ? 'Account created successfully!' : 'Login successful',
...
app.post('/api/admin/auth/set-pin', async (req, res) => {
    try {
        ...
        await pool.query('INSERT INTO admin_sessions (user_id, token, expires_at) VALUES ($1, $2, $3)', [user.id, token, expiresAt]);
...
app.post('/api/admin/trigger-qr-weekly-report', async (req, res) => {
    try {
        ...
        const recipients = sendTo ? (Array.isArray(sendTo) ? sendTo : String(sendTo).split(',')).map((s) => s.trim()).filter(Boolean) : undefined;
...
app.post('/api/admin/users', async (req, res) => {
    try {
        ...
        const expiresAt = new Date(Date.now() + 24 * 60 * 60 * 1000); // 24 hours
        // Create user
        const result = await pool.query(`INSERT INTO users (email, name, role, state, verification_code, verification_expires_at, is_verified)
       VALUES (LOWER($1), $2, $3, $4, $5, $6, false)
       RETURNING id, email, name, role, state, created_at, is_verified`, [email, name, role, state || null, verificationCode, expiresAt]);
...
app.put('/api/admin/users/:userId', async (req, res) => {
    try {
        ...
        values.push(userId);
        const result = await pool.query(`UPDATE users SET ${updates.join(', ')}, updated_at = NOW()
       WHERE id = $${paramCount}
       RETURNING id, email, name, role, state, created_at, is_verified`, values);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
...
app.post('/api/admin/users/:userId/resend-verification', async (req, res) => {
    try {
        ...
        const { userId } = req.params;
        // Get user
        const userResult = await pool.query('SELECT id, email, name, is_verified FROM users WHERE id = $1', [userId]);
        if (userResult.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
        const user = userResult.rows[0];
        ...
        await pool.query(`UPDATE users SET verification_code = $1, verification_expires_at = $2 WHERE id = $3`, [verificationCode, expiresAt, userId]);
        // Send verification email
        await emailService.sendVerificationCode({
            email: user.email,
            code: verificationCode
...
app.patch('/api/admin/users/:userId/status', async (req, res) => {
    try {
        ...
        const result = await pool.query(`UPDATE users SET is_active = $1, updated_at = NOW()
       WHERE id = $2
       RETURNING id, email, name, role, state, is_active`, [is_active, userId]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'User not found' });
        }
...
app.get('/api/admin/goals/:repId', async (req, res) => {
    try {
        ...
        // Get rep info
        const repResult = await pool.query('SELECT * FROM sales_reps WHERE id = $1', [repId]);
        if (repResult.rows.length === 0) {
            return res.status(404).json({ error: 'Sales rep not found' });
        }
...
app.get('/api/rep/goals', async (req, res) => {
    try {
        ...
        const repResult = await pool.query('SELECT id FROM sales_reps WHERE LOWER(email) = LOWER($1)', [requestingEmail]);
        if (repResult.rows.length === 0) {
    ...
    `);
        ...
        for (const email of marketingEmails) {
            ...
            const inferredName = email.split('@')[0]
                ...
                .join(' ');
            await pool.query(`INSERT INTO users (email, name, role) VALUES ($1, $2, 'marketing')
         ON CONFLICT (email) DO UPDATE SET role = 'marketing'
         WHERE users.role NOT IN ('admin', 'marketing')`, [email, inferredName]);
        }
...
app.post('/api/admin/user-mappings', async (req, res) => {
    try {
        ...
        const adminResult = await pool.query('SELECT id FROM users WHERE LOWER(email) = LOWER($1)', [email]);
...
app.delete('/api/admin/user-mappings/:id', async (req, res) => {
    try {
        ...
        const { id } = req.params;
        const result = await pool.query('DELETE FROM user_sales_rep_mapping WHERE id = $1 RETURNING *', [id]);
...
app.get('/api/admin/settings-history', async (req, res) => {
    try {
        ...
        const history = await settingsService.getSettingHistory(key, Number(limit));
        res.json({ history });
    }
    catch (error) {
...
app.post('/api/admin/budget/alerts/:id/acknowledge', async (req, res) => {
    try {
        ...
        const result = await pool.query(`UPDATE budget_alerts
       SET acknowledged = true,
           acknowledged_at = CURRENT_TIMESTAMP,
           acknowledged_by = $1
       WHERE id = $2
       RETURNING *`, [adminUser.rows[0].id, id]);
        if (result.rows.length === 0) {
...
app.get('/api/admin/budget/usage-log', async (req, res) => {
    try {
        ...
        const { page = 1, limit = 100, userId, provider, serviceType, startDate, endDate, export: exportCsv } = req.query;
        const offset = (Number(page) - 1) * Number(limit);
        // Build filters
        const filters = [];
        const params = [];
    ...
    `;
        ...
        if (exportCsv === 'true') {
            ...
            const csvRows = result.rows.map(row => `${row.created_at},${row.user_email},${row.provider_name},${row.service_type},${row.model_name},${row.input_tokens},${row.output_tokens},${row.total_tokens},${row.duration_ms},${row.estimated_cost},${row.feature_used},${row.success}`).join('\n');
...
app.put('/api/admin/budget/user/:userId', async (req, res) => {
    try {
        ...
        // Upsert user budget
        const result = await pool.query(`INSERT INTO user_budgets (user_id, monthly_budget, updated_at)
       VALUES ($1, $2, CURRENT_TIMESTAMP)
       ON CONFLICT (user_id)
       DO UPDATE SET
         monthly_budget = $2,
         updated_at = CURRENT_TIMESTAMP
       RETURNING *`, [userId, monthly_budget]);
...
app.put('/api/admin/budget/company', async (req, res) => {
    try {
        ...
        // Update company budget
        const result = await pool.query(`UPDATE company_budget
...
app.post('/api/memory', async (req, res) => {
    try {
        ...
        for (const memory of memories) {
            ...
            const result = await pool.query(`INSERT INTO user_memory (user_id, memory_type, category, key, value, confidence, source_type, source_session_id, source_message_id)
...
app.get('/api/memory/relevant', async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT * FROM get_relevant_memories(
...
app.post('/api/memory/:memoryId/feedback', async (req, res) => {
    try {
        ...
        // Log feedback
        await pool.query(`INSERT INTO memory_feedback (memory_id, user_id, feedback_type)
       SELECT $1, u.id, $2
       FROM users u WHERE LOWER(u.email) = LOWER($3)`, [memoryId, feedback, email]);
...
app.delete('/api/memory/:memoryId', async (req, res) => {
    try {
        const email = getRequestEmail(req);
        const { memoryId } = req.params;
        const result = await pool.query(`DELETE FROM user_memory
       WHERE id = $1 AND user_id = (SELECT id FROM users WHERE LOWER(email) = LOWER($2))
       RETURNING id`, [memoryId, email]);
        if (result.rows.length === 0) {
...
app.post('/api/memory/summaries', async (req, res) => {
    try {
        ...
        const { session_id, summary, key_facts, decisions_reached, open_questions, action_items, topics, insurers_mentioned, states_mentioned, job_numbers_mentioned, message_count, user_sentiment, conversation_start, conversation_end } = req.body;
        if (!session_id || !summary) {
            return res.status(400).json({ error: 'session_id and summary required' });
        }
        const result = await pool.query(`INSERT INTO conversation_summaries (
...
app.get('/api/memory/summaries', async (req, res) => {
    try {
        const email = getRequestEmail(req);
        const { limit = 10 } = req.query;
        const result = await pool.query(`SELECT cs.* FROM conversation_summaries cs
       JOIN users u ON cs.user_id = u.id
       WHERE LOWER(u.email) = LOWER($1)
       ORDER BY cs.created_at DESC
       LIMIT $2`, [email, Number(limit) || 10]);
        return res.json(result.rows);
    }
    catch (error) {
...
app.post('/api/email-patterns', async (req, res) => {
    try {
        ...
        const { email_type, insurer, state, subject_template, arguments_used, primary_argument, code_citations, tone, source_job_id, source_email_id } = req.body;
        if (!email_type) {
            return res.status(400).json({ error: 'email_type required' });
        }
        const result = await pool.query(`INSERT INTO email_patterns (
...
app.patch('/api/email-patterns/:patternId/outcome', async (req, res) => {
    try {
        ...
        const { outcome, outcome_notes, response_time_days, amount_approved, is_successful, success_factors, outcome_recorded_at } = req.body;
        if (!outcome) {
            return res.status(400).json({ error: 'outcome required' });
        }
        const result = await pool.query(`UPDATE email_patterns SET
         outcome = $2,
         outcome_notes = $3,
...
app.get('/api/email-patterns', async (req, res) => {
    try {
        const email = getRequestEmail(req);
        const { insurer, state, email_type, outcome, successful_only, limit = 20 } = req.query;
        let query = `
...
app.get('/api/jobs/:jobId/conversations', async (req, res) => {
    try {
        ...
        const result = await pool.query(`SELECT cs.session_id, cs.summary, jcl.job_specific_decisions as key_decisions,
...
app.post('/api/jobs/:jobId/conversations', async (req, res) => {
    try {
        ...
        const summaryResult = await pool.query(`INSERT INTO conversation_summaries (user_id, session_id, summary, topics, message_count)
       VALUES ($1, $2, $3, $4, 0)
       ON CONFLICT (user_id, session_id) DO UPDATE SET summary = EXCLUDED.summary
       RETURNING id`, [userId, session_id, summary, JSON.stringify(topics || [])]);
        const summaryId = summaryResult.rows[0].id;
        // Create the link
        await pool.query(`INSERT INTO job_conversation_links (job_id, summary_id, user_id, job_specific_decisions)
       VALUES ($1, $2, $3, $4)
       ON CONFLICT (job_id, summary_id) DO UPDATE SET
         job_specific_decisions = EXCLUDED.job_specific_decisions`, [jobId, summaryId, userId, JSON.stringify(key_decisions || [])]);
...
app.get('/vets', (req, res) => {
    ...
    try {
        ...
        if (rep)
            target.searchParams.set('rep', rep);
...
// ============================================================================
const serveCompanyLanding = async (_req, res) => {
    try {
        ...
        const repSlug = String(_req.query.rep || '').trim().toLowerCase();
        if (/^[a-z0-9][a-z0-9-]{0,80}$/.test(repSlug)) {
            const rp = await pool.query(`SELECT slug FROM employee_profiles WHERE slug = $1 AND is_active = true LIMIT 1`, [repSlug]);
            if (rp.rows[0]) {
                const qs = new URLSearchParams();
                for (const [k, v] of Object.entries(_req.query)) {
                    if (k !== 'rep' && typeof v === 'string')
                        qs.set(k, v);
                }
                if (!qs.has('src'))
                    qs.set('src', 'qr');
        ...
        const gr = await pool.query(`SELECT id, text, author, date_label, source, rating
...
app.get('/admin/qr-print', async (req, res, next) => {
    try {
        ...
        const result = await pool.query(`SELECT name, slug, phone_number, image_url, role_type
...
app.get('/profile/:slug', async (req, res, next) => {
    try {
        ...
        const featureResult = await pool.query("SELECT enabled FROM feature_flags WHERE key = 'qr_profiles_enabled' LIMIT 1");
        ...
        const profileResult = await pool.query('SELECT * FROM employee_profiles WHERE slug = $1 AND is_active = true LIMIT 1', [slug]);
       ...
       FROM profile_videos WHERE profile_id = $1 ORDER BY display_order ASC`, [profile.id]);
        ...
        pool.query(`INSERT INTO qr_scans (profile_id, profile_slug, user_agent, referrer, ip_hash, device_type, source, is_bot)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8)`, [
      ...
      <div class="hero-content">
        <div class="hero-left">
          ...
          <div class="contact-buttons">
            ...
            <button type="button" class="contact-btn" id="share-btn" data-slug="${profile.slug}" data-name="${String(name).replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]))}" style="cursor:pointer;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.05);font-family:inherit;font-size:14px;"><svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"/></svg>Share</button>
          </div>
          <script>
            (function(){
              ...
              btn.addEventListener('click', async function(){
                ...
                try {
                  if (navigator.share) {
                    await navigator.share({ title: name + ' — The Roof Docs', url: url });
                ...
                fetch('/api/profiles/track-share', {
                  method: 'POST',
                  headers: { 'Content-Type': 'application/json' },
                  body: JSON.stringify({ profileSlug: slug, shareType: shareType })
                }).catch(function(){});
              });
            })();
          </script>
        ...
        <div class="video-section">
          ${(profile.videos && profile.videos.length > 0) ? profile.videos.map((v) => {
        const vUrl = v.url || '';
  ...
  <section class="reviews-section">
    <div class="container">
      ...
      <div style="text-align:center;margin-top:24px;">
        <a href="https://www.google.com/maps/search/The+Roof+Docs+Vienna+VA" target="_blank" rel="noopener noreferrer" style="display:inline-flex;align-items:center;gap:8px;background:white;color:#1a1a1a;border:1px solid #e5e5e5;padding:10px 20px;border-radius:8px;text-decoration:none;font-weight:600;font-size:14px;box-shadow:0 1px 2px rgba(0,0,0,0.04);transition:all 0.2s;">
...
// ============================================================================
function renderProfilePageV2(profile) {
    const esc = (s) => String(s ?? '').replace(/[&<>"']/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));
    const escAttr = (s) => esc(s);
    const name = profile.name || 'Team Member';
    const firstName = String(name).trim().split(/\s+/)[0] || name;
    const initials = String(name).split(' ').map((n) => n[0]).join('').toUpperCase().substring(0, 2);
    const role = profile.title || getRoleLabel(profile.role_type);
    const imageUrl = profile.image_url || '';
    const email = profile.email || '';
    ...
    // Veterans Day roof giveaway button. Off unless VETERANS_GIVEAWAY_URL is set; rep pages pass
    // their slug so the nomination is credited to the rep who sent it.
    let veteransUrl = '';
    const veteransBase = (process.env.VETERANS_GIVEAWAY_URL || '').trim();
    if (veteransBase) {
        try {
            const u = new URL(veteransBase);
            u.searchParams.set('src', isCompany ? 'inspection' : 'rep-page');
            if (!isCompany && slug)
                u.searchParams.set('rep', slug);
            veteransUrl = u.toString();
        }
        catch { /* malformed env value: leave the button off */ }
    }
