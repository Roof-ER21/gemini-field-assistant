 ...
 */
export function promptReauthEnabled() {
    return String(process.env.SA21_PROMPT_REAUTH || '').toLowerCase() === 'true';
}
export function requireSessionEnabled() {
 ...
 */
export async function mintSession(pool, input) {
    ...
    await pool.query(`INSERT INTO app_sessions (token_hash, user_id, email, expires_at, user_agent)
     VALUES ($1, $2, LOWER($3), $4, $5)`, [hash(token), input.userId, input.email, expiresAt, (input.userAgent || '').slice(0, 400) || null]);
...
export async function resolveSession(pool, token) {
    ...
    let session = null;
    try {
        const r = await pool.query(`SELECT s.user_id, s.email, s.expires_at
         FROM app_sessions s
...
function touch(pool, token) {
    const h = hash(token);
    const now = Date.now();
    if ((lastTouched.get(h) ?? 0) > now - 60_000)
        return;
