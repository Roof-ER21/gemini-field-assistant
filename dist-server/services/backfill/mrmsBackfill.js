...
function coarseStateFromLatLng(lat, lng) {
    // DC is smallest — check first
    if (lat >= 38.79 && lat <= 38.99 && lng >= -77.12 && lng <= -76.91)
        return 'DC';
...
export const mrmsBackfill = {
    ...
    async run({ pool, verifiedSvc, states, fromDate, toDate, dryRun, onProgress }) {
        ...
        const yearMonthsToProcess = new Set();
         ...
         LIMIT 1`, [label]);
            ...
            try {
                for (const date of dateRange(clippedStart, clippedEnd)) {
                    ...
                    const rowEnd = Math.min(composite.height - 1, Math.ceil((composite.north - DEFAULT_BBOX.south) / latStep));
                    const colStart = Math.max(0, Math.floor((DEFAULT_BBOX.west - composite.west) / lonStep));
                    const colEnd = Math.min(composite.width - 1, Math.ceil((DEFAULT_BBOX.east - composite.west) / lonStep));
                    ...
                    if (batch.length > 0) {
                        ...
                        const { rows: prevRows } = await pool.query(`SELECT latitude, longitude, hail_size_inches
                 FROM verified_hail_events
                WHERE event_date = $1 AND source_mrms = true
                  AND state = ANY($2::text[])`, [prevDate, [...allowedStates]]);
