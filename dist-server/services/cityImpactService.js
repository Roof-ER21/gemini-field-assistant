export async function getCityImpactOnDate(pool, lat, lng, city, state, dateIso) {
    ...
    const { rows } = await pool.query(`SELECT
     ...
     ORDER BY miles ASC`, [lat, lng, dateIso]);
    const bandCount = (miles) => rows.filter((r) => Number(r.miles) <= miles).length;
    const totalWithin10mi = rows.length;
    // Closest hail: smallest miles among rows with a hail_size
    let closestHail = null;
    for (const r of rows) {
    ...
    for (const r of rows) {
        ...
        if (r.source_ncei_swdi)
            sourceSet.add('NEXRAD (NCEI SWDI)');
        if (r.source_nexrad_l2)
            sourceSet.add('NEXRAD Level II');
        if (r.source_mrms)
            sourceSet.add('MRMS');
        if (r.source_iem_lsr)
            sourceSet.add('NWS LSR');
 ...
 */
export function renderCityImpactBlock(r) {
    ...
    if (r.biggestHail) {
        lines.push(`  BIGGEST HAIL: ${r.biggestHail.inches.toFixed(2)}" at ${r.biggestHail.miles.toFixed(1)} mi from ${loc} (${r.biggestHail.state || '?'})`);
    }
    if (r.closestHail) {
        lines.push(`  CLOSEST HAIL: ${r.closestHail.inches.toFixed(2)}" at ${r.closestHail.miles.toFixed(1)} mi from ${loc} (${r.closestHail.state || '?'})`);
    }
    if (r.closestWind) {
        lines.push(`  CLOSEST WIND: ${Math.round(r.closestWind.mph)} mph at ${r.closestWind.miles.toFixed(1)} mi from ${loc}`);
    }
