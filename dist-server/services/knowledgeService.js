...
export const knowledgeService = {
    ...
    async getDocumentIndex() {
        ...
        try {
            ...
            const uploadDocs = uploads.map((u) => ({
    ...
    async loadDocument(path) {
        try {
            ...
            if (path.startsWith('local:uploads/')) {
                ...
                const doc = uploads.find((u) => u.id === id || u.path === path);
                if (doc) {
    ...
    async searchDocuments(query, limit = 5, selectedState) {
        ...
        const loadResult = await documentBatchLoader.loadInBatches(topResults, async (result) => {
