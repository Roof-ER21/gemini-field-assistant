 ...
 */
import { SynopticClient, } from './synopticClient.js';
...
export async function corroborateProperty(input) {
    ...
    const corroborated = stations.map((s) => ({
        ...s,
    ...
    return {
        ...
        stationsWithSevereWindSignal: corroborated.filter((s) => s.signal.peakGustMph !== null && s.signal.peakGustMph >= thresholds.severeGustMph).length,
        stations: corroborated,
        fetchedAt: new Date().toISOString(),
    };
}
export function detectHailSignal(obs, th) {
    const reasons = [];
    const hailKeywordHits = new Set();
    const hailCondCodeHits = new Set();
    let peakGust = null;
    let peakPrecip1h = null;
    let peakPrecip15 = null;
    for (const o of obs) {
    ...
    if (peakPrecip1h !== null && peakPrecip1h >= th.heavyPrecipOneHourIn) {
        reasons.push(`heavy 1h precip ${peakPrecip1h.toFixed(2)}" >= ${th.heavyPrecipOneHourIn}"`);
    }
    if (peakPrecip15 !== null && peakPrecip15 >= th.burstPrecipFifteenMinIn) {
        reasons.push(`15-min burst ${peakPrecip15.toFixed(2)}" >= ${th.burstPrecipFifteenMinIn}"`);
    }
    const explicitHail = hailKeywordHits.has('hail') ||
        hailKeywordHits.has('gr') ||
        hailCondCodeHits.size > 0;
    const convective = peakGust !== null &&
...
export async function trySurfaceCorroboration(input) {
    ...
    try {
        ...
        const startUtc = new Date(dateMidnightEt.getTime() - (input.windowHoursBefore ?? 0) * 3600 * 1000);
        const endUtc = new Date(dateMidnightEt.getTime() + (24 + (input.windowHoursAfter ?? 0)) * 3600 * 1000);
        ...
        const ranked = [...result.stations]
            ...
            if (aHasHail !== bHasHail)
                return aHasHail - bHasHail;
        ...
        return {
            available: false,
            reason: err.message || 'unknown synoptic error',
        };
    }
}
