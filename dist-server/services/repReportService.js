...
export class RepReportService {
    ...
    async submit(input) {
        ...
        if (input.submittedByUserId) {
            const rateLimitCheck = await this.pool.query(`SELECT COUNT(*)::int AS c FROM verified_hail_events
         WHERE rep_report_submitted_by_user_id = $1
           AND rep_report_submitted_at >= NOW() - INTERVAL '24 hours'`, [input.submittedByUserId]);
        ...
        if ((input.hailSizeInches == null || input.hailSizeInches <= 0) &&
        ...
        const source = input.isCustomerReport ? 'customer_report' : 'rep_report';
     ...
     */
    async approve(eventId, approvedByUserId) {
        await this.pool.query(`UPDATE verified_hail_events
       SET rep_report_verified_by_admin = TRUE,
           rep_report_verified_at = NOW(),
           rep_report_verified_by_user_id = $2,
           last_updated_at = NOW()
       WHERE id = $1
         AND (source_rep_report OR source_customer_report)`, [eventId, approvedByUserId]);
    }
    async reject(eventId, rejectedByUserId, reason) {
        await this.pool.query(`UPDATE verified_hail_events
       SET rep_report_verified_by_admin = FALSE,
           ...
           last_updated_at = NOW()
       WHERE id = $1
         AND (source_rep_report OR source_customer_report)`, [eventId, rejectedByUserId, reason || null]);
    }
    /**
     ...
     */
    async checkAutoApprove(userId) {
        if (!userId)
            return false;
        if (process.env.REP_AUTO_APPROVE_ENABLED !== 'true')
            return false;
        // Check user's approval history
        const history = await this.pool.query(`SELECT
       ...
       FROM verified_hail_events
       WHERE rep_report_submitted_by_user_id = $1`, [userId]);
