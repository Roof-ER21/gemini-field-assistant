...
class CronService {
    ...
    startAll(pool) {
        ...
        // Daily summary: one send per day at 11:00 PM ET (end-of-day totals).
        // Was 4x/day (5am/12pm/7pm/11pm) — intraday sends carried partial counts
        // and quadrupled inbox noise.
        ...
        if (pool) {
            ...
            const fastStormJob = cron.schedule('*/5 * * * *', async () => {
                try {
                    ...
                    console.error('❌ [Fast Storm Scan] Error:', err.message);
                }
            }, { timezone: 'America/New_York' });
            ...
            const pushScanJob = cron.schedule('*/15 * * * *', async () => {
                try {
                    ...
                    console.error('❌ [Push Scan] Error:', err.message);
                }
            }, { timezone: 'America/New_York' });
            ...
            const noaaReconcileJob = cron.schedule('0 6 * * *', async () => {
                ...
                try {
                    ...
                    console.error('❌ [NOAA Phase 3] Error:', err.message);
                }
            }, { timezone: 'America/New_York' });
