...
class CronService {
    ...
    startAll() {
        ...
        const job5am = cron.schedule('0 5 * * *', async () => {
            console.log('⏰ [5:00 AM] Running morning summary emails...');
            try {
                const result = await dailySummaryService.sendAllDailySummaries();
                console.log('✅ [5:00 AM] Morning summaries sent:', result);
            }
            catch (error) {
        ...
        const job12pm = cron.schedule('0 12 * * *', async () => {
            console.log('⏰ [12:00 PM] Running midday summary emails...');
            try {
                const result = await dailySummaryService.sendAllDailySummaries();
                console.log('✅ [12:00 PM] Midday summaries sent:', result);
            }
            catch (error) {
        ...
        const job7pm = cron.schedule('0 19 * * *', async () => {
            console.log('⏰ [7:00 PM] Running evening summary emails...');
            try {
                const result = await dailySummaryService.sendAllDailySummaries();
                console.log('✅ [7:00 PM] Evening summaries sent:', result);
            }
            catch (error) {
        ...
        const job11pm = cron.schedule('0 23 * * *', async () => {
            console.log('⏰ [11:00 PM] Running night summary emails...');
            try {
                const result = await dailySummaryService.sendAllDailySummaries();
                console.log('✅ [11:00 PM] Night summaries sent:', result);
            }
            catch (error) {
