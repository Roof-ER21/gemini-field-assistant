...
export class AgentProactiveService {
    ...
    start() {
        ...
        this.checkDueTasks().catch(err => console.error('[AgentProactive] Initial check error:', err));
        this.intervalId = setInterval(() => {
            this.checkDueTasks().catch(err => console.error('[AgentProactive] Check error:', err));
        }, 5 * 60 * 1000);
    }
    stop() {
        if (this.intervalId) {
    ...
    async checkDueTasks() {
        try {
            ...
            const result = await this.pool.query(`SELECT t.id, t.user_id, t.title, t.description, t.task_type, t.due_at, t.job_number
         FROM agent_tasks t
         ...
         LIMIT 20`);
            ...
            for (const task of result.rows) {
                ...
                await this.pool.query(`INSERT INTO notifications (user_id, type, title, body, data, created_at)
           ...
           ON CONFLICT DO NOTHING`, [
                ...
                await this.pool.query(`UPDATE agent_tasks
