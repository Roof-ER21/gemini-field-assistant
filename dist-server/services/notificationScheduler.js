 ...
 */
export async function scanForNewStorms(pool, pushService) {
    let alertsSent = 0;
    let errors = 0;
    try {
        // Find impact alerts that haven't been pushed yet
        const pendingAlerts = await pool.query(`SELECT ia.id, ia.user_id, ia.alert_type, ia.alert_severity,
       ...
       LIMIT 50`);
        ...
        for (const alert of pendingAlerts.rows) {
            try {
                ...
                const results = await pushService.sendToUser(alert.user_id, {
                ...
                if (anySent) {
                    await pool.query(`UPDATE impact_alerts SET push_sent = TRUE, push_sent_at = NOW() WHERE id = $1`, [alert.id]);
                    alertsSent++;
                }
            }
            catch (err) {
                console.error(`[NotificationScheduler] Error pushing storm alert ${alert.id}:`, err.message);
                errors++;
            }
        }
        // Also check for new storm_events without impact_alerts — send territory-level alerts
        const newStorms = await pool.query(`SELECT se.id, se.city, se.state, se.event_type, se.hail_size_inches,
       ...
       LIMIT 20`);
        for (const storm of newStorms.rows) {
            try {
                ...
                const affectedUsers = await pool.query(`SELECT DISTINCT u.id AS user_id
           FROM territories t
           ...
           WHERE $1::numeric BETWEEN t.south_lat AND t.north_lat
             AND $2::numeric BETWEEN t.west_lng AND t.east_lng
             AND t.archived_at IS NULL
             AND (ta.expires_at IS NULL OR ta.expires_at > NOW())`, [storm.latitude, storm.longitude]);
                ...
                console.error(`[NotificationScheduler] Error pushing territory storm alert:`, err.message);
                errors++;
            }
        }
        if (alertsSent > 0) {
 ...
 * Called every 15 minutes.
 */
export async function sendCalendarReminders(pool, pushService) {
    let remindersSent = 0;
    try {
        // Check local calendar_events table for events starting in the next 30 minutes
        const upcoming = await pool.query(`SELECT ce.id, ce.user_id, ce.summary, ce.start_time, ce.end_time,
       ...
       LIMIT 30`);
        for (const event of upcoming.rows) {
            try {
                ...
                const results = await pushService.sendToUser(event.user_id, {
                ...
                console.error(`[NotificationScheduler] Calendar reminder error:`, err.message);
            }
        }
        if (remindersSent > 0) {
 ...
 * Called every 15 minutes.
 */
export async function sendTaskReminders(pool, pushService) {
    let remindersSent = 0;
    try {
        const dueTasks = await pool.query(`SELECT at.id, at.user_id, at.title, at.description, at.due_at,
       ...
       LIMIT 20`);
        for (const task of dueTasks.rows) {
            try {
                ...
                const results = await pushService.sendToUser(task.user_id, {
                ...
                console.error(`[NotificationScheduler] Task reminder error:`, err.message);
            }
        }
        if (remindersSent > 0) {
 ...
 */
export async function sendMorningBriefing(pool, pushService) {
    let sent = 0;
    try {
        // Get all active users
        const users = await pool.query(`SELECT u.id, u.name, u.email
       FROM users u
        ...
        for (const user of users.rows) {
            try {
                ...
                const [taskCount, unreadCount, todayEvents] = await Promise.all([
                    pool.query(`SELECT COUNT(*)::integer as count FROM agent_tasks
             WHERE user_id = $1 AND status = 'pending' AND due_at::date <= $2::date`, [user.id, today]),
            ...
            FROM conversation_participants cp WHERE cp.user_id = $1`, [user.id]),
                    pool.query(`SELECT COUNT(*)::integer as count FROM calendar_events
             WHERE user_id = $1 AND status = 'active'
               AND start_time::date = $2::date`, [user.id, today])
                ]);
                ...
                const parts = [];
                if (tasks > 0)
                    parts.push(`${tasks} task${tasks > 1 ? 's' : ''} due`);
                if (events > 0)
                    parts.push(`${events} event${events > 1 ? 's' : ''}`);
                if (messages > 0)
                    parts.push(`${messages} unread message${messages > 1 ? 's' : ''}`);
                ...
                console.error(`[NotificationScheduler] Morning briefing error for ${user.id}:`, err.message);
            }
        }
        if (sent > 0) {
 ...
 */
export async function sendEndOfDaySummary(pool, pushService) {
    let sent = 0;
    try {
        const users = await pool.query(`SELECT u.id, u.name
       FROM users u
       WHERE u.is_active = TRUE
         AND NOT EXISTS (
           SELECT 1 FROM push_notification_log pnl
        ...
        for (const user of users.rows) {
            try {
                ...
                const [completed, chatSessions] = await Promise.all([
                    pool.query(`SELECT COUNT(*)::integer as count FROM agent_tasks
             WHERE user_id = $1 AND status = 'done' AND completed_at::date = $2::date`, [user.id, today]),
                    pool.query(`SELECT COUNT(*)::integer as count FROM user_activity_log
             WHERE user_id = $1 AND activity_type = 'chat' AND DATE(created_at) = $2::date`, [user.id, today])
                ]);
                const completedTasks = completed.rows[0]?.count || 0;
                const chats = chatSessions.rows[0]?.count || 0;
                const parts = [];
                if (completedTasks > 0)
                    parts.push(`${completedTasks} task${completedTasks > 1 ? 's' : ''} completed`);
                if (chats > 0)
                    parts.push(`${chats} Susan chat${chats > 1 ? 's' : ''}`);
                const body = parts.length > 0
                ...
                console.error(`[NotificationScheduler] EOD summary error for ${user.id}:`, err.message);
            }
        }
        if (sent > 0) {
