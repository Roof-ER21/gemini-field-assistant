...
function nearestCity(lat, lng) {
    ...
    if (!best || best.dist > CITY_MAX_DIST_MI)
        return null;
    return best;
}
async function fetchLiveMrms() {
    ...
    try {
        ...
        console.warn('[LiveMrmsAlert] fetch err:', e.message);
        return null;
    }
}
...
export async function runLiveMrmsAlertCheck(pool, opts = {}) {
    ...
    for (const c of cells) {
        ...
        if (!existing || c.inches > existing.inches)
            dedupedMap.set(cityKey, c);
    }
    ...
    for (const c of perCity) {
        ...
        const { rows } = await pool.query(`SELECT 1 FROM bot_storm_alerts_sent
         WHERE dedup_key = $1
            OR (lat_bucket = $2 AND lng_bucket = $3 AND max_inches_bucket = $4
                AND posted_at > NOW() - INTERVAL '${DEDUP_WINDOW_HOURS} hours')`, [dedupKey, latBucket.toFixed(2), lngBucket.toFixed(2), inchesBucket.toFixed(2)]);
        if (rows.length > 0) {
            suppressed++;
            continue;
        }
    ...
    for (const c of newCells) {
        ...
        const dedupKey = `${refTime}|${latBucket.toFixed(2)}|${lngBucket.toFixed(2)}|${inchesBucket.toFixed(2)}`;
        try {
            await pool.query(`INSERT INTO bot_storm_alerts_sent
