...
async function cachedListTools(endpoint, token) {
...
export function toGeminiSchema(input) {
    ...
    if (Array.isArray(src.enum)) {
        const values = src.enum.filter((v) => typeof v === 'string');
        // Gemini only understands an enum on a string; on anything else it is noise.
        if (values.length > 0 && type === Type.STRING)
            out.enum = values;
    }
    if (type === Type.ARRAY) {
    ...
    if (type === Type.OBJECT && src.properties && typeof src.properties === 'object') {
        const props = {};
        for (const [name, raw] of Object.entries(src.properties)) {
        ...
        const required = Array.isArray(src.required)
            ? src.required.filter((r) => typeof r === 'string' && r in props)
            : [];
