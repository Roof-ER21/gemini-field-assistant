    ...
    for (const state of SERVICE_STATES) {
        try {
            ...
            for (const feature of features) {
                ...
                if (hailSize < 0.25)
                    continue; // Any documented hail (pea-size or larger)
...
async function fetchSPCForStates(states) {
    ...
    for (const { url, dateStr } of csvSources) {
        const type = url.includes('hail') ? 'hail' : 'wind';
        try {
            const response = await fetch(url);
            if (!response.ok)
                continue;
 ...
 */
export async function reconcileWithNOAA(pool, pushService) {
    let reconciled = 0;
    let notified = 0;
    let errors = 0;
    try {
        // Get unreconciled alerts older than 7 days, up to 90 days back
        const pending = await pool.query(`SELECT * FROM storm_alerts
       ...
       ORDER BY event_date DESC`);
        ...
        for (const alert of pending.rows) {
            try {
                ...
                const matches = noaaEvents.filter(e => {
                    if (e.eventType !== alert.event_type)
                        return false;
                    if (!e.id.startsWith('noaa-'))
                        return false;
                ...
                const best = matches.sort((a, b) => (a.distanceMiles || 999) - (b.distanceMiles || 999))[0];
                await pool.query(`UPDATE storm_alerts
           SET noaa_reconciled = TRUE,
               noaa_magnitude = $1,
               noaa_event_id = $2,
               noaa_narrative = $3,
               alert_phase = 'noaa_reconciled'
           WHERE id = $4`, [best.magnitude, best.id, best.narrative, alert.id]);
                reconciled++;
        ...
        if (reconciled > 0) {
            try {
                const newlyReconciled = await pool.query(`SELECT * FROM storm_alerts
           WHERE noaa_reconciled = TRUE AND noaa_update_sent = FALSE`);
                if (newlyReconciled.rows.length > 0) {
                    ...
                    for (const alert of newlyReconciled.rows) {
                        if (!groups.has(alert.state))
                            groups.set(alert.state, []);
                 ...
                 ON CONFLICT DO NOTHING`, [
                                ...
                                body,
                                JSON.stringify({ type: 'noaa_reconciliation', state, alertIds: alerts.map((a) => a.spc_event_id) })
                            ]).catch(() => { });
                        }
                        // Mark all as notified
                        for (const alert of alerts) {
                            await pool.query(`UPDATE storm_alerts SET noaa_update_sent = TRUE, noaa_update_sent_at = NOW() WHERE id = $1`, [alert.id]);
                        }
                        notified++;
