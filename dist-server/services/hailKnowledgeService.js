...
export class HailKnowledgeService {
    ...
    async indexStormLookup(lookup) {
        ...
        const result = await this.pool.query(`INSERT INTO hail_knowledge (
    ...
    async getContextForChat(params) {
        ...
        for (const doc of documents) {
            ...
            if (doc.state)
                contextParts.push(`State: ${doc.state}`);
     ...
     */
    generateKnowledgeContent(lookup) {
        const parts = [];
        // Location summary
        ...
        // Data sources
        const sources = [];
        if (lookup.dataSources.noaa)
            sources.push('NOAA');
        if (lookup.dataSources.ihm)
            sources.push('Interactive Hail Maps');
     ...
     */
    async deleteByStormLookupId(lookupId) {
        const result = await this.pool.query('DELETE FROM hail_knowledge WHERE storm_lookup_id = $1', [lookupId]);
     ...
     */
    async getById(id) {
        const result = await this.pool.query('SELECT * FROM hail_knowledge WHERE id = $1', [id]);
 ...
 */
export const createHailKnowledgeService = (pool) => {
