export async function materializeIhmDates(pool) {
    ...
    let rowsMerged = 0;
    let skippedNoHail = 0;
    const { rows: cities } = await pool.query(`SELECT city, state, lat, lng, unique_dates, fetched_at
    ...
    for (const c of cities) {
        ...
        for (const d of c.unique_dates || []) {
            ...
            const result = await pool.query(`INSERT INTO verified_hail_events
