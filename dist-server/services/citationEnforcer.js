 ...
 */
export function getExpectedCitationCount(sources) {
 ...
 */
export function enforceCitations(text, sources) {
 ...
 */
function extractKeywords(docName, category) {
    const keywords = [];
...
export function validateCitations(text, sources) {
    ...
    if (citations.length < expectedMinCitations) {
        issues.push(`Low citation density: ${citations.length} citations for ${sentences.length} sentences ` +
