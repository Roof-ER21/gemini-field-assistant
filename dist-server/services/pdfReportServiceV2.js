...
export class PDFReportServiceV2 {
    ...
    generateReport(input) {
        ...
        doc.fontSize(8.5).fillColor(this.C.text).font('Helvetica')
            .text('This report is compiled from the following federally maintained data sources:', this.M + 10, doc.y, { width: this.CW - 20 });
        doc.moveDown(0.4);
        ...
        doc.fontSize(8).fillColor(this.C.sourceGreen).font('Helvetica-Oblique')
            .text('These are the same federally maintained data sources referenced by Certified Consulting Meteorologists (CCM) ' +
            'in professional storm damage assessments and insurance documentation.', this.M + 10, doc.y, { width: this.CW - 20, lineGap: 1 });
        // =========================================================
        ...
        doc.fontSize(8).fillColor(this.C.lightText).font('Helvetica')
            .text(`Hail observations verified through federal weather monitoring systems near ${input.address}`, this.M, doc.y, { width: this.CW });
        doc.moveDown(0.4);
        ...
        if (windObsNoaa.length > 0) {
            ...
            doc.fontSize(8).fillColor(this.C.lightText).font('Helvetica')
                .text(`Damaging wind observations verified through federal weather monitoring systems near ${input.address}`, this.M, doc.y, { width: this.CW });
            doc.moveDown(0.4);
        ...
        if (hasWarnings || hasLegacyNexrad) {
            ...
            if (warningCount > 0) {
                doc.fontSize(9).fillColor(this.C.text).font('Helvetica')
                    .text(`The following severe weather warnings were issued by the National Weather Service (NWS) for the area encompassing ${input.address}:`, this.M + 20, doc.y, { width: this.CW - 40, lineGap: 2 });
                doc.moveDown(0.8);
            }
            if (hasAlertImages) {
                alertImages.slice(0, 5).forEach((item, idx) => {
                    ...
                    doc.fontSize(9.5).fillColor(headlineColor).font('Helvetica-Bold')
                        .text(`${alert.event} issued ${this.fmtDateET(alert.onset)} at ${this.fmtTimeET(alert.onset)}`, detailX, blockY, { width: detailW });
                    ...
                    const drawDetail = (label, value, col, row) => {
                ...
                const labelW2 = 70;
                const valW = (detailW - labelW2 * 2) / 2;
                const drawLegacyDetail = (label, value, col, row) => {
                    const x = detailX + col * (labelW2 + valW);
                ...
                try {
                    ...
                    doc.y += radarH + 4;
                    doc.fontSize(7).fillColor(this.C.mutedText).font('Helvetica')
                        .text(`NEXRAD WSR-88D Radar | ${this.fmtFullDateTimeET(input.nexradTimestamp || primaryStormDate)} | Source: Iowa Environmental Mesonet (IEM/NOAA)`, this.M, doc.y, { width: this.CW, align: 'center' });
                }
                catch (e) {
        ...
        doc.fontSize(8).fillColor(this.C.lightText).font('Helvetica')
            .text('This Storm Impact Analysis is generated using publicly available data from the National Oceanic and Atmospheric ' +
            ...
            'no independent representations regarding the accuracy of the underlying federal data.', this.M + 20, doc.y, { width: this.CW - 40, lineGap: 1.5, align: 'justify' });
        // =========================================================
        ...
        doc.fontSize(8).fillColor('#ffffff').font('Helvetica')
            .text(`Prepared by Roof-ER  |  Data sourced from NOAA, NWS, and NEXRAD federal weather systems  |  \u00A9 ${year}`, this.M, copyY + 10, { width: this.CW, align: 'center' });
        // =========================================================
        // FINALIZE
        // =========================================================
        doc.end();
