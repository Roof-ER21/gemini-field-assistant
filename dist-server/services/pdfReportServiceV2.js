...
export class PDFReportServiceV2 {
    ...
    generateReport(input) {
        ...
        for (const e of input.events || [])
            mergedHistoryMap.set(e.id, e);
        for (const e of input.historyEvents || [])
            mergedHistoryMap.set(e.id, e);
        const allHistoryEvents = Array.from(mergedHistoryMap.values());
        ...
        if (input.showRoofAge && input.propertyRisk?.estimatedRoofAge) {
            doc.moveDown(0.3);
        ...
        const hitLabel = (g) => {
            if (swathDirectHitSet.has(g.date))
                return 'DIRECT HIT';
            if (g.atLoc >= 0.5)
                return 'DIRECT HIT';
            if (g.atLoc > 0)
        ...
        const isDirectByAtLoc = (g) => g.atLoc > 0 || swathDirectHitSet.has(g.date);
        const isActionable = (g) => g.atLoc >= 0.5 || swathDirectHitSet.has(g.date);
