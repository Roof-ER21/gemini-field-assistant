...
export async function ingestArtifact(pool, artifact) {
    return ingestArtifactInternal(pool, artifact, null);
}
export async function ingestLatestCrawl(pool) {
    const latest = await findLatestArtifact();
    if (!latest)
        return { ok: false, inserted: 0, updated: 0, skipped: 0, artifact: null };
    const raw = await fs.readFile(latest, 'utf8');
    const artifact = JSON.parse(raw);
    return ingestArtifactInternal(pool, artifact, latest);
}
async function ingestArtifactInternal(pool, artifact, artifactPath) {
    ...
    for (const c of artifact.cities) {
        ...
        const result = await pool.query(`INSERT INTO ihm_city_mirror
 ...
 */
export async function diffCityAgainstOurs(pool, city, state, radiusMiles = 10) {
    // Load IHM mirror
    const ihm = await pool.query(`SELECT city, state, lat, lng, unique_dates, fetched_at
    ...
    const ours = await pool.query(`SELECT event_date::text AS event_date,
      ...
      GROUP BY event_date
      ORDER BY event_date DESC`, [lat, lng, radiusMiles]);
    const oursByDate = new Map();
