 ...
 */
export async function summarizeAndSaveSession(pool, userId, sessionId, messages) {
...
Respond with ONLY valid JSON (no markdown, no text before/after):
{
  ...
  "user_sentiment": "positive | neutral | frustrated | confused"
    ...
    try {
        ...
        // Save to DB
        await pool.query(`INSERT INTO conversation_summaries
       ...
       ON CONFLICT (user_id, session_id) DO UPDATE SET
        ...
        console.error('[ConversationIntel] Summary generation failed:', err.message);
        return null;
    }
}
/**
 ...
 */
export async function loadRecentSummaries(pool, userId, limit = 3) {
    try {
        const { rows } = await pool.query(`SELECT summary, key_facts, action_items, topics, conversation_end
       ...
       LIMIT $2`, [userId, limit]);
        ...
        console.error('[ConversationIntel] Failed to load summaries:', err.message);
        return '';
    }
}
/**
 ...
 */
export async function createLearningFromFeedback(pool, feedbackId, userQuery, susanResponse, feedbackComment, tags, state, insurer) {
    try {
        ...
        await pool.query(`INSERT INTO global_learnings
       ...
       ON CONFLICT (normalized_key, scope_key) DO UPDATE SET
         ...
         END`, [normalizedKey, scopeKey, state || null, insurer || null, content]);
        // Link feedback to learning
        const learningRow = await pool.query(`SELECT id FROM global_learnings WHERE normalized_key = $1 AND scope_key = $2`, [normalizedKey, scopeKey]);
        if (learningRow.rows.length > 0) {
            await pool.query(`INSERT INTO global_learning_sources (global_learning_id, feedback_id)
         VALUES ($1, $2::uuid) ON CONFLICT DO NOTHING`, [learningRow.rows[0].id, feedbackId]).catch(() => { }); // Ignore if feedback_id doesn't match UUID format
        }
        console.log(`[ConversationIntel] Created learning candidate from negative feedback: "${normalizedKey.slice(0, 50)}..."`);
    }
    catch (err) {
        console.error('[ConversationIntel] Failed to create learning from feedback:', err.message);
    }
}
