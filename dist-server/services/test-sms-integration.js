...
async function testSMSIntegration() {
    ...
    try {
        ...
        let userResult = await pool.query('SELECT id FROM users WHERE LOWER(email) = LOWER($1)', [testEmail]);
        let userId;
        if (userResult.rows.length === 0) {
            // Create test user
            const newUserResult = await pool.query(`INSERT INTO users (name, email, role, phone_number, sms_alerts_enabled)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING id`, ['Test User', testEmail, 'user', testPhone, true]);
         ...
         WHERE id = $3`, [testPhone, true, userId]);
        ...
        await pool.query(`DELETE FROM customer_properties
       WHERE user_id = $1 AND customer_name = 'SMS Test Property'`, [userId]);
        ...
        const alertCheck = await pool.query(`SELECT sms_sent, sms_sent_at, sms_message_sid
       FROM impact_alerts
       WHERE id = $1`, [alert.id]);
        ...
        const smsLog = await pool.query(`SELECT *
       ...
       LIMIT 1`, [alert.id]);
        ...
        if (alerts2.length > 0) {
            await new Promise(resolve => setTimeout(resolve, 2000));
            const alert2Check = await pool.query(`SELECT sms_sent
         FROM impact_alerts
         WHERE id = $1`, [alerts2[0].id]);
