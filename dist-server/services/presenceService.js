...
export class PresenceService {
    ...
    offlineThreshold = 45000; // 45 seconds without heartbeat = offline
    heartbeatTimers = new Map();
    sessionStartTimes = new Map();
    userSockets = new Map(); // userId -> Set of socketIds
    constructor(httpServer, pool, allowedOrigins) {
        this.pool = pool;
    ...
    async setUserOnline(userId, socketId, deviceType) {
        const sessionStart = new Date();
        this.sessionStartTimes.set(userId, sessionStart);
        try {
            await this.pool.query(`INSERT INTO user_presence (user_id, status, last_seen, socket_id, device_type)
         ...
         DO UPDATE SET
           ...
           last_seen = NOW(),
           socket_id = $2,
           device_type = $3`, [userId, socketId, deviceType]);
        }
        catch (error) {
    ...
    async updateUserStatus(userId, status) {
        try {
            await this.pool.query(`UPDATE user_presence
    ...
    async updateLastSeen(userId) {
        try {
            await this.pool.query(`UPDATE user_presence SET last_seen = NOW() WHERE user_id = $1`, [userId]);
        }
        catch (error) {
            console.error('[Presence] Error updating last seen:', error);
        }
    }
    startHeartbeatMonitoring(userId, socketId) {
    ...
    resetHeartbeatTimer(userId, socketId) {
        this.clearHeartbeatTimer(userId);
    ...
    async handleDisconnect(userId, socketId) {
        ...
        try {
         ...
         WHERE user_id = $1`, [userId]);
            ...
            await this.pool.query(`INSERT INTO presence_history (user_id, event_type, session_duration_seconds, device_type)
         SELECT user_id, 'offline', $2, device_type
         FROM user_presence WHERE user_id = $1`, [userId, sessionDuration]);
        }
        catch (error) {
    ...
    emitReadReceipt(conversationId, userId, messageIds) {
    ...
    // Emit pin updates
    emitPinUpdate(conversationId, messageId) {
    ...
    emitPollVoteUpdate(conversationId, messageId, votes) {
    ...
    emitEventRsvpUpdate(conversationId, messageId, rsvps) {
    ...
    startCleanupJob() {
        setInterval(async () => {
            try {
                ...
                const result = await this.pool.query(`UPDATE user_presence
           SET status = 'offline', socket_id = NULL
           WHERE status != 'offline'
           AND last_seen < $1
           RETURNING user_id`, [staleThreshold]);
...
export function initializePresenceService(httpServer, pool, allowedOrigins) {
    if (!presenceService) {
