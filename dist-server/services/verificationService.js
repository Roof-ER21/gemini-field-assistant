 ...
 */
import { computeConsensusSize, isSterlingClassStorm, } from './displayCapService.js';
 ...
 */
export async function buildVerification(pool, opts) {
    const map = await buildVerificationBulk(pool, {
        lat: opts.lat,
        lng: opts.lng,
        dates: [opts.date],
    });
    return (map.get(opts.date) ?? {
        isVerified: false,
        isAtLocation: false,
 ...
 */
export async function buildVerificationBulk(pool, opts) {
    const out = new Map();
    ...
    `;
        ...
        for (const r of countRes.rows) {
            const dateStr = r.event_date instanceof Date
            ...
            const consensusSize = computeConsensusSize(reports.map((rr) => ({ source: rr.source, sizeInches: rr.size_inches })));
            out.set(dateStr, {
        ...
        console.warn('[verificationService] bulk query failed:', err.message);
    }
    return out;
}
