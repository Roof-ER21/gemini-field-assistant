 ...
 */
export function samplePixelInchesAtLatLng(grid, lat, lng) {
    if (!Number.isFinite(lat) || !Number.isFinite(lng))
        return null;
    if (lat < grid.south || lat > grid.north)
        return null;
    if (lng < grid.west || lng > grid.east)
        return null;
    ...
    if (inches < 0.25)
        return null; // sub-trace radar noise
 ...
 */
export function samplePixelInchesNeighborhood(grid, lat, lng, cellRadius = 1) {
    if (!Number.isFinite(lat) || !Number.isFinite(lng))
        return null;
    if (lat < grid.south || lat > grid.north)
        return null;
    ...
    for (let dr = -cellRadius; dr <= cellRadius; dr += 1) {
        for (let dc = -cellRadius; dc <= cellRadius; dc += 1) {
            ...
            if (r < 0 || r >= grid.height)
                continue;
            if (c < 0 || c >= grid.width)
                continue;
            const mm = grid.mmGrid[r * grid.width + c];
            if (Number.isFinite(mm) && mm > maxMm)
                maxMm = mm;
        }
    }
    if (maxMm <= 0)
