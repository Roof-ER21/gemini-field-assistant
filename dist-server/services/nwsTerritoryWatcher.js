...
export async function watchTerritoriesForStorms(pool) {
    ...
    try {
       ...
       LIMIT 50`);
        ...
        try {
            await pool.query(`INSERT INTO storm_monitoring_runs (run_type, properties_checked, alerts_generated, errors)
         VALUES ('scheduled', $1, $2, $3)`, [territoriesChecked, eventsCreated, errors]);
        }
        catch { /* logging is non-critical */ }
        // If new events were created, check customer_properties for impact alerts
        if (eventsCreated > 0) {
            try {
                const impactResult = await pool.query(`SELECT cp.id as property_id, cp.customer_name, cp.address, cp.latitude, cp.longitude,
           ...
           WHERE cp.is_active = true
             ...
             AND (
               ...
               ON CONFLICT DO NOTHING`, [
                ...
                console.error('[NWSWatcher] Impact check failed:', impactErr.message);
            }
        }
