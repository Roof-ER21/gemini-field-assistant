 ...
 */
import { trySurfaceCorroboration } from './synopticObservationsService.js';
...
export async function buildConsilience(pool, input) {
    ...
    const surfaceAvailable = surfaceResult.status === 'fulfilled' && surfaceResult.value.available;
    let surfaceSkipReason = null;
    if (surfaceResult.status === 'fulfilled') {
        ...
        surfaceSkipReason = surfaceResult.reason?.message || 'rejected';
    }
    const surface = buildSurfaceFinding(surfaceResult.status === 'fulfilled' && surfaceResult.value.available
        ...
        : 0, surfaceResult.status === 'fulfilled' && surfaceResult.value.available
    ...
    if (mrms.present)
        sourcesAgreeing.push(1);
    if (nexrad.present)
        sourcesAgreeing.push(2);
    if (nws.present)
        sourcesAgreeing.push(3);
    if (ground.present)
        sourcesAgreeing.push(4);
    if (mping.present)
        sourcesAgreeing.push(5);
    if (surface.present)
        sourcesAgreeing.push(6);
    return {
        property: { lat, lng },
        dateIso,
...
async function queryDatabase(pool, lat, lng, dateIso) {
    const [pointResult, swathResult] = await Promise.all([
       ...
       LIMIT 5`, [dateIso, lat, lng]),
    ]);
    const pointReports = pointResult.rows.map((r) => ({
    ...
    const swathMax = swathResult.rows.reduce((m, r) => r.max_mesh_inches !== null && Number(r.max_mesh_inches) > m
...
// ──────────────────────────────────────────────────────────────────────────
function buildMrmsFinding(db, lat, lng, dateIso) {
...
function buildNexradFinding(db, _dateIso) {
    ...
    for (const r of nexradRows) {
        ...
        if (typeof rs === 'string')
            radarSites.add(rs);
        if (r.hail_size_inches !== null)
            maxHail = Math.max(maxHail, r.hail_size_inches);
    }
    const headline = present
...
function buildNwsFinding(db, dateIso) {
    const nwsRows = db.pointReports.filter((r) => r.sources.nws_alert || r.sources.iem_vtec);
...
function buildGroundFinding(db, dateIso) {
    const groundRows = db.pointReports.filter((r) => r.sources.noaa_ncei ||
        r.sources.ncei_swdi ||
        r.sources.iem_lsr ||
        r.sources.spc_wcm ||
        r.sources.cocorahs);
    ...
    let maxHailObserved = null;
    let nearestMi = Number.POSITIVE_INFINITY;
    for (const r of groundRows) {
        if (r.sources.noaa_ncei)
            sourcesPresent.add('noaa_ncei');
        if (r.sources.ncei_swdi)
            sourcesPresent.add('ncei_swdi');
        if (r.sources.iem_lsr)
            sourcesPresent.add('iem_lsr');
        if (r.sources.spc_wcm)
            sourcesPresent.add('spc_wcm');
        if (r.sources.cocorahs)
            sourcesPresent.add('cocorahs');
...
function buildMpingFinding(db, _dateIso) {
...
function buildSurfaceFinding(topStations, total, withHail, withSevereWind, lat, lng, dateIso) {
