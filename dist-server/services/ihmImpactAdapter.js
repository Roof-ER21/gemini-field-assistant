...
function ihmAuthHeader() {
    const raw = (process.env.IHM_BASIC_AUTH || '').trim();
    if (raw) {
        if (raw.startsWith('Basic '))
            return raw; // already a header value
        if (raw.includes(':'))
            return 'Basic ' + Buffer.from(raw).toString('base64'); // user:pass
...
export async function ihmImpactDatesForLatLong(lat, lng, months = 24) {
    if (!ihmConfigured())
        throw new Error('IHM not configured — set IHM_API_USER/IHM_API_PASS (or IHM_BASIC_AUTH) in Railway');
 ...
 */
export async function getAddressHailImpactViaIHM(_lat, _lng, _monthsBack = 24) {
