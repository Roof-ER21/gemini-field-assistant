...
function documentToSearchableItem(doc) {
...
async function initializeSemanticIndex(documents) {
...
export const knowledgeService = {
    ...
    async searchDocuments(query, documents) {
        ...
        return results.map(result => {
            const doc = result.item.metadata;
    ...
    async findSimilarDocuments(document, allDocuments) {
        ...
        const results = semanticSearch.findSimilar(document.path, {
            minScore: 0.15,
            maxResults: 5
        });
        return results.map(result => ({
    ...
    async getCategories() {
        const allDocs = await this.getDocumentIndex();
        const categories = new Set(allDocs.map(doc => doc.category).filter(Boolean));
