...
export function encryptionKey() {
    ...
    console.error(`[roofhr-connection] ${ENC_KEY_ENV} is set but is not a 32-byte key ` +
...
function aad(app, userId) {
    return Buffer.from(`${app}:${userId}`, 'utf8');
}
function encrypt(plaintext, key, app, userId) {
...
function decrypt(parts, key, app, userId) {
    try {
        ...
        console.error('[roofhr-connection] could not decrypt a stored token:', err.message);
        return null;
    }
}
/** Idempotent; called from runStartupMigrations on every boot. */
export async function ensureAgentConnectionsTable(pool) {
    await pool.query(`
 ...
 * old grant.
 */
export async function saveConnection(pool, input) {
    ...
    try {
        await pool.query(`INSERT INTO agent_connections
       ...
       ON CONFLICT (user_id, app) DO UPDATE SET
         ...
         created_at       = NOW(),
         last_used_at     = NULL`, [input.userId, app, input.remoteUserId, ciphertext, iv, tag, input.scopes, input.endpoint, input.expiresAt]);
        return { ok: true };
    }
    catch (err) {
        console.error('[roofhr-connection] save failed:', err.message);
 ...
 */
export async function getConnection(pool, userId, app = ROOFHR_APP) {
    const key = encryptionKey();
    if (!key)
        return null;
    try {
        const r = await pool.query(`SELECT remote_user_id, token_ciphertext, token_iv, token_tag, scopes, endpoint,
        ...
        if (r.rows.length === 0)
            return null;
        const row = r.rows[0];
        const token = decrypt({ ciphertext: row.token_ciphertext, iv: row.token_iv, tag: row.token_tag }, key, app, userId);
        if (!token)
            return null;
        return {
            userId,
        ...
        console.error('[roofhr-connection] lookup failed:', err.message);
        return null;
    }
}
/** What the rep is told about their own connection. Never includes the token. */
export async function connectionSummary(pool, userId, app = ROOFHR_APP) {
...
export async function deleteConnection(pool, userId, app = ROOFHR_APP) {
    try {
        const r = await pool.query('DELETE FROM agent_connections WHERE user_id = $1 AND app = $2', [userId, app]);
        return (r.rowCount ?? 0) > 0;
    }
    catch (err) {
        console.error('[roofhr-connection] delete failed:', err.message);
        return false;
    }
}
/** Best effort, off the request path. */
export function touchConnection(pool, userId, app = ROOFHR_APP) {
    pool
