...
export function createSheetsService(pool) {
    ...
    function toDisplayName(nameLower) {
        return nameLower
            .split(' ')
            .filter(Boolean)
    ...
    function findHeaderRow(rows) {
        for (let i = 0; i < rows.length; i += 1) {
            ...
            const hasName = normalized.some(cell => cell === 'repname' ||
                cell === 'name' ||
    ...
    function pickSheetTitle(sheetInfo, candidates) {
        const titles = sheetInfo.data.sheets?.map(sheet => sheet.properties?.title).filter(Boolean);
    ...
    function parseSignupsRows(rows) {
        if (rows.length === 0)
            return new Map();
    ...
    async function fetchSignupsForYear(year, allowGeneric) {
        ...
        let sheetTitle = pickSheetTitle(sheetInfo, candidates);
        if (!sheetTitle) {
            sheetTitle =
    ...
    async function fetchAllTimeRevenue() {
        const sheets = await connect();
        let response;
        try {
            ...
            const sheetInfo = await sheets.spreadsheets.get({ spreadsheetId });
            const fallbackSheet = sheetInfo.data.sheets?.find(sheet => sheet.properties?.title?.toLowerCase().includes('all time'));
    ...
    async function fetchYearlyRevenue(year) {
        ...
        try {
            ...
            console.warn(`[SHEETS] Error finding ${year} revenue sheet:`, error.message);
            return new Map();
        }
        if (!response) {
            return new Map();
    ...
    function parseEstimateRows(rows) {
        if (rows.length === 0)
            return new Map();
        const headerIndex = findHeaderRow(rows);
        const headerRow = rows[headerIndex] || [];
    ...
    async function fetchEstimatesForYear(year) {
        const sheets = await connect();
        const sheetInfo = await sheets.spreadsheets.get({ spreadsheetId });
        const candidates = [
    ...
    async function upsertMonthlyMetrics(rows) {
        if (rows.length === 0)
            return;
    ...
    async function performFullSync() {
        ...
        try {
            ...
            try {
                const logResult = await pool.query(`INSERT INTO sheets_sync_log (sync_type, started_at)
           VALUES ($1, $2)
           RETURNING id`, ['full', startedAt]);
                syncLogId = logResult.rows[0]?.id ?? null;
            }
            catch (logError) {
                console.warn('[SHEETS] Unable to record sync start:', logError.message);
            }
            currentStep = 'fetch_data';
            const currentYear = new Date().getFullYear();
            const previousYear = currentYear - 1;
            const [currentSignups, previousSignups, currentEstimates, previousEstimates, allTimeRevenue, revenue2025, revenue2026] = await Promise.all([
            ...
            const signupsByYear = new Map([
                [currentYear, currentSignups],
                [previousYear, previousSignups]
            ]);
            const estimatesByYear = new Map([
            ...
            const existingRepsResult = await pool.query(`SELECT id, name, email, monthly_signup_goal
         FROM sales_reps`);
            ...
            for (const [nameLower, data] of combinedData.entries()) {
             ...
             SET name = $1,
                 email = $2,
                 monthly_signups = $3::numeric,
                 yearly_signups = $4::numeric,
                 monthly_revenue = COALESCE($5::numeric, monthly_revenue),
                 yearly_revenue = $6::numeric,
                 revenue_2025 = $7::numeric,
                 revenue_2026 = $8::numeric,
                 all_time_revenue = CASE WHEN $9::numeric > 0 THEN $9::numeric ELSE all_time_revenue END,
                 goal_progress = $10::numeric,
                 current_bonus_tier = $11::integer,
                 is_active = true,
                 updated_at = NOW()
             WHERE id = $12::integer`, updateValues);
                        updated += 1;
              ...
              $13::numeric, $14::integer, $15::integer,
              $16::integer, $17
            )
            ...
            try {
                ...
                if (revenue2025.size > 0)
                    yearsToSync.add(2025);
                if (revenue2026.size > 0)
                    yearsToSync.add(2026);
                const monthlyRows = [];
                const yearlyRows = [];
            ...
            for (const rep of existingRepsResult.rows) {
                ...
                if (nameLower && sheetNames.has(nameLower))
                    continue;
           ...
           SET records_synced = $1,
               ...
               records_deleted = $4,
               completed_at = NOW()
           WHERE id = $5`, [synced, created, updated, deactivated, syncLogId]);
            }
            return {
                success: true,
                total,
            ...
            if (syncLogId) {
             ...
             SET error_message = $1,
                 completed_at = NOW()
             WHERE id = $2`, [debugInfo.substring(0, 1000), syncLogId]);
                }
                catch (logError) {
