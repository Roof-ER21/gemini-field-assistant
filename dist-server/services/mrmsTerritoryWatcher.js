...
export async function watchTerritoriesForMrmsHail(pool, 
// Keep injected so tests can stub without loading real MRMS service
getRecentMrmsHailAtPoint) {
    ...
    if (!getRecentMrmsHailAtPoint) {
        ...
        getRecentMrmsHailAtPoint = mod.getRecentMrmsHailAtPoint;
    }
    if (!getRecentMrmsHailAtPoint) {
    ...
    `);
        ...
        for (const p of props.rows) {
            ...
            const existing = uniquePoints.find((u) => Math.abs(u.lat - lat) < PROXIMITY_BUCKET_DEG &&
        ...
        for (const point of uniquePoints) {
            try {
                ...
                if (!mrms || !mrms.maxHailSize || mrms.maxHailSize < MIN_ALERT_HAIL_INCHES)
                    continue;
                ...
                if (detectionAgeMs > LOOKBACK_HOURS * 60 * 60 * 1000)
                    continue;
                ...
                for (const property of point.properties) {
                    try {
                        await pool.query(`INSERT INTO impact_alerts (
              ...
              ON CONFLICT (customer_property_id, storm_event_id) DO NOTHING`, [
                        ...
                        await pool.query(`UPDATE customer_properties SET last_alert_at = NOW() WHERE id = $1`, [property.id]).catch(() => { });
                        notificationsSent++;
                    }
                    catch (e) {
                        errors++;
