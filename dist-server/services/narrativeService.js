 ...
 */
function formatHailSizeInches(inches) {
    if (inches < 0.25)
        return null;
    return `${inches.toFixed(2)}"`;
}
function getHailDamagePotential(inches) {
    if (inches >= 2.0)
 ...
 */
function getWindDamagePotential(mph) {
    if (mph >= 90)
        return 'severe structural damage including roof decking failure, lifted or torn shingles, siding separation, and downed trees';
    if (mph >= 75)
        return 'significant damage to roofing materials including shingle tear-off, flashing displacement, ridge vent separation, and siding damage';
    if (mph >= 60)
        return 'notable damage to roof surfaces — especially aged or compromised shingles — along with gutter detachment, fascia damage, and fence/outbuilding impacts';
    if (mph >= 50)
...
export function generateHailNarrative(params) {
    const { address, city, state, stormDate, maxHailSize, totalEvents, severeCount, windEvents = 0, maxWindMph = 0, nearbyReports, stormDirection, stormSpeed, radiusMiles } = params;
    ...
    if (hasMeaningfulWind && hasMeaningfulHail) {
        ...
        if (maxWindMph >= 50)
            narrative += ` with peak gusts of ${Math.round(maxWindMph)} mph`;
        narrative += `. `;
    }
...
export function generateExecutiveSummary(params) {
    ...
    const bits = [];
    if (hailDesc && hailSizeText)
        bits.push(`hail measuring up to ${hailSizeText} (${hailDesc})`);
    if (windEvents > 0 && maxWindMph >= 50)
        bits.push(`peak wind gusts up to ${Math.round(maxWindMph)} mph`);
    else if (windEvents > 0)
        bits.push(`${windEvents} damaging wind event${windEvents !== 1 ? 's' : ''}`);
    if (bits.length > 0)
        summary += `, with ${bits.join(' and ')}`;
    summary += `. `;
    if (severeCount > 0) {
