...
export class CanvassingService {
    ...
    async markAddress(params) {
        const { address, status, userId, streetAddress, city, state, zipCode, latitude, longitude, contactMethod, homeownerName, phoneNumber, email, notes, followUpDate, followUpNotes, relatedStormEventId, territory, sessionId, 
        // Neighborhood Intel fields
        homeownerPhone, homeownerEmail, propertyNotes, bestContactTime, propertyType, roofType, roofAgeYears, autoMonitor, linkedPropertyId } = params;
        // Upsert the canvassing status
        const result = await this.pool.query(`INSERT INTO canvassing_status (
    ...
    async getAddressStatus(address) {
        const result = await this.pool.query('SELECT * FROM canvassing_status WHERE address = $1', [address]);
     ...
     */
    async getNearbyCanvassing(latitude, longitude, radiusMiles = 1) {
      ...
      ORDER BY distance_miles ASC`, [latitude, longitude, radiusMiles]);
        return result.rows.map(this.rowToEntry);
    }
    /**
     * Get addresses needing follow-up
     */
    async getFollowUpList(userId, territory) {
        let query = `
    ...
    async startSession(params) {
        const result = await this.pool.query(`INSERT INTO canvassing_sessions (
    ...
    async endSession(sessionId) {
        const result = await this.pool.query(`UPDATE canvassing_sessions
    ...
    async getActiveSession(userId) {
        const result = await this.pool.query(`SELECT * FROM canvassing_sessions
    ...
    async logActivity(params) {
        try {
            await this.pool.query(`INSERT INTO canvassing_activity_log (
    ...
    async getUserStats(userId, daysBack = 30) {
        const result = await this.pool.query(`SELECT * FROM get_user_canvassing_stats($1, $2)`, [userId, daysBack]);
     ...
     */
    async getTeamStats(daysBack = 30) {
        const result = await this.pool.query(`SELECT
        cs.user_id,
        u.name as user_name,
      ...
      ORDER BY total_leads DESC`, [daysBack]);
        const byUser = result.rows.map(row => ({
            ...
            stats: {
                ...
                totalLeads: parseInt(row.total_leads) || 0,
                totalAppointments: parseInt(row.total_appointments) || 0,
                conversionRate: row.total_doors > 0
        ...
        // Calculate totals
        const total = byUser.reduce((acc, user) => ({
     ...
     */
    async getNeighborhoodIntel(latitude, longitude, radiusMiles = 0.5) {
        const result = await this.pool.query(`SELECT * FROM get_neighborhood_intel($1, $2, $3)`, [latitude, longitude, radiusMiles]);
        return result.rows.map(row => ({
            address: row.address,
            status: row.status,
    ...
    async bulkUpdateStatus(ids, status, userId) {
        const result = await this.pool.query(`UPDATE canvassing_status
      SET status = $1, contacted_by = $2, contact_date = NOW(), updated_at = NOW()
      WHERE id = ANY($3)`, [status, userId, ids]);
 ...
 */
export const createCanvassingService = (pool) => {
