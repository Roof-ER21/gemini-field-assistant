...
// ============================================================
function getRadiusMiles(magnitudeInches) {
...
function getHailColor(magnitudeInches) {
    if (magnitudeInches >= 4.5)
        return '#800080';
    if (magnitudeInches >= 2.5)
        return '#8B0000';
    if (magnitudeInches >= 1.75)
        return '#FF0000';
    if (magnitudeInches >= 1.5)
        return '#FF6600';
    if (magnitudeInches >= 1.0)
        return '#FF9900';
    if (magnitudeInches >= 0.75)
        return '#FFFF00';
    if (magnitudeInches >= 0.25)
        return '#00FF00';
    return '#90EE90';
}
...
function organicRadius(baseRadius, angle, seed) {
...
function toProjected(lat, lng, refLat, refLng) {
...
function clusterEvents(events) {
    ...
    for (let idx = 0; idx < events.length; idx++) {
        ...
        const seed = { ...event, radiusMiles: getRadiusMiles(event.magnitude) };
        const cluster = [seed];
        visited.add(idx);
        let expanded = true;
        while (expanded) {
...
function sampleFootprint(event, refLat, refLng) {
...
// ============================================================
function milesToPixel(point, bounds, imgWidth, imgHeight, refLat, refLng) {
    const milesPerLat = 69;
    const milesPerLng = 69 * Math.cos((refLat * Math.PI) / 180);
...
function buildContourSVG(events, bounds, imgWidth, imgHeight) {
...
/**
 * Render MRMS vector swath polygons onto a base map image.
 *
 * Unlike compositeContourOverlay (which fakes contours from sparse ground
 * reports via convex hulls), this uses the real radar-derived hail footprint
 * from meshVectorService. Produces PDF maps that match IHM/HailTrace quality.
 *
 * Features render low-to-high so heavier hail shows on top. Each polygon in
 * a MultiPolygon is rendered as its own SVG path so disconnected regions do
 * not get reinterpreted as holes of each other.
 */
export async function compositeVectorSwathOverlay(baseMapBuffer, collection, center, zoom, imgWidth = 600, imgHeight = 300) {
    if (!collection.features || collection.features.length === 0) {
        return baseMapBuffer;
    }
    // Map bounds from center + zoom (Web Mercator approximation — matches
    // the compositeContourOverlay math so the overlay lines up with the
    // static map image generated by Google Static Maps).
    ...
    const lngToPx = (lng) => ((lng - bounds.west) / (bounds.east - bounds.west)) * imgWidth;
    const latToPx = (lat) => ((bounds.north - lat) / (bounds.north - bounds.south)) * imgHeight;
    // Sort low → high so heavier hail renders on top.
    const sorted = [...collection.features].sort((a, b) => a.properties.sizeInches - b.properties.sizeInches);
    const paths = [];
    let renderedPolygons = 0;
    for (const feature of sorted) {
        const color = feature.properties.color;
        const opacity = 0.35 + Math.min(0.35, feature.properties.sizeInches * 0.1);
        for (const polygon of feature.geometry.coordinates) {
            // First ring is outer, subsequent are holes — use SVG fill-rule: evenodd
            // to handle holes. Combine all rings into one path with M/Z segments.
            const ringPaths = [];
            for (const ring of polygon) {
                if (ring.length < 3)
                    continue;
                const pts = ring
                    .map(([lng, lat]) => `${lngToPx(lng).toFixed(1)},${latToPx(lat).toFixed(1)}`)
                    .join(' L ');
                ringPaths.push(`M ${pts} Z`);
            }
            if (ringPaths.length === 0)
                continue;
            paths.push(`<path d="${ringPaths.join(' ')}" fill="${color}" fill-opacity="${opacity.toFixed(2)}" ` +
                `stroke="${color}" stroke-opacity="0.75" stroke-width="1" fill-rule="evenodd"/>`);
            renderedPolygons += 1;
        }
    }
    if (paths.length === 0) {
    ...
    // Small corner badge so PDF viewers can tell this is MRMS-derived.
    const badge = `<g transform="translate(8, ${imgHeight - 22})">` +
        `<rect x="0" y="0" width="132" height="16" rx="3" fill="#111827" fill-opacity="0.75"/>` +
        `<text x="6" y="12" font-family="system-ui, sans-serif" font-size="10" fill="#fff">` +
        `MRMS radar · ${collection.metadata.maxMeshInches.toFixed(2)}" peak` +
        `</text>` +
        `</g>`;
    const svg = `<svg xmlns="http://www.w3.org/2000/svg" width="${imgWidth}" height="${imgHeight}" ` +
        `viewBox="0 0 ${imgWidth} ${imgHeight}">${paths.join('')}${badge}</svg>`;
    void renderedPolygons;
 ...
 */
export async function compositeContourOverlay(baseMapBuffer, events, center, zoom, imgWidth = 600, imgHeight = 300) {
