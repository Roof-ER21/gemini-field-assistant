...
function day0Message(p) {
    const name = p.homeownerName.split(' ')[0]; // First name only
    return (`Hi ${name}! This is ${COMPANY_NAME}. ` +
...
function day3Message(p) {
    const name = p.homeownerName.split(' ')[0];
    return (`Hi ${name}, just checking in from ${COMPANY_NAME}. ` +
...
function day7Message(p) {
    const name = p.homeownerName.split(' ')[0];
    return (`Hi ${name}, final follow-up from ${COMPANY_NAME}. ` +
...
export class LeadSmsService {
    ...
    async enqueueSequence(params) {
        ...
        try {
            for (const { step, scheduledAt } of steps) {
                await this.pool.query(`INSERT INTO lead_sms_followups
           (lead_id, phone_number, homeowner_name, sequence_step, scheduled_at, status)
           VALUES ($1, $2, $3, $4, $5, 'pending')
           ON CONFLICT DO NOTHING`, [leadId, homeownerPhone, homeownerName, step, scheduledAt.toISOString()]);
            }
    ...
    async processPendingFollowups() {
        ...
        try {
            ...
            const result = await this.pool.query(`SELECT f.id, f.lead_id, f.phone_number, f.homeowner_name,
         ...
         LIMIT 50`);
            for (const row of result.rows) {
                ...
                if (await this.isOptedOut(row.phone_number)) {
                    await this.pool.query(`UPDATE lead_sms_followups SET status = 'opted_out', opted_out = TRUE WHERE id = $1`, [row.id]);
                    stats.skipped++;
                    continue;
                }
                // Check if lead was already converted — skip follow-ups
                const leadStatus = await this.pool.query(`SELECT status FROM profile_leads WHERE id = $1`, [row.lead_id]);
                if (['converted', 'closed'].includes(leadStatus.rows[0]?.status)) {
                    await this.pool.query(`UPDATE lead_sms_followups SET status = 'skipped' WHERE id = $1`, [row.id]);
                    stats.skipped++;
                    continue;
                }
        ...
        if (stats.processed > 0) {
            console.log(`[LeadSMS] Processed ${stats.processed}: ` +
    ...
    async handleOptOut(phoneNumber) {
        try {
            // Add to opt-out list
            await this.pool.query(`INSERT INTO sms_opt_outs (phone_number, source)
         VALUES ($1, 'reply_stop')
         ON CONFLICT (phone_number) DO NOTHING`, [phoneNumber]);
            // Mark all pending follow-ups for this phone as opted out
            await this.pool.query(`UPDATE lead_sms_followups
         SET status = 'opted_out', opted_out = TRUE
         WHERE phone_number = $1 AND status = 'pending'`, [phoneNumber]);
    ...
    async getStats() {
        try {
         ...
         FROM lead_sms_followups`);
            const byStep = await this.pool.query(`SELECT
    ...
    async isOptedOut(phone) {
        try {
            const result = await this.pool.query(`SELECT 1 FROM sms_opt_outs WHERE phone_number = $1 LIMIT 1`, [phone]);
