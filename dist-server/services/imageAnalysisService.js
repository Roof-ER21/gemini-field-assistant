 ...
 */
export async function analyzeRoofImage(imageFile) {
 ...
 */
export async function answerFollowUpQuestion(assessment, questionIndex, answer) {
    const apiKey = env.GEMINI_API_KEY;
    if (!apiKey || apiKey === 'PLACEHOLDER_API_KEY') {
        throw new Error('Gemini API key not configured');
    }
 ...
 */
export async function analyzeBatchImages(imageFiles) {
    const results = [];
