 ...
 */
export async function assessPropertyRisk(input) {
    const { zip, lat, lng } = input;
    let medianYearBuilt = null;
    let housingUnits = null;
    ...
    let roofVulnerability = 'moderate';
    if (estimatedRoofAge !== null) {
        if (estimatedRoofAge >= 25)
            roofVulnerability = 'critical';
        else if (estimatedRoofAge >= 15)
            roofVulnerability = 'high';
        else if (estimatedRoofAge >= 8)
            roofVulnerability = 'moderate';
        else
    ...
    if (estimatedRoofAge !== null) {
        if (estimatedRoofAge >= 25)
            riskMultiplier = 1.8; // Old roof, very vulnerable
        else if (estimatedRoofAge >= 15)
            riskMultiplier = 1.4;
        else if (estimatedRoofAge >= 8)
            riskMultiplier = 1.1;
        else
            riskMultiplier = 0.7; // New roof, less likely to have damage
    }
    // Build summary
    let summary = '';
    if (estimatedRoofAge !== null) {
 ...
 */
async function fetchCensusHousingData(zip) {
    ...
    try {
        ...
        console.warn(`[PropertyRisk] Census data fetch failed for ZIP ${zip}:`, err.message);
        return null;
    }
}
