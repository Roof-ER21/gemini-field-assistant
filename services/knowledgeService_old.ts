import { semanticSearch, SearchableItem } from './semanticSearch';
...
// Flag to track if semantic index is initialized
let isIndexInitialized = false;

// Convert document to searchable item
function documentToSearchableItem(doc: Document): SearchableItem {
  // Combine name and category for better semantic matching
  const searchText = `${doc.name} ${doc.category || ''} ${doc.type}`;
  return {
    id: doc.path,
    text: searchText,
    metadata: doc
...
async function initializeSemanticIndex(documents: Document[]): Promise<void> {
  if (isIndexInitialized) return;

  const searchableItems = documents.map(documentToSearchableItem);
  semanticSearch.indexDocuments(searchableItems);
  isIndexInitialized = true;

  console.log('[Semantic Search] Index initialized:', semanticSearch.getStats());
}
...
export const knowledgeService = {
  ...
  async searchDocuments(query: string, documents: Document[]): Promise<Array<{
    ...
    // Initialize semantic index on first search
    await initializeSemanticIndex(documents);

    const startTime = performance.now();

    // Use semantic search
    const results = semanticSearch.search<SearchableItem>(query, {
      minScore: 0.05, // Lower threshold to catch more results
      maxResults: 50
    });
    ...
    console.log(`[Semantic Search] Query: "${query}" | Results: ${results.length} | Time: ${searchTime.toFixed(2)}ms`);

    // Convert search results to expected format
    return results.map(result => {
      const doc = result.item.metadata as Document;
      const matchedTermsStr = result.matchedTerms.length > 0
      ...
      return {
        ...
        snippet: `${doc.name} - ${doc.category || 'General'}${matchedTermsStr ? ` (${matchedTermsStr})` : ''}`
      };
    });
  },

  // Find similar documents (bonus feature)
  async findSimilarDocuments(document: Document, allDocuments: Document[]): Promise<Array<{
    document: Document;
    similarity: number;
  }>> {
    // Initialize semantic index
    await initializeSemanticIndex(allDocuments);

    const results = semanticSearch.findSimilar<SearchableItem>(document.path, {
      minScore: 0.15,
      maxResults: 5
    });

    return results.map(result => ({
      document: result.item.metadata as Document,
