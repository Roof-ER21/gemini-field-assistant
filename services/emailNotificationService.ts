...
class EmailNotificationService {
   ...
   */
  private async sendNotification(type: 'login' | 'chat', data: any): Promise<EmailResponse> {
    if (!this.enabled) {
   ...
   */
  async notifyLogin(payload: LoginNotificationPayload): Promise<EmailResponse> {
    const data = {
   ...
   */
  async notifyChat(payload: ChatNotificationPayload): Promise<EmailResponse> {
    const data = {
      userName: payload.userName,
      userEmail: payload.userEmail,
