...
function pinRateCheck(key: string): { ok: boolean; retryAfterSec: number } {
  const rec = pinAttempts.get(key);
  if (!rec) return { ok: true, retryAfterSec: 0 };
  if (Date.now() - rec.first > PIN_WINDOW_MS) { pinAttempts.delete(key); return { ok: true, retryAfterSec: 0 }; }
  if (rec.count >= PIN_MAX_ATTEMPTS) return { ok: false, retryAfterSec: Math.ceil((PIN_WINDOW_MS - (Date.now() - rec.first)) / 1000) };
  return { ok: true, retryAfterSec: 0 };
}
function pinRecordFail(key: string): void {
  const rec = pinAttempts.get(key);
  if (!rec || Date.now() - rec.first > PIN_WINDOW_MS) pinAttempts.set(key, { count: 1, first: Date.now() });
  else rec.count += 1;
}
function pinRecordSuccess(key: string): void { pinAttempts.delete(key); }
