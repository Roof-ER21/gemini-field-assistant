...
app.use('/mcp', mcpLimiter, createMcpRouter({
  ...
  agentAuth: {
    resolve: async (token) => {
      ...
      if (principal) touchAgentToken(pool, principal.tokenId);
      return principal;
    },
  ...
  try {
    ...
    if (file === 'mesh1440.png') {
      try {
        ...
        console.error('[MRMS] IEM fallback also failed:', (e as Error).message);
      }
    }
  ...
  try {
    ...
    const result = await pool.query(
      `UPDATE users
       ...
       WHERE email = $4
       RETURNING *`,
      [name, state, division, email.toLowerCase()]
    );
    res.json(result.rows[0]);
  } catch (error) {
...
app.patch('/api/admin/users/:userId/division', async (req, res) => {
  try {
    ...
    const result = await pool.query(
      'UPDATE users SET division = $1, updated_at = CURRENT_TIMESTAMP WHERE id = $2 RETURNING id, email, name, division',
      [division, userId]
    );
    if (result.rows.length === 0) {
      return res.status(404).json({ error: 'User not found' });
...
async function learningGlobalHandler(req: express.Request, res: express.Response) {
  try {
...
const googleLoginHandoffs = new Map<string, { user: any; exp: number }>(); // one-time post-login tokens
function readCookie(req: express.Request, name: string): string | null {
  const raw = req.headers.cookie || '';
  const hit = raw.split(';').map((s) => s.trim()).find((s) => s.startsWith(name + '='));
  return hit ? decodeURIComponent(hit.slice(name.length + 1)) : null;
}
async function findOrCreateGoogleUser(email: string, displayName: string) {
  const existing = await pool.query('SELECT id, name, email, role FROM users WHERE LOWER(email) = $1', [email]);
  if (existing.rows.length > 0) {
...
app.get('/api/auth/google/callback', async (req, res) => {
  try {
    ...
    if (!code || !state || !cookieState || state !== cookieState) return res.redirect('/?gl_error=state');
    if (!GOOGLE_CLIENT_SECRET) { console.error('[AUTH] GOOGLE_CLIENT_SECRET is not set'); return res.redirect('/?gl_error=config'); }
    ...
    if (!tokens.id_token) { console.error('[AUTH] Google token exchange failed:', tokens.error_description || tokens.error || tokens); return res.redirect('/?gl_error=token'); }

    const ticket = await googleAuthClient.verifyIdToken({ idToken: tokens.id_token, audience: GOOGLE_CLIENT_ID });
    const payload = ticket.getPayload();
    if (!payload?.email || payload.email_verified !== true) return res.redirect('/?gl_error=email');
    const email = String(payload.email).toLowerCase();
...
app.post('/api/admin/trigger-qr-weekly-report', async (req, res) => {
  try {
    ...
    res.status(500).json({ success: false, error: 'Failed to trigger weekly QR report', message: (error as Error).message });
  }
});
  ...
  try {
    ...
    for (const doc of vaDocs) {
      ...
      const { rows } = await pool.query(
        `SELECT id, LENGTH(content) as len FROM knowledge_documents WHERE name = $1 LIMIT 1`,
        [doc.name]
      );
      if (rows.length > 0 && rows[0].len < 500) {
        await pool.query(
          `UPDATE knowledge_documents SET content = $1 WHERE id = $2`,
          [doc.content, rows[0].id]
        );
        console.log(`✅ Updated VA doc: ${doc.name} (${doc.content.length} chars)`);
      } else if (rows.length === 0) {
        await pool.query(
          `INSERT INTO knowledge_documents (name, category, state, content) VALUES ($1, $2, $3, $4)`,
          [doc.name, 'Insurance Arguments', 'VA', doc.content]
        );
...
(async () => {
  try {
    ...
    const { rows: check } = await pool.query(
      `SELECT COUNT(*) as cnt FROM insurance_companies WHERE phone IS NOT NULL AND phone != ''`
    );
    ...
    for (const [name, category, email, phone, website, notes] of companies) {
      await pool.query(
        `INSERT INTO insurance_companies (name, category, email, phone, website, notes)
         VALUES ($1, $2, $3, $4, $5, $6)
         ON CONFLICT (name) DO UPDATE SET
           ...
           notes = COALESCE(EXCLUDED.notes, insurance_companies.notes)`,
...
app.post('/api/admin/knowledge-docs', async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      'INSERT INTO knowledge_documents (name, category, state, content) VALUES ($1, $2, $3, $4) RETURNING *',
      [name, category || 'General', state || null, content]
    );
    res.json(rows[0]);
  } catch (e) { res.status(500).json({ error: (e as Error).message }); }
});
...
app.put('/api/admin/knowledge-docs/:id', async (req, res) => {
  try {
    const { name, category, state, content } = req.body;
    await pool.query(
      'UPDATE knowledge_documents SET name = COALESCE($1, name), category = COALESCE($2, category), state = $3, content = COALESCE($4, content) WHERE id = $5',
      [name, category, state || null, content, req.params.id]
    );
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: (e as Error).message }); }
});
  ...
  try {
    ...
    const { rows } = await pool.query(
      `SELECT gl.*,
       ...
       WHERE gl.status = $1
       ORDER BY gl.updated_at DESC LIMIT 100`,
      [status]
    );
    res.json(rows);
  } catch (e) { res.status(500).json({ error: (e as Error).message }); }
...
app.put('/api/admin/learnings/:id/approve', async (req, res) => {
  try {
    ...
    await pool.query(
      'UPDATE global_learnings SET status = $1, approved_by = $2, approved_at = NOW(), updated_at = NOW() WHERE id = $3',
...
app.put('/api/admin/learnings/:id/reject', async (req, res) => {
  try {
    await pool.query(
      'UPDATE global_learnings SET status = $1, updated_at = NOW() WHERE id = $2',
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT key, value FROM user_memory
       WHERE user_id = $1 AND category = 'agent_personality'`,
      [userId]
    );
  ...
  try {
    ...
    for (const [key, value] of Object.entries(updates)) {
      ...
      if (!value || value.trim().length === 0) {
        ...
        await pool.query(
          `DELETE FROM user_memory WHERE user_id = $1 AND category = 'agent_personality' AND key = $2`,
          [userId, key]
        );
      } else {
        await pool.query(
...
async function insuranceCompaniesHandler(req: express.Request, res: express.Response) {
  try {
...
(async () => {
  try {
    ...
    console.error('[web] schema bootstrap failed:', (e as Error).message);
  }
...
app.post('/api/webhooks/veterans-nomination', async (req, res) => {
  ...
  if (!expected) return res.status(503).json({ error: 'veterans-notify: not configured' });
  ...
  try {
    ...
    console.error('[Veterans nomination email] error:', (e as Error)?.message);
...
app.get('/admin/campaigns', async (req, res, next) => {
  try {
    const email = String(req.query.email || req.header('x-user-email') || '').trim();
    if (!email) return res.status(401).send('<h1>401</h1><p>Pass ?email=&lt;your email&gt; to view the marketing dashboard.</p>');
    ...
    const reps = (await pool.query(
      `SELECT id, name, slug, role_type FROM employee_profiles WHERE is_active = true AND slug IS NOT NULL ORDER BY name ASC`
    )).rows;
    let countRows: any[] = [];
    try {
      countRows = (await pool.query(
        `SELECT profile_id, COALESCE(NULLIF(source,''),'(direct)') AS source, COUNT(*)::int AS n
           FROM profile_leads GROUP BY profile_id, COALESCE(NULLIF(source,''),'(direct)')`
      )).rows;
    } catch (e) { console.error('[campaigns] count query failed:', e); }
    res.set('Content-Type', 'text/html');
    res.set('Cache-Control', 'private, no-store');
    res.send(renderCampaignDashboard(reps, countRows, email));
  } catch (err) { console.error('campaign dashboard error:', err); next(err); }
});

function renderCampaignDashboard(reps: any[], countRows: any[], email: string): string {
  const esc = (s: any) => String(s ?? '').replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' } as any)[c]);
  const LEAD = 'https://get.theroofdocs.com';
  const PAGES: [string, string][] = [['free-inspection', 'Free Inspection'], ['claim-help', 'Claim Help'], ['storm-checklist', 'Storm Checklist'], ['roofcheck', 'RoofCheck']];
  const byRep: Record<string, { total: number; src: Record<string, number> }> = {};
  let grand = 0;
  for (const r of countRows) {
    const id = String(r.profile_id || '');
    if (!byRep[id]) byRep[id] = { total: 0, src: {} };
    byRep[id].total += r.n; byRep[id].src[r.source] = (byRep[id].src[r.source] || 0) + r.n; grand += r.n;
  }
...
app.get('/profile/:slug', async (req, res, next) => {
  try {
    ...
    if (ownReviews.rows.length > 0) {
      ...
      const globalReviews = await pool.query(
        `SELECT id, text, author, date_label, source, rating
           FROM profile_reviews
          WHERE profile_id IS NULL AND is_active = TRUE
          ORDER BY display_order ASC, created_at ASC
      ...
      <div class="hero-content">
        <div class="hero-left">
          ...
          <div class="contact-buttons">
            ...
            <button type="button" class="contact-btn" id="share-btn" data-slug="${profile.slug}" data-name="${String(name).replace(/[&<>"']/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'} as any)[c])}" style="cursor:pointer;border:1px solid rgba(255,255,255,0.1);background:rgba(255,255,255,0.05);font-family:inherit;font-size:14px;"><svg width="20" height="20" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8.684 13.342C8.886 12.938 9 12.482 9 12c0-.482-.114-.938-.316-1.342m0 2.684a3 3 0 110-2.684m0 2.684l6.632 3.316m-6.632-6l6.632-3.316m0 0a3 3 0 105.367-2.684 3 3 0 00-5.367 2.684zm0 9.316a3 3 0 105.368 2.684 3 3 0 00-5.368-2.684z"/></svg>Share</button>
          </div>
          <script>
            (function(){
...
function renderProfilePageV2(profile: any): string {
  ...
  if (veteransBase) {
    try {
      ...
      if (!isCompany && slug) u.searchParams.set('rep', slug);
