...
export function agentMayCallTool(
  principal: Pick<AgentPrincipal, 'scopes'>,
  tool: { name: string; annotations?: { readOnlyHint?: boolean } } | undefined,
): boolean {
  if (!tool) return false;
  if (!principal.scopes.includes(AGENT_READ_SCOPE)) return false;
  if (!(AGENT_READ_TOOLS as readonly string[]).includes(tool.name)) return false;
  return tool.annotations?.readOnlyHint === true;
}

/** Idempotent; called from runStartupMigrations on every boot, like ensureSessionTable. */
export async function ensureAgentTokenTables(pool: pg.Pool): Promise<void> {
  await pool.query(`
...
function present(row: Record<string, any>): AgentTokenRow {
...
export function parseAgentTokenRequest(body: unknown):
  | { ok: true; name: string; days: number }
  | { ok: false; error: string } {
  const input = (body ?? {}) as { name?: unknown; expiresInDays?: unknown };
...
export async function mintAgentToken(
  pool: pg.Pool,
  input: { userId: string; name: string; days: number },
  ...
  const result = await pool.query(
    `INSERT INTO agent_tokens (user_id, name, token_hash, token_hint, scopes, expires_at)
     VALUES ($1, $2, $3, $4, ARRAY[$5]::text[], $6)
     RETURNING id, name, token_hint, scopes, created_at, last_used_at, expires_at, revoked_at`,
    [input.userId, input.name, hashAgentToken(token), token.slice(-4), AGENT_READ_SCOPE, expiresAt],
  );
...
export async function listAgentTokens(pool: pg.Pool, userId: string): Promise<AgentTokenRow[]> {
  const result = await pool.query(
...
export async function revokeAgentToken(
  ...
  if (!/^[0-9a-f-]{36}$/i.test(input.id)) return 'not_found';
  ...
  if ((updated.rowCount ?? 0) > 0) return 'revoked';
  const existing = await pool.query(
    `SELECT revoked_at FROM agent_tokens WHERE id = $1 AND user_id = $2`,
    [input.id, input.userId],
  );
 ...
 */
export async function resolveAgentToken(pool: pg.Pool, token: string): Promise<AgentPrincipal | null> {
  ...
  try {
    ...
    console.error('[agent-tokens] lookup failed:', (err as Error).message);
    return null;
  }
}
...
export function touchAgentToken(pool: pg.Pool, tokenId: string): void {
  const now = Date.now();
  if ((lastTouched.get(tokenId) ?? 0) > now - 60_000) return;
...
export async function recordAgentToolUse(pool: pg.Pool, use: AgentToolUse): Promise<void> {
  try {
    await pool.query(
      `INSERT INTO agent_token_audit (token_id, user_id, tool, argument_keys, outcome, error, duration_ms)
    ...
    console.warn('[agent-tokens] audit write failed:', (err as Error).message);
  }
}

export type AgentRequest = express.Request & { agent?: AgentPrincipal };

export function agentBearerFrom(req: express.Request): string | null {
...
/** Test seam. */
export function clearAgentTokenTouches(): void {
  lastTouched.clear();
}
