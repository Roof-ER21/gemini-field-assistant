...
export type ScanSource = 'qr' | 'card_likely' | 'social' | 'referral' | 'direct';
...
const IN_APP_BROWSER =
  /(FBAN|FBAV|FB_IAB|FBIOS|FBDV|Instagram|Snapchat|TikTok|musical_ly|Line\/|Twitter|LinkedInApp|Pinterest)/i;
 ...
 */
export function classifyScanSource(req: Request, isMobile?: boolean): ScanSource {
  const marker = String(req.query.src || req.query.utm_source || '').toLowerCase();
  if (marker === 'qr' || marker === 'card') return 'qr';

  const userAgent = String(req.headers['user-agent'] || '');
  if (isInAppBrowser(userAgent)) return 'social';
