...
import type { SessionRequest } from '../auth/session.js';
import {
  agentBearerFrom,
  agentMayCallTool,
  type AgentPrincipal,
...
interface StringProp { type: 'string'; description: string; minLength?: number; maxLength: number; pattern?: string }
const strProp = (description: string, minLength = 1, maxLength = MCP_ARG_MAX_CHARS): StringProp =>
...
const limitProp = (max: number): StringProp =>
  ({ type: 'string', description: `Optional row cap, "1" to "${max}" (default "${max}").`, minLength: 1, maxLength: 2, pattern: '^[0-9]{1,2}$' });

type InputSchema = { type: 'object'; properties: Record<string, StringProp>; required: string[]; additionalProperties: false };

/** Strict input schemas: only string params, each bounded, no extras. */
const INPUT_SCHEMAS: Record<McpToolName, InputSchema> = {
  ask: {
    type: 'object',
...
const DESCRIPTIONS: Record<McpToolName, string> = {
  ask:
    'Susan\'s own answer to one roofing or insurance question, in her voice. MODEL CALL: this runs sa21\'s Gemini model and spends its quota on every call; ' +
  ...
  carrier_directory:
    'An insurance carrier\'s phone, email, address, website and the team\'s notes on it, from sa21\'s carrier directory (the query is part of the carrier\'s name).',
  carrier_learnings:
    'What the team has learned about one carrier: approved, reusable learnings scoped to that insurer, most helpful first (the query is the carrier\'s exact name).',
  adjuster_learnings:
    'What the team has learned about one adjuster: approved learnings scoped to that adjuster\'s name, most helpful first (the query is the adjuster\'s exact name).',
  rep_directory:
    'A rep\'s name, email and username from sa21\'s team roster, narrowed by a required search string (at most 10 rows). Never the whole roster.',
};
...
export interface McpToolDescriptor {
  ...
  annotations: { readOnlyHint: true; destructiveHint: false; idempotentHint: boolean; openWorldHint: boolean };
}

export const MCP_TOOLS: readonly McpToolDescriptor[] = MCP_TOOL_NAMES.map((name) => ({
  name,
...
/** What running one tool produces. `ok:false` becomes an isError result. */
export interface McpExecResult {
  ok: boolean;
  data: unknown;
  /** Short, hostname-free reason when !ok. */
  error?: string;
}
/** Who the call is made as — the resolved session, never a client header. */
export interface McpCaller { userId: string; email: string }
export type McpToolExecutor = (args: Record<string, string>, caller: McpCaller) => Promise<McpExecResult>;
export type McpExecutors = Record<McpToolName, McpToolExecutor>;
...
function send(res: Response, body: unknown, status = 200, headers: Record<string, string> = {}): void {
  res.status(status);
...
export interface McpServerOptions {
  executors: McpExecutors;
  /**
   * The SAME session middleware that fronts /api (createSessionMiddleware(pool)).
   * The router mounts BEFORE the app-wide body parser so a malformed body is a
   * JSON-RPC parse error rather than the app's HTML/JSON 500, which means it
   * also runs before the app-wide session middleware — so it takes its own
   * instance. Same pool, same cache, same counters.
   */
  sessionMiddleware: (req: Request, res: Response, next: express.NextFunction) => unknown;
  /** The rep/homeowner split: true when the request hit the homeowner domain. Same check the rep login uses. */
  isHomeownerHost: (req: Request) => boolean;
  /** The app's own origins (the CORS allowlist). Absent Origin and localhost pass without consulting this. */
  originAllowed: (origin: string) => boolean;
  /**
   * Agent tokens (`s21a_…`). Absent → an agent token is simply not a
   * credential here (401), exactly as before this option existed.
   */
  agentAuth?: {
    resolve: (token: string) => Promise<AgentPrincipal | null>;
    /** Called for every agent tools/call, allowed or refused. Must not throw. */
    onToolUse?: (use: AgentToolUse) => void;
  };
}

/** Returns the refusal, or null when the request's Origin may pass. */
export function originRefusal(req: Request, originAllowed: (origin: string) => boolean): string | null {
  const origin = req.header('origin');
  if (origin == null) return null;
  if (LOCAL_ORIGIN.test(origin)) return null;
 ...
 */
export function mcpParseErrorHandler(err: unknown, _req: Request, res: Response, next: (e?: unknown) => void): void {
 ...
 */
export function validateArgs(name: McpToolName, raw: unknown): { ok: true; args: Record<string, string> } | { ok: false; reason: string } {
  const schema = INPUT_SCHEMAS[name];
  const input = raw === undefined ? {} : raw;
  if (typeof input !== 'object' || input === null || Array.isArray(input)) return { ok: false, reason: 'arguments must be an object.' };
  const args: Record<string, string> = {};
  for (const [key, value] of Object.entries(input as Record<string, unknown>)) {
    const prop = schema.properties[key];
    if (!prop) return { ok: false, reason: `Unknown argument: ${key.slice(0, 40)}` };
    if (typeof value !== 'string') return { ok: false, reason: `${key} must be a string.` };
    if (hasControlChar(value)) return { ok: false, reason: `${key} must be one line.` };
    const cleaned = value.replace(/\s+/g, ' ').trim();
    if (cleaned.length > prop.maxLength) return { ok: false, reason: `${key} is longer than ${prop.maxLength} characters.` };
    if (cleaned.length === 0) continue; // an empty optional is the same as absent
    if (cleaned.length < (prop.minLength ?? 0)) return { ok: false, reason: `${key} is too short.` };
    if (prop.pattern && !new RegExp(prop.pattern).test(cleaned)) return { ok: false, reason: `${key} is not in the expected form.` };
    args[key] = cleaned;
  }
  for (const required of schema.required) {
    if (!(required in args)) return { ok: false, reason: `${required} is required.` };
  }
  return { ok: true, args };
}
...
function toolResult(result: McpExecResult) {
  if (!result.ok) {
    return { content: [{ type: 'text', text: result.error ?? 'The tool could not answer.' }], isError: true };
  }
  let text: string;
  try { text = JSON.stringify(result.data ?? null); } catch { text = String(result.data); }
...
export function createMcpHandler(options: McpServerOptions) {
  ...
  return async function mcpPost(req: Request, res: Response): Promise<void> {
    const origin = originRefusal(req, originAllowed);
    if (origin) { send(res, rpcError(null, JSON_RPC.CONNECTION_REFUSED, origin), 403); return; }
    ...
    const identified = req as SessionRequest & AgentRequest;
    ...
    if (!message || typeof message !== 'object' || (message as { jsonrpc?: unknown }).jsonrpc !== '2.0' || typeof (message as { method?: unknown }).method !== 'string') {
      send(res, rpcError(null, JSON_RPC.INVALID_REQUEST, 'Invalid request.'), 400);
      return;
    }
    const { id: rawId, method, params } = message as { id?: unknown; method: string; params?: unknown };
    ...
    if (!hasId) {
      if (method.startsWith('notifications/')) {
        res.status(202); res.setHeader('Cache-Control', 'private, no-store'); res.end();
        return;
      }
    ...
    if (method === 'initialize') {
      send(res, rpcResult(id, {
        ...
        instructions:
          'Susan (sa21) read tools for Roof-ER reps: the carrier directory, the team\'s carrier and adjuster learnings, a rep lookup, and Susan\'s own answer to a question. ' +
    ...
    if (method === 'ping') { send(res, rpcResult(id, {})); return; }
    ...
    if (method === 'tools/call') {
      ...
      if (!call || typeof call.name !== 'string') { send(res, rpcError(id, JSON_RPC.INVALID_PARAMS, 'tools/call needs a tool name.')); return; }
      ...
      const argumentKeys = call.arguments && typeof call.arguments === 'object' && !Array.isArray(call.arguments)
        ? Object.keys(call.arguments as Record<string, unknown>)
        : [];
      const started = Date.now();
      const audit = (outcome: AgentToolUse['outcome'], error?: string) => {
        if (!agent) return;
        try {
      ...
      const validated = validateArgs(name as McpToolName, call.arguments);
      if (validated.ok === false) {
        audit('error', 'invalid arguments');
        send(res, rpcError(id, JSON_RPC.INVALID_PARAMS, validated.reason));
        return;
      }
      let result: McpExecResult;
      try {
        result = await executors[name as McpToolName](validated.args, caller);
      } catch (err) {
...
export function createMcpRouter(options: McpServerOptions): Router {
  ...
  router.use((req, res, next) => {
    if (options.isHomeownerHost(req)) { res.status(404).json({ error: 'Not found' }); return; }
    if (req.path !== '/') { res.status(404).json({ error: 'Not found' }); return; }
    if (req.method !== 'POST') { mcpMethodNotAllowed(req, res); return; }
    next();
  });
  ...
  router.use((req, res, next) => {
    ...
    options.agentAuth.resolve(agentToken).then((principal) => {
      if (!principal) { unauthorized(res); return; }
      (req as AgentRequest).agent = principal;
      (req as SessionRequest).authMechanism = 'agent-token';
      next();
    }).catch(next);
  });
