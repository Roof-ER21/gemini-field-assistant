...
// Each concrete backfiller is imported lazily to avoid circular deps
// and to let individual sources be added/removed without touching this file.

export interface BackfillJobConfig {
  sources: SourceName[];
  states: string[];                   // e.g. ['VA', 'MD', 'PA']
  fromDate: string;                   // YYYY-MM-DD
  toDate: string;                     // YYYY-MM-DD
  dryRun?: boolean;                   // default false — if true, count only, no writes
  resumeFromProgress?: boolean;       // default true — pick up where previous run stopped
  onProgress?: (update: BackfillProgress) => void;
}

export interface BackfillProgress {
  source: SourceName;
  phase: 'starting' | 'fetching' | 'parsing' | 'upserting' | 'done' | 'error';
  rowsInput: number;
  rowsInserted: number;
  rowsUpdated: number;
  rowsSkipped: number;
...
export interface BackfillResult {
  ...
  rowsUpdated: number;
  rowsSkipped: number;
  errors: Array<{ reason: string; sample?: any }>;
  startedAt: string;
  finishedAt: string;
  durationSec: number;
}
...
export interface BackfillRunner {
  ...
  run(params: {
    pool: Pool;
    verifiedSvc: VerifiedEventsService;
    states: string[];
    fromDate: string;
    toDate: string;
    dryRun: boolean;
...
async function ensureProgressTable(pool: Pool): Promise<void> {
  await pool.query(`
...
export async function markWindowStart(
  pool: Pool,
  source: string,
  runId: string,
  windowKey: string,
  ...
  await pool.query(
    `INSERT INTO backfill_progress (source, run_id, window_key, status, started_at)
     VALUES ($1, $2, $3, 'in_progress', NOW())
     ON CONFLICT (source, run_id, window_key) DO UPDATE SET
       status = 'in_progress', started_at = NOW(), error_detail = NULL`,
    [source, runId, windowKey],
  );
}

export async function markWindowComplete(
  pool: Pool,
  source: string,
  runId: string,
  windowKey: string,
  stats: { rowsInput: number; rowsInserted: number; rowsUpdated: number; rowsSkipped: number; errorCount: number },
): Promise<void> {
  await pool.query(
    `UPDATE backfill_progress
     ...
     WHERE source = $1 AND run_id = $2 AND window_key = $3`,
    [source, runId, windowKey, stats.rowsInput, stats.rowsInserted, stats.rowsUpdated, stats.rowsSkipped, stats.errorCount],
  );
}

export async function markWindowFailed(
  pool: Pool,
  source: string,
  runId: string,
  windowKey: string,
  ...
  await pool.query(
    `UPDATE backfill_progress
     ...
     WHERE source = $1 AND run_id = $2 AND window_key = $3`,
    [source, runId, windowKey, errorMsg.slice(0, 2000)],
  );
}

export async function isWindowComplete(
  pool: Pool,
  source: string,
  runId: string,
  windowKey: string,
...
class BackfillOrchestrator {
  private runners = new Map<SourceName, BackfillRunner>();

  register(runner: BackfillRunner) {
    this.runners.set(runner.name, runner);
  }

  async run(pool: Pool, config: BackfillJobConfig): Promise<BackfillResult[]> {
   ...
   */
  async summary(pool: Pool): Promise<any> {
    const r = await pool.query(`
      SELECT
        source,
