...
function buildTextQuery(propertyLabel: string, stormDate: string | null): string {
...
async function searchFlickrCandidates(
  params: EvidenceSearchParams,
  stormDate: string | null,
): Promise<EvidenceCandidate[]> {
  if (!FLICKR_API_KEY) {
  ...
  const response = await fetch(
    `https://www.flickr.com/services/rest/?${query.toString()}`,
    { signal: AbortSignal.timeout(12000) },
  );
...
export async function searchEvidenceCandidates(
  ...
  const uniqueStormDates = Array.from(
    new Set(params.stormDates.filter(Boolean).slice(0, 2)),
  );
...
export type { EvidenceCandidate, EvidenceSearchParams };
