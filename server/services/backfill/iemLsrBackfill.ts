...
export const iemLsrBackfill: BackfillRunner = {
  name: 'iem_lsr' as SourceName,
  ...
  async run({ pool, verifiedSvc, states, fromDate, toDate, dryRun, onProgress }) {
    ...
    const windows: Array<{ sts: string; ets: string; label: string }> = [];
    ...
    for (const w of windows) {
      ...
      try {
        ...
        onProgress?.({
          source: 'iem_lsr' as SourceName, phase: 'fetching',
        ...
        for (const r of relevant) {
          ...
          else if (WIND_TYPES.has(t) && !isNaN(mag)) wind = Math.round(mag);       // LSR wind already in mph
