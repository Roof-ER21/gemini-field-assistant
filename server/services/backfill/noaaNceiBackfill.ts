...
async function fetchAndParseYear(year: number): Promise<Record<string, string>[]> {
 ...
 */
function parseNceiDate(dateStr: string): string | null {
 ...
 */
function extractMeasurements(row: Record<string, string>): {
  ...
  if (eventType === 'Hail' || eventType === 'Marine Hail') {
    ...
    if (magnitude != null) windMph = Math.round(magnitude * 1.15078);
    ...
    if (emf) tornadoEfRank = parseInt(emf[1], 10);
    // Also try old F-scale
    const fmf = ts.match(/^F([0-5])$/);
    if (!emf && fmf) tornadoEfRank = parseInt(fmf[1], 10);
  }
 ...
 */
function extractLocation(row: Record<string, string>): { lat: number; lng: number } | null {
  ...
  if (isNaN(lat) || isNaN(lng) || lat === 0 || lng === 0) {
    ...
    if (isNaN(elat) || isNaN(elng) || elat === 0 || elng === 0) return null;
...
export const noaaNceiBackfill: BackfillRunner = {
  name: 'noaa_ncei' as SourceName,
  ...
  async run({ pool, verifiedSvc, states, fromDate, toDate, dryRun, onProgress }) {
    ...
    const stateFullNames = new Set(
      states.map((s) => STATE_FULL_NAMES[s]).filter(Boolean)
    );
    const stateAbbrevByFullName = Object.fromEntries(
      Object.entries(STATE_FULL_NAMES).map(([k, v]) => [v, k])
    );
    ...
    for (let year = startYear; year <= endYear; year++) {
      ...
      try {
        ...
        const relevant = allRows.filter((r) =>
          stateFullNames.has((r.STATE || '').toUpperCase().trim()) &&
        ...
        for (const row of relevant) {
          ...
          if (hailSizeInches == null && windMph == null && tornadoEfRank == null) continue;
