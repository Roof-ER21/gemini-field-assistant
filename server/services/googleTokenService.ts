...
function createOAuth2Client() {
  return new google.auth.OAuth2(
    ...
    // Redirect URI determined at request time, not here
  );
}
...
export async function getValidOAuth2Client(
  ...
  const result = await pool.query(
    `SELECT * FROM google_oauth_tokens WHERE user_id = $1 AND revoked_at IS NULL AND access_token_encrypted != 'pending'`,
    [userId]
  );
  ...
  const accessToken = decrypt(
    row.access_token_encrypted, row.access_token_iv, row.access_token_tag
  );
  const refreshToken = decrypt(
    row.refresh_token_encrypted, row.refresh_token_iv, row.refresh_token_tag
  );
  ...
  if (Date.now() > expiresAt - bufferMs) {
    ...
    try {
      ...
      console.error(`[GoogleToken] Refresh failed for user=${userId}:`, (err as Error).message);
...
export async function getGoogleStatus(
  pool: pg.Pool,
  userId: string
): Promise<{ connected: boolean; google_email: string | null; scope: string | null; connected_at: string | null }> {
  const result = await pool.query(
    `SELECT google_email, scope, created_at FROM google_oauth_tokens
     WHERE user_id = $1 AND revoked_at IS NULL AND access_token_encrypted != 'pending'`,
    [userId]
  );
 ...
 */
export async function revokeTokens(pool: pg.Pool, userId: string): Promise<void> {
  ...
  try {
    ...
    console.warn(`[GoogleToken] Revoke with Google failed (best-effort):`, (err as Error).message);
  }

  await pool.query(
    `UPDATE google_oauth_tokens SET revoked_at = NOW() WHERE user_id = $1 AND revoked_at IS NULL`,
    [userId]
  );
