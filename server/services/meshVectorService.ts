 ...
 */
export function meshGridToPolygons(grid: GridInput, date: string): SwathPolygonCollection {
  ...
  for (let i = 0; i < mmGrid.length; i++) {
    ...
    if (mm >= HAIL_LEVELS[0].sizeMm) hailCells++;
  }
  ...
  function pixelToGeo(x: number, y: number): [number, number] {
  ...
  const features: SwathPolygonFeature[] = [];
  ...
  for (const contour of contourResults) {
    ...
    for (const polygon of contour.coordinates) {
      const geoPolygon: number[][][] = [];

      for (const ring of polygon) {
        const geoRing: number[][] = [];
