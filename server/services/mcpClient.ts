...
export async function mcpCall<T = unknown>(
  ...
  try {
    ...
    const message = (err as Error).name === 'AbortError'
      ...
      : `Could not reach Roof HR: ${(err as Error).message}`;
  ...
  const rpc = envelope as { result?: unknown; error?: { code?: number; message?: string } };
  if (rpc.error) {
    throw new McpError(rpc.error.message || 'Roof HR refused that request.', rpc.error.code);
  }
  if (rpc.result === undefined) {
    if (!response.ok) throw new McpError(`Roof HR returned HTTP ${response.status}.`);
    throw new McpError('Roof HR returned an empty result.');
  }
  return rpc.result as T;
}

export async function listTools(endpoint: string, token: string): Promise<McpToolDefinition[]> {
  const result = await mcpCall<{ tools?: McpToolDefinition[] }>(endpoint, token, 'tools/list');
...
export async function callTool(
  ...
  return await mcpCall<McpToolCallResult>(endpoint, token, 'tools/call', { name, arguments: args });
}

/** Flatten an MCP result's text blocks into the one string a Gemini tool response carries. */
export function resultText(result: McpToolCallResult): string {
