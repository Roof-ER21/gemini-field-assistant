/**
 * mPING (Meteorological Phenomena Identification Near the Ground) live ingest
 *
 * University of Oklahoma / NWS-sponsored crowd-sourced weather reports app.
 * Covers hail, wind damage, rain types, etc. Fills coverage gaps where
 * official NWS LSRs / CoCoRaHS observers are sparse.
 *
 * API: https://mping.ou.edu/mping/api/v2/reports
 * Auth: Token (free, email signup at mping.ou.edu/mping/profile/register).
 *       Set MPING_API_TOKEN env var.
 *
 * Feature flags:
 *   MPING_LIVE_ENABLED (default false)
 *   MPING_API_TOKEN    (required; service no-ops if missing)
 */
import { Pool } from 'pg';
...
const HAIL_SIZE_MAP: Record<string, number> = {
  'Pea (0.25 in)': 0.25,
...
function parseHailSize(desc: string | undefined | null): number | null {
  if (!desc) return null;
  if (HAIL_SIZE_MAP[desc] !== undefined) return HAIL_SIZE_MAP[desc];
...
export class MpingLiveService {
  ...
  private bbox: { minLat: number; maxLat: number; minLng: number; maxLng: number };
  ...
  private reportsToBatch(reports: MpingReport[]): any[] {
    const batch: any[] = [];
    for (const r of reports) {
      ...
      if (!r.obtime) continue;
      const hail = parseHailSize(r.description);
      if (hail == null) continue;
      batch.push({
        eventDate: r.obtime,
        latitude: lat,
        longitude: lng,
   ...
   */
  async ingestRecent(hoursBack = 12): Promise<{ fetched: number; inserted: number; updated: number; errors: number }> {
    if (!this.enabled) {
