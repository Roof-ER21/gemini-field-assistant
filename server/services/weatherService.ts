...
class WeatherService {
  ...
  async getStormEvents(lat: number, lng: number, months: number = 24): Promise<WeatherEvent[]> {
    if (!this.apiKey) {
  ...
  private parseEvents(data: any, defaultLat: number, defaultLng: number): WeatherEvent[] {
  ...
  private classifyEvent(event: any): WeatherEvent['type'] {
    ...
    if (type.includes('wind') || type.includes('gust') || type.includes('thunderstorm')) return 'wind';
    return 'other';
  }

  private inferSeverity(event: any, day: any): WeatherEvent['severity'] {
    // Check hail size first
    if (event.hailsize >= 2) return 'severe';
    if (event.hailsize >= 1) return 'moderate';

    // Check wind speeds
    const windSpeed = event.windspeed || event.windgust || day.windgust || 0;
    if (windSpeed >= 75) return 'severe';       // Severe thunderstorm
    if (windSpeed >= 58) return 'moderate';     // Severe criteria threshold
