...
export interface SynopticEnvelope {
  SUMMARY: {
...
export interface SynopticRawStation {
  STID: string;
  NAME?: string;
  LATITUDE?: string;
  LONGITUDE?: string;
  ...
  SENSOR_VARIABLES?: Record<string, Record<string, { position: string }>>;
}
...
export class SynopticClient {
  ...
  async fetchTimeseriesByRadius(q: RadiusQuery): Promise<GroundStation[]> {
  ...
  async fetchTimeseriesByBbox(q: BboxQuery): Promise<GroundStation[]> {
  ...
  private async fetchAndParse(params: URLSearchParams): Promise<GroundStation[]> {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          `Synoptic HTTP ${res.status} ${res.statusText}: ${body.slice(0, 300)} (url=${safeUrl})`,
        );
      }
