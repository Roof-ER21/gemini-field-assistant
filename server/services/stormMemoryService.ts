...
export interface SaveStormLookupParams {
  userId: string;
  address: string;
  city?: string;
  state?: string;
...
export class StormMemoryService {
   ...
   */
  async saveStormLookup(params: SaveStormLookupParams): Promise<StormLookup> {
    const {
      userId,
      address,
      city,
   ...
   */
  async findNearbyStorms(
    latitude: number,
    longitude: number,
    radiusMiles: number = 10,
    userId?: string
  ): Promise<NearbyStorm[]> {
    const params: any[] = [latitude, longitude, radiusMiles];
    let userFilter = '';
   ...
   */
  async getStormsByZipCode(zipCode: string, userId?: string): Promise<StormLookup[]> {
    const params: any[] = [zipCode];
    let userFilter = '';
   ...
   */
  async getStormsByCity(city: string, state: string, userId?: string): Promise<StormLookup[]> {
   ...
   * Get lookup by ID
   */
  async getLookupById(lookupId: string): Promise<StormLookup | null> {
    const result = await this.pool.query(
      'SELECT * FROM storm_lookups WHERE id = $1',
      [lookupId]
    );
   ...
   */
  async getLookupsByUser(userId: string, limit: number = 100): Promise<StormLookup[]> {
    const result = await this.pool.query(
      `SELECT * FROM storm_lookups
      WHERE user_id = $1
      ORDER BY created_at DESC
   ...
   */
  async getStormStats(userId?: string): Promise<{
    totalLookups: number;
    totalEvents: number;
    byOutcome: Record<string, number>;
    byDataSource: { noaa: number; ihm: number };
  }> {
   ...
   */
  async deleteLookup(lookupId: string, userId: string): Promise<boolean> {
    const result = await this.pool.query(
      'DELETE FROM storm_lookups WHERE id = $1 AND user_id = $2',
      [lookupId, userId]
    );
   ...
   */
  private rowToStormLookup(row: any): StormLookup {
    return {
      id: row.id,
      userId: row.user_id,
      address: row.address,
 ...
 */
export const createStormMemoryService = (pool: Pool): StormMemoryService => {
