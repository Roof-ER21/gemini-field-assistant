...
// ============================================================
// MRMS Vector Swath compositing (replaces convex-hull approximation)
// ============================================================

interface SwathPolygonFeature {
  type: 'Feature';
  properties: {
    level: number;
    sizeInches: number;
...
interface SwathPolygonCollection {
 ...
 */
export async function compositeVectorSwathOverlay(
  baseMapBuffer: Buffer,
  collection: SwathPolygonCollection,
  ...
  const lngToPx = (lng: number) =>
    ((lng - bounds.west) / (bounds.east - bounds.west)) * imgWidth;
  const latToPx = (lat: number) =>
    ((bounds.north - lat) / (bounds.north - bounds.south)) * imgHeight;

  // Sort low → high so heavier hail renders on top.
  const sorted = [...collection.features].sort(
    (a, b) => a.properties.sizeInches - b.properties.sizeInches,
  );
  ...
  for (const feature of sorted) {
    ...
    for (const polygon of feature.geometry.coordinates) {
      ...
      paths.push(
        `<path d="${ringPaths.join(' ')}" fill="${color}" fill-opacity="${opacity.toFixed(2)}" ` +
  ...
  const badge =
    `<g transform="translate(8, ${imgHeight - 22})">` +
