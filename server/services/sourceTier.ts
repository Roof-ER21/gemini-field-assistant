...
const PRIMARY_SOURCE_KEYS = new Set<string>([
  'mrms',
  'nexrad-mrms',
  'source_mrms',
  'nws-lsr',
...
const SUPPLEMENTAL_SOURCE_KEYS = new Set<string>([
  'mping',
  'source_mping',
  'cocorahs',
  'source_cocorahs',
...
export function classifySource(source: string): SourceTier {
  ...
  if (PRIMARY_SOURCE_KEYS.has(normalized)) return 'primary';
  if (SUPPLEMENTAL_SOURCE_KEYS.has(normalized)) return 'supplemental';
  return 'supplemental';
}

export function isPrimary(source: string): boolean {
  return classifySource(source) === 'primary';
}

export function isSupplemental(source: string): boolean {
 ...
 */
const GOVERNMENT_OBSERVER_KEYS = new Set<string>([
  'nws-lsr',
  'nws-alert',
  'nws-alerts',
  'source_nws_alert',
...
export function isGovernmentObserver(source: string): boolean {
 ...
 */
export function filterToPrimary<T extends { source: string }>(items: T[]): T[] {
