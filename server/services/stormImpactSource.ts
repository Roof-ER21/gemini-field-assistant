...
import type { AddressImpactReport } from './addressImpactService.js';

export async function getAddressHailImpactPreferIHM(
  lat: number,
  lng: number,
  ...
  if (ihmConfigured()) {
    try {
      ...
      console.warn('[StormSource] IHM primary failed, falling back to Hail Yes:', (e as Error).message);
    }
  }
