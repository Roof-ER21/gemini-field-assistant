...
export interface DamageScoreInput {
  lat?: number;
  lng?: number;
  address?: string;
  events?: HailEvent[];
...
export interface DamageScoreFactors {
  eventCount: number;
  stormSystemCount: number; // Deduplicated storm systems (temporal clustering)
  maxHailSize: number;
  ...
  recencyScore: number;
  documentedDamage: number; // Total property damage from NOAA records ($)
  windEvents: number;       // Count of damaging wind events
}
...
export class DamageScoreService {
  ...
  calculateDamageScore(input: DamageScoreInput): DamageScoreResult {
    ...
    // Initialize factors
    const factors: DamageScoreFactors = {
  ...
  private combineEvents(
    events: HailEvent[],
    noaaEvents: NOAAEvent[]
  ): Array<{ date: Date; hailSize: number; episodeId?: string }> {
    const combined: Array<{ date: Date; hailSize: number; episodeId?: string }> = [];
   ...
   */
  private clusterByStormSystem(
    events: Array<{ date: Date; hailSize: number; episodeId?: string }>
  ): Array<{ date: Date; maxHailSize: number; eventCount: number }> {
    const systems = new Map<string, { date: Date; maxHailSize: number; eventCount: number }>();
   ...
   */
  private sumDocumentedDamage(noaaEvents: NOAAEvent[]): number {
    let total = 0;
   ...
   */
  private getMaxHailSize(events: Array<{ date: Date; hailSize: number }>): number {
   ...
   */
  private getRecentEventCount(events: Array<{ date: Date; hailSize: number }>): number {
   ...
   */
  private getCumulativeExposure(events: Array<{ date: Date; hailSize: number }>): number {
   ...
   */
  private getSeverityDistribution(events: Array<{ date: Date; hailSize: number }>): {
    severe: number;
    moderate: number;
    minor: number;
  } {
   ...
   */
  private calculateRecencyScore(events: Array<{ date: Date; hailSize: number }>): number {
    if (events.length === 0) return 0;
   ...
   */
  private calculateWeightedScore(factors: DamageScoreFactors): number {
    let score = 0;
    ...
    const severityScore =
      factors.severityDistribution.severe * 2.5 +
   ...
   */
  private generateSummary(score: number, factors: DamageScoreFactors): string {
    const riskLevel = this.getRiskLevel(score);
  ...
  private getScoreColor(score: number): string {
    if (score >= 76) return '#dc2626'; // Red (Critical)
    if (score >= 51) return '#f97316'; // Orange (High)
    if (score >= 26) return '#eab308'; // Yellow (Moderate)
