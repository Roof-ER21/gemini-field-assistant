...
export interface NotificationPreference {
  ...
  checkinProximityMiles?: number;
  allNotificationsEnabled: boolean;
}

export class CheckinNotificationService {
  private pushService: ReturnType<typeof createPushNotificationService>;
   ...
   */
  async notifyTeamOfCheckin(
    checkinData: CheckinNotificationData
    ...
    const usersToNotify = await this.getUsersToNotify(
      checkinData.userId,
      checkinData.checkInLat,
      checkinData.checkInLng
    );
    ...
    for (const user of usersToNotify) {
      try {
        ...
        // Send notification
        const notificationResults = await this.pushService.sendToUser(
          user.id,
          {
            title,
            body,
        ...
        console.log(
          `✅ Sent check-in notification to ${user.name} (${user.email})`
        );
      } catch (error) {
        console.error(
  ...
  private async getUsersToNotify(
    ...
    const usersToNotify: UserWithLocation[] = [];

    for (const row of result.rows) {
      // Check if user has notifications enabled
      if (!row.all_notifications_enabled || !row.checkin_alerts_enabled) {
        console.log(
          `⏭️  Skipping ${row.name} - notifications disabled`
        );
        continue;
      }
      ...
      if (row.checkin_proximity_miles) {
        ...
        if (!row.last_known_lat || !row.last_known_lng) {
          console.log(
            `⏭️  Skipping ${row.name} - no location data for proximity check`
          );
          continue;
        }
        ...
        if (distance > row.checkin_proximity_miles) {
          console.log(
            `⏭️  Skipping ${row.name} - outside proximity range (${distance.toFixed(1)} mi > ${row.checkin_proximity_miles} mi)`
          );
          continue;
        }
      }
   ...
   */
  async updateCheckinPreferences(
    userId: string,
    enabled: boolean,
    ...
    try {
      ...
      console.log(
        `✅ Updated check-in notification preferences for user ${userId}`
      );
      return true;
    } catch (error) {
   ...
   */
  async getCheckinPreferences(userId: string): Promise<{
    enabled: boolean;
    proximityMiles?: number;
  }> {
    try {
 ...
 */
export const createCheckinNotificationService = (
  pool: Pool
