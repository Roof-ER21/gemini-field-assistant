...
import {
  decodeMrmsGrib2,
  buildCompositeGrid,
  type CompositeDecodedMrmsGrib,
} from './historicalMrmsService.js';
import { meshGridToPolygons, type SwathPolygonCollection } from './meshVectorService.js';
...
interface CachedGrid {
  decoded: CompositeDecodedMrmsGrib;
  fetchedAt: number;
  sourceUrl: string;
  refTime: string;
}

let liveGridCache: CachedGrid | null = null;

function todayUtcParts(): { year: string; month: string; day: string; compact: string } {
  const now = new Date();
...
async function listTodaysFiles(): Promise<string[]> {
  ...
  const pattern = new RegExp(
    `href="(MESH_Max_1440min_00\\.50_${compact}-\\d{6}\\.grib2\\.gz)"`,
    'g',
  );
  const files: string[] = [];
  ...
  if (files.length === 0) {
    ...
    if (r2.ok) {
      ...
      const p2 = new RegExp(
        `href="(MESH_Max_1440min_00\\.50_${ycompact}-\\d{6}\\.grib2\\.gz)"`,
        'g',
      );
 ...
 */
export async function fetchLiveMrmsGrid(): Promise<CachedGrid> {
  ...
  for (const [url, ts] of recentlyFailed.entries()) {
    if (now - ts > STALE_URL_COOLDOWN_MS) recentlyFailed.delete(url);
  }
  ...
  for (let i = files.length - 1; i >= Math.max(0, files.length - 3); i--) {
    const url = files[i];
    if (recentlyFailed.has(url)) continue;
    try {
      ...
      lastErr = err;
      console.warn(`[LiveMRMS] failed ${url}: ${(err as Error).message}, trying older file`);
    }
  }

  throw new Error(
    `Live MRMS fetch failed after ${Math.min(3, files.length)} attempts: ${(lastErr as Error)?.message || 'unknown error'}`,
  );
}

export type LiveMrmsProduct = 'mesh60' | 'mesh1440';
...
export async function getLiveMrmsSwathPolygons(params: {
  ...
  east: number;
  west: number;
  product?: LiveMrmsProduct;
}): Promise<
  SwathPolygonCollection & { live: true; product: LiveMrmsProduct; refTime: string }
> {
  ...
  const rowStart = clamp(
    Math.floor((composite.north - params.north) / latStep) - padCells,
    0,
    composite.height - 1,
  );
  const rowEnd = clamp(
    Math.ceil((composite.north - params.south) / latStep) + padCells,
    0,
    composite.height - 1,
  );
  const colStart = clamp(
    Math.floor((params.west - composite.west) / lonStep) - padCells,
    0,
    composite.width - 1,
  );
  const colEnd = clamp(
    Math.ceil((params.east - composite.west) / lonStep) + padCells,
    0,
    composite.width - 1,
  );
 ...
 */
export async function getLiveConusGrid(): Promise<CompositeDecodedMrmsGrib> {
  const entry = await fetchLiveMrmsGrid();
