...
const liveGridCache = new Map<string, CachedGrid>();

export type LiveMrmsProduct = 'mesh60' | 'mesh1440';

function urlForProduct(product: LiveMrmsProduct): string {
...
export async function fetchLiveMrmsGrid(
  product: LiveMrmsProduct = 'mesh60',
): Promise<CachedGrid> {
...
export async function getLiveConusGrid(
  product: LiveMrmsProduct = 'mesh60',
): Promise<CompositeDecodedMrmsGrib> {
  const entry = await fetchLiveMrmsGrid(product);
