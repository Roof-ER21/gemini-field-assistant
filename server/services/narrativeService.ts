 ...
 */
function getHailSizeDesc(inches: number): string {
 ...
 */
function getDamagePotential(maxSize: number): string {
  if (maxSize >= 2.0) return 'significant damage to roofing materials, siding, gutters, and outdoor equipment';
  if (maxSize >= 1.5) return 'notable damage to asphalt shingles, window screens, and exposed surfaces';
  if (maxSize >= 1.0) return 'potential damage to roof surfaces, especially aged or compromised roofing materials';
  if (maxSize >= 0.75) return 'surface damage to roofing granules and cosmetic damage to exterior finishes';
