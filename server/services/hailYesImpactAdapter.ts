 ...
 */
import { getImpact, filterHits, byMostRecent, type HailYesHit } from './hailYesClient.js';
import type { AddressImpactReport, AddressImpactTier } from './addressImpactService.js';

function tierFromHit(hit: HailYesHit): AddressImpactTier {
  ...
  const s = hit.sources || ({} as any);
  if (s.ncei) sources.push('ncei');
  if (s.swdi) sources.push('swdi');
  if (s.iem) sources.push('iem');
  if (s.hailtrace) sources.push('hailtrace');
  if (s.ihm) sources.push('ihm');
  return {
    date: hit.event_date,
    maxHailInches,
    sizeLabel,
...
export async function getAddressHailImpactViaHailYes(
  lat: number,
  lng: number,
  ...
  const empty: AddressImpactReport = {
    lat,
    lng,
    monthsBack,
    directHits: [],
  ...
  for (const hit of sorted) {
    ...
    if (hit.impact_tier === 'direct_hit') directHits.push(tier);
    else if (hit.impact_tier === 'near_miss') nearMiss.push(tier);
    else areaImpact.push(tier);
  }
  console.log(
    `[HailYesAdapter] ${lat.toFixed(3)},${lng.toFixed(3)} → ${directHits.length} DH / ${nearMiss.length} NM / ${areaImpact.length} AI in ${Date.now() - start}ms`
  );
  return {
    lat,
    lng,
 ...
 */
export async function getCityHailViaHailYes(
  cityCentroidLat: number,
  cityCentroidLng: number,
  monthsBack: number = 24,
  onDates?: string[],
): Promise<HailYesHit[]> {
