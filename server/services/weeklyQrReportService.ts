...
export function priorWeekRangeET(now: Date = new Date()): WeekRange {
  ...
  const thisMon = new Date(et); thisMon.setHours(12, 0, 0, 0); thisMon.setDate(et.getDate() - sinceMon);
  ...
  const nice = (d: Date) => `${MONTHS[d.getMonth()]} ${d.getDate()}`;
...
function renderEmail(d: Awaited<ReturnType<typeof fetchWeeklyReportData>>, r: WeekRange): string {
  const conv = (n: number, den: number) => (den > 0 ? ((n / den) * 100).toFixed(1) + '%' : '—');
...
export async function sendWeeklyQrReport(
  ...
  for (const to of recipients) {
    ...
    if (res.success) anySuccess = true; else lastErr = res.error;
