...
export interface HistoricalMrmsRequest {
  date: string;
  north: number;
  south: number;
  east: number;
...
export interface HistoricalMrmsOverlayMetadata {
  product: 'mesh1440';
  ...
  archive_file: string;
  archive_url: string;
  /** All archive files composited into this overlay (1–3 files). */
  composite_files: string[];
  has_hail: boolean;
  max_mesh_mm: number;
...
export interface HistoricalMrmsOverlayResult {
  imageBuffer: Buffer;
  metadata: HistoricalMrmsOverlayMetadata;
}
...
export interface CompositeDecodedMrmsGrib {
  ...
  east: number;
  west: number;
  /** Pre-computed physical values in millimeters, one float per grid cell (row-major). */
  mmGrid: Float32Array;
  sourceFiles: string[];
}
...
const HAIL_COLORS: Array<{ minInches: number; maxInches: number; rgba: [number, number, number, number] }> =
  CANONICAL_HAIL_LEVELS.map((l) => ({ minInches: l.minInches, maxInches: l.maxInches, rgba: l.rgba }));

const cache = new Map<string, { expiresAt: number; result: HistoricalMrmsOverlayResult }>();
...
function normalizeRequest(input: HistoricalMrmsRequest): HistoricalMrmsRequest {
...
function getCacheKey(request: HistoricalMrmsRequest): string {
...
function buildArchiveDirectory(date: string): string {
...
function parseArchiveFiles(html: string, directoryUrl: string): ArchiveFile[] {
  ...
  return Array.from(files.values()).sort((a, b) =>
    a.compactTimestamp.localeCompare(b.compactTimestamp),
  );
}

async function listArchiveFiles(date: string): Promise<ArchiveFile[]> {
 ...
 */
function findFileNearTimestamp(files: ArchiveFile[], compactTarget: string): ArchiveFile {
 ...
 */
function chooseArchiveFiles(files: ArchiveFile[], anchorTimestamp?: string | null): ArchiveFile[] {
  if (files.length === 1) {
    return files;
  }
...
function readSectionLength(buffer: Buffer, offset: number): number {
  return buffer.readUInt32BE(offset);
}

function decodeGrayscale16Png(payload: Buffer): { width: number; height: number; data: Buffer } {
  ...
  if (bitDepth !== 16 || colorType !== 0 || interlace !== 0) {
    throw new Error(
      `Historical MRMS PNG uses unsupported format bitDepth=${bitDepth} colorType=${colorType} interlace=${interlace}`,
    );
  }
  ...
  if (inflated.length !== expectedLength) {
    throw new Error(
      `Historical MRMS PNG inflated length mismatch ${inflated.length} !== ${expectedLength}`,
    );
  }
...
export function decodeMrmsGrib2(raw: Buffer): DecodedMrmsGrib {
  ...
  if (png.width !== width || png.height !== height) {
    throw new Error(
      `Historical MRMS PNG dimensions ${png.width}x${png.height} do not match GRIB grid ${width}x${height}`,
    );
  }
...
function getColorForInches(inches: number): [number, number, number, number] | null {
...
function getValueMillimeters(decoded: DecodedMrmsGrib, rawValue: number): number {
  return (
    (decoded.refValue + rawValue * 2 ** decoded.binaryScale) /
...
export function buildCompositeGrid(grids: DecodedMrmsGrib[], sourceFiles: string[]): CompositeDecodedMrmsGrib {
  ...
  for (let g = 1; g < grids.length; g += 1) {
    ...
    if (grid.width !== base.width || grid.height !== base.height) {
      ...
      console.warn(
        `Historical MRMS composite: grid ${g} dimensions ${grid.width}x${grid.height} ` +
...
function gridBoundsToIndices(
  ...
  const rowStart = clamp(
    Math.floor((decoded.north - requestedBounds.north) / latStep) - padCells,
    0,
    decoded.height - 1,
  );
  const rowEnd = clamp(
    Math.ceil((decoded.north - requestedBounds.south) / latStep) + padCells,
    0,
    decoded.height - 1,
  );
  const colStart = clamp(
    Math.floor((requestedBounds.west - decoded.west) / lonStep) - padCells,
    0,
    decoded.width - 1,
  );
  const colEnd = clamp(
    Math.ceil((requestedBounds.east - decoded.west) / lonStep) + padCells,
    0,
    decoded.width - 1,
  );
...
async function renderOverlay(
  composite: CompositeDecodedMrmsGrib,
  requestedBounds: HistoricalMrmsRequest,
  primaryFile: ArchiveFile,
): Promise<HistoricalMrmsOverlayResult> {
  ...
  for (let row = rowStart; row <= rowEnd; row += 1) {
    for (let col = colStart; col <= colEnd; col += 1) {
      ...
      if (row < hailRowMin) hailRowMin = row;
      if (row > hailRowMax) hailRowMax = row;
      if (col < hailColMin) hailColMin = col;
      if (col > hailColMax) hailColMax = col;
    }
  }
...
async function downloadArchiveFile(file: ArchiveFile): Promise<Buffer> {
...
export async function getHistoricalMrmsOverlay(
  input: HistoricalMrmsRequest,
): Promise<HistoricalMrmsOverlayResult> {
...
export interface MrmsRecentHailDay {
  date: string;           // YYYY-MM-DD
  atLocation: number | null;
  within1mi: number | null;
  within3mi: number | null;
  within10mi: number | null;
  maxInches: number;      // largest non-null band
}
 ...
 */
export async function getRecentMrmsHailAtPoint(
  lat: number,
  lng: number,
  days: number = 3
): Promise<MrmsRecentHailDay[]> {
  ...
  const results = await Promise.all(
    dayStrings.map(async (date) => {
      try {
        const r = await getMrmsHailAtPoint(date, lat, lng);
        if (!r) return null;
        const values = [r.atLocation, r.within1mi, r.within3mi, r.within10mi].filter(
          (v): v is number => v !== null
        );
        if (values.length === 0) return null;
        const maxInches = Math.max(...values);
        return { date, ...r, maxInches } satisfies MrmsRecentHailDay;
      } catch {
        return null;
      }
    })
  ...
  return results.filter((d): d is MrmsRecentHailDay => d !== null);
}

// ─── MRMS Composite Grid Export (for vector polygon pipeline) ─────────────

import { meshGridToPolygons, type SwathPolygonCollection, type GridInput } from './meshVectorService.js';

/** Cache for pre-computed swath polygon GeoJSON (keyed by date + bounds). */
const swathPolyCache = new Map<string, { expiresAt: number; result: SwathPolygonCollection }>();

/** Round bounds to 2dp so near-identical viewport requests share the cache. */
function roundBounds(r: HistoricalMrmsRequest) {
  return {
 ...
 */
async function loadSwathFromDb(
  pool: Pool,
  date: string,
  bounds: ReturnType<typeof roundBounds>,
): Promise<SwathPolygonCollection | null> {
  try {
    const { rows } = await pool.query(
      `SELECT geojson FROM mrms_swath_cache
         WHERE storm_date = $1
           AND north = $2 AND south = $3 AND east = $4 AND west = $5
           AND anchor_timestamp = $6
           AND expires_at > NOW()
         LIMIT 1`,
      [date, bounds.north, bounds.south, bounds.east, bounds.west, bounds.anchorTimestamp],
    );
    if (rows.length === 0) return null;
    return rows[0].geojson as SwathPolygonCollection;
  } catch (err) {
    console.warn('[MRMS Vector] DB cache read failed:', (err as Error).message);
    return null;
  }
}
 ...
 */
async function saveSwathToDb(
  pool: Pool,
  date: string,
  bounds: ReturnType<typeof roundBounds>,
  result: SwathPolygonCollection,
): Promise<void> {
  try {
    await pool.query(
      `INSERT INTO mrms_swath_cache
    ...
    console.warn('[MRMS Vector] DB cache write failed:', (err as Error).message);
  }
}

/** Periodic cleanup — deletes expired swath cache rows. */
export async function cleanExpiredSwathCache(pool: Pool): Promise<number> {
  try {
    const { rowCount } = await pool.query(
      `DELETE FROM mrms_swath_cache WHERE expires_at <= NOW()`,
    );
    return rowCount ?? 0;
  } catch (err) {
    console.warn('[MRMS Vector] DB cache cleanup failed:', (err as Error).message);
    return 0;
  }
}
 ...
 */
export async function getHistoricalMrmsSwathPolygons(
  input: HistoricalMrmsRequest,
  pool?: Pool,
): Promise<SwathPolygonCollection> {
  const request = normalizeRequest(input);
  ...
  if (swathPolyCache.size > 50) {
    ...
    for (const [k, v] of swathPolyCache.entries()) {
      if (v.expiresAt <= now) swathPolyCache.delete(k);
    }
  }
