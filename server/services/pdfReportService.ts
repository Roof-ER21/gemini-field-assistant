...
export class PDFReportService {
   ...
   */
  private generateMetadata(input: ReportInput): ReportMetadata {
   ...
   * Add header section
   */
  private addHeader(doc: PDFKit.PDFDocument, input: ReportInput, metadata: ReportMetadata): void {
   ...
   */
  private addPropertyInformation(doc: PDFKit.PDFDocument, input: ReportInput): void {
   ...
   */
  private addDamageScoreSection(doc: PDFKit.PDFDocument, damageScore: DamageScoreResult): void {
   ...
   */
  private addExecutiveSummary(doc: PDFKit.PDFDocument, metadata: ReportMetadata, input: ReportInput): void {
   ...
   */
  private addStormTimeline(doc: PDFKit.PDFDocument, events: HailEvent[], noaaEvents: NOAAEvent[]): void {
   ...
   */
  private addEvidenceSection(doc: PDFKit.PDFDocument): void {
    // Check if we need a new page
   ...
   * Add footer to page
   */
  private addFooterToPage(doc: PDFKit.PDFDocument, input: ReportInput, pageNum: number): void {
    // Save current position
    ...
    if (input.repName || input.repPhone || input.repEmail) {
      const repInfo = [];
      if (input.repName) repInfo.push(input.repName);
      if (input.repPhone) repInfo.push(input.repPhone);
      if (input.repEmail) repInfo.push(input.repEmail);
    ...
    doc.fontSize(8)
       .fillColor(this.COLORS.textLight)
       .text(`Page ${pageNum}`,
         doc.page.width - this.MARGINS.right - 80,
         footerY + 5, {
         width: 80,
         align: 'right',
       });
    ...
    doc.fontSize(7)
       .fillColor(this.COLORS.textLight)
       .font('Helvetica-Oblique')
       .text('CONFIDENTIAL - For insurance and property assessment purposes only',
         this.MARGINS.left,
         footerY + 28, {
         width: doc.page.width - this.MARGINS.left - this.MARGINS.right,
         align: 'center',
   ...
   */
  private drawHorizontalLine(doc: PDFKit.PDFDocument, color: string, lineWidth: number): void {
    doc.save();
   ...
   */
  private drawInfoTable(doc: PDFKit.PDFDocument, data: Array<{ label: string; value: string }>): void {
   ...
   */
  private combineAndSortEvents(events: HailEvent[], noaaEvents: NOAAEvent[]): Array<any> {
    const combined = [
      ...events.map(e => ({
   ...
   */
  private calculateSeverity(eventType: string, magnitude: number | null): string {
    ...
    if (eventType === 'wind' && magnitude !== null && magnitude >= 60) return 'severe';
    return 'moderate';
  }
   ...
   */
  private formatEventRow(event: any): string[] {
