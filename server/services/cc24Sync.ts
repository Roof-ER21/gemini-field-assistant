/**
 * CC24 close-the-loop sync.
 *
 * sa21 forwards leads into CC24 and stores the returned jobId on profile_leads
 * (forwardLeadToCC24). This polls CC24's read-only /api/public/lead-status
 * endpoint for the current pipeline status of the OPEN forwarded jobs and caches
 * it on profile_leads.cc24_status — so the Scan Analytics funnel can reach
 * scan → signup → booked → signed → approved → paid (close-the-loop), instead of
 * stopping at "booked" (CC24 is the system of record for the outcome).
 *
 * Read-only against CC24; idempotent; throttled. Safe to call on every dashboard
 * load (it self-skips if it ran recently).
 */
...
export async function syncCc24Statuses(
  pool: pg.Pool,
  opts: { force?: boolean } = {},
): Promise<{ checked: number; updated: number; skipped?: boolean; reason?: string }> {
  ...
  if (!intakeUrl || !secret) return { checked: 0, updated: 0, skipped: true, reason: 'not-configured' };

  // Throttle to once / 15 min unless forced (so it's safe to call on dashboard load).
  const now = Date.now();
  if (!opts.force && now - lastSyncAt < 15 * 60 * 1000) return { checked: 0, updated: 0, skipped: true, reason: 'throttled' };
  lastSyncAt = now;
  ...
  const open = await pool.query<{ id: string; cc24_job_id: string; cc24_status: string | null }>(
    `SELECT id, cc24_job_id, cc24_status
       FROM profile_leads
  ...
  if (!open.rows.length) return { checked: 0, updated: 0 };
  ...
  for (let i = 0; i < ids.length; i += 400) {
    ...
    try {
      ...
      if (!r.ok) { console.error('[cc24 sync] non-OK', r.status); continue; }
      const j: any = await r.json();
      for (const job of j?.jobs || []) {
        if (job?.jobId && job?.status) statusById[String(job.jobId)] = String(job.status);
      }
    } catch (e) {
      console.error('[cc24 sync] fetch failed:', (e as Error)?.message);
    }
  }
  ...
  for (const row of open.rows) {
    ...
    if (st !== row.cc24_status) {
      await pool.query(
        `UPDATE profile_leads SET cc24_status = $1, cc24_status_at = NOW(), cc24_synced_at = NOW() WHERE id = $2`,
        [st, row.id],
      );
      updated++;
    } else {
...
export const CC24_STAGE_RANK: Record<string, number> = {
