...
/** A single message in the conversation (mirrors the frontend AIMessage type) */
interface AIMessage {
...
const getEnvKey = (key: string): string | undefined =>
 ...
 */
async function resolveUserId(pool: pg.Pool, email: string): Promise<string | null> {
  ...
  try {
    ...
    const adminEmail = normalizeEmail(
      process.env.EMAIL_ADMIN_ADDRESS || process.env.ADMIN_EMAIL
    );
 ...
 */
async function resolvePersonality(pool: pg.Pool, userId: string): Promise<Record<string, string>> {
  try {
 ...
 */
async function resolveDirectives(pool: pg.Pool): Promise<Array<{ title: string; content: string; priority: string }>> {
  try {
    const result = await pool.query(
      `SELECT title, content, priority FROM manager_directives
 ...
 */
async function resolveUserState(pool: pg.Pool, userId: string): Promise<string> {
  try {
    const result = await pool.query(
      `SELECT value
       FROM user_memory
 ...
 */
async function resolveRepMemories(
  pool: pg.Pool,
  userId: string
 ...
 */
async function resolveUserName(pool: pg.Pool, userId: string, email: string): Promise<string> {
  try {
    const result = await pool.query(
 ...
 */
function buildContents(messages: AIMessage[]): Content[] {
...
export function createSusanAgentRoutes(pool: pg.Pool): Router {
  ...
  let genAI: GoogleGenAI | null = geminiKey ? new GoogleGenAI({ apiKey: geminiKey }) : null;
  ...
  router.post('/chat', async (req: Request, res: Response) => {
          ...
          if (tokens.length > 0) {
            ...
            try {
              ...
              if (kbHits.rows.length > 0) {
                const kbLines = kbHits.rows.map((r: any) => {
    ...
    try {
      for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
        console.log(
          `[SusanAgent:${requestId}] Gemini call round ${round + 1}/${MAX_TOOL_ROUNDS}, contents length=${contents.length}`
        );
        ...
        if (functionCallParts.length === 0) {
          ...
          if (!responseText) {
            ...
            console.warn(
              `[SusanAgent:${requestId}] Empty text response. finishReason=${finishReason}`
            );
          ...
          console.log(
            `[SusanAgent:${requestId}] Final text response (round ${round + 1}), ` +
        ...
        console.log(
          `[SusanAgent:${requestId}] Round ${round + 1}: executing ${functionCallParts.length} tool(s)`
        );
      ...
      console.warn(
        `[SusanAgent:${requestId}] Reached max tool rounds (${MAX_TOOL_ROUNDS}). Forcing final response.`
      );
  ...
  router.post('/summarize', async (req: Request, res: Response) => {
    try {
      ...
      console.error('[SusanAgent] Summarize error:', (err as Error).message);
  ...
  router.get('/context', async (req: Request, res: Response) => {
    try {
      ...
      const userId = await resolveUserId(pool, email);
      if (!userId) return res.json({ context: '' });
      ...
      console.error('[SusanAgent] Context load error:', (err as Error).message);
      res.json({ context: '' });
    }
  });
