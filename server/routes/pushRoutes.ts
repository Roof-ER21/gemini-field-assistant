...
router.put('/preferences', async (req: Request, res: Response) => {
  try {
    ...
    // Upsert preferences
    const result = await pool.query(
      `INSERT INTO notification_preferences (user_id, ${Object.values(fieldMap).filter((_, i) => i < updateFields.length).join(', ')})
...
router.get('/stats', async (req: Request, res: Response) => {
  try {
    ...
    const stats = await service.getNotificationStats(parseInt(daysBack as string));
