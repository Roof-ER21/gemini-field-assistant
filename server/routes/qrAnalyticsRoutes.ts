...
export function createQRAnalyticsRoutes(pool: Pool) {
  ...
  router.get('/dashboard', async (req: Request, res: Response) => {
    try {
      ...
      const slug = (req.query.slug as string)?.trim() || null;
      const fromQ = (req.query.from as string)?.trim() || null;
      const toQ = (req.query.to as string)?.trim() || null;
      ...
      const bd = await pool.query(
        `SELECT to_char(COALESCE($1::date, (now() AT TIME ZONE '${ET}')::date - 29), 'YYYY-MM-DD') AS from_d,
                to_char(COALESCE($2::date, (now() AT TIME ZONE '${ET}')::date),       'YYYY-MM-DD') AS to_d`,
        [fromQ, toQ]
      );
      let fromD: string = bd.rows[0].from_d;
      let toD: string = bd.rows[0].to_d;
      if (fromD > toD) { const t = fromD; fromD = toD; toD = t; } // tolerate reversed range
      ...
      res.json({
        ...
        funnel: (() => {
          ...
          for (const r of (cc24R.rows as Array<{ status: string; n: number }>)) {
            const st = String(r.status || '').toLowerCase();
            if (st === 'lost' || st === 'cancelled') { lost += r.n; continue; }
            const rank = CC24_STAGE_RANK[st] || 0;
            if (rank >= 7) signed += r.n;
            if (rank >= 12) approved += r.n;
            if (rank >= 16) won += r.n;
          }
  ...
  router.post('/sync-cc24', async (req: Request, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
      if (!await canManageQRUser(userEmail)) {
        return res.status(403).json({ success: false, error: 'Admin access required' });
   ...
   */
  router.get('/card-register', async (req: Request, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
      if (!await canManageQRUser(userEmail)) {
        return res.status(403).json({ success: false, error: 'Admin access required' });
   ...
   */
  router.post('/card-register/:slug', async (req: Request, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
      if (!await canManageQRUser(userEmail)) {
        return res.status(403).json({ success: false, error: 'Admin access required' });
      ...
      const result = await pool.query(
        `UPDATE employee_profiles
            SET card_issued_at = $2,
                ...
                updated_at     = NOW()
          WHERE slug = $1
        RETURNING id, name, slug, card_issued_at, card_batch, card_notes`,
        [slug, when, batch || null, defaultBatch, notes || null],
      );
