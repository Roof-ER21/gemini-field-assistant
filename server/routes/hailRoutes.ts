...
router.post('/generate-report', async (req: Request, res: Response) => {
  try {
    ...
    if (req.body?.sync !== true) {
      ...
      const enrichedPayload: any = { ...(req.body || {}) };
      ...
      if (Number.isFinite(latNum) && Number.isFinite(lngNum)
        ...
        try {
          ...
          enrichedPayload.swathDirectHits = (impact?.directHits || []).map((d: any) => ({
            date: String(d.date),
      ...
      const { rows } = await pool.query(
        ...
        [JSON.stringify(enrichedPayload), requestedBy],
      );
...
async function renderClaimPacketToResponse(
  ...
  let claimPacketSwathDirectHits: { date: string; hailInches: number }[] = [];
  try {
    const { getAddressHailImpact } = await import('../services/addressImpactService.js');
    const impact = await getAddressHailImpact(pool, lat, lng, 60, { skipColdFetch: true });
    claimPacketSwathDirectHits = (impact?.directHits || []).map((d: any) => ({
      date: String(d.date),
      hailInches: Number(d.maxHailInches || 0.5),
    }));
  } catch (e) {
    console.warn('[claim-packet] swath-first impact check failed:', (e as Error).message);
  }
...
router.post('/admin/nws-alerts-poll', async (req: Request, res: Response) => {
  try {
...
router.post('/admin/mping-backfill', async (req: Request, res: Response) => {
  try {
...
router.post('/admin/nexrad-l2-ingest', async (req: Request, res: Response) => {
...
router.get('/admin/coverage-summary', async (req: Request, res: Response) => {
  try {
    const pool = req.app.get('pool');
    const { rows } = await pool.query(`
      SELECT
...
router.post('/admin/live-nws-warning-check', async (req: Request, res: Response) => {
  try {
...
router.post('/admin/live-mrms-alert-check', async (req: Request, res: Response) => {
  try {
...
router.post('/admin/materialize-ihm-dates', async (req: Request, res: Response) => {
  try {
...
let _ihmSummaryCache: { at: number; body: any } | null = null;
const IHM_SUMMARY_TTL_MS = 15 * 60 * 1000;

router.get('/ihm-coverage-summary', async (req: Request, res: Response) => {
  try {
    const now = Date.now();
...
router.get('/address-impact', async (req: Request, res: Response) => {
  try {
    ...
    const { getAddressHailImpact } = await import('../services/addressImpactService.js');
    const report = await getAddressHailImpact(
      req.app.get('pool'),
      lat,
      lng,
      months,
      { skipColdFetch: !allowColdFetch },
    );
