...
export function createRoofRoutes(pool: pg.Pool) {
  // ============================================================================
  // POSTS
  // ============================================================================
  ...
  router.post('/posts', async (req: AuthRequest, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
      const { content, post_type, shared_content } = req.body as {
        content: string;
        post_type?: string;
      ...
      const postResult = await pool.query(
        `INSERT INTO team_posts (author_id, content, post_type, shared_content)
         VALUES ($1, $2, $3, $4)
         RETURNING *`,
        [userId, content.trim(), post_type || 'text', shared_content ? JSON.stringify(shared_content) : null]
      );
      ...
      if (mentionedUsernames.length > 0) {
        ...
        const mentionedUsers = await pool.query(
          `SELECT id FROM users WHERE LOWER(SPLIT_PART(email, '@', 1)) = ANY($1)`,
          [mentionedUsernames]
        );
  ...
  router.delete('/posts/:id', async (req: AuthRequest, res: Response) => {
    try {
      ...
      const deleteResult = await pool.query(
        'DELETE FROM team_posts WHERE id = $1 AND author_id = $2 RETURNING id',
        [id, userId]
      );
  ...
  router.post('/posts/:id/like', async (req: AuthRequest, res: Response) => {
    try {
      ...
      const countResult = await pool.query(
        'SELECT like_count FROM team_posts WHERE id = $1',
        [id]
      );
   ...
   * Unlike a post
   */
  router.delete('/posts/:id/like', async (req: AuthRequest, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
      const { id } = req.params;
  ...
  router.post('/posts/:id/comments', async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (mentionedUsernames.length > 0) {
        ...
        for (const user of mentionedUsers.rows) {
          await pool.query(
            `INSERT INTO post_mentions (comment_id, mentioned_user_id)
             VALUES ($1, $2)
             ON CONFLICT DO NOTHING`,
            [comment.id, user.id]
          );
   ...
   * Like a comment
   */
  router.post('/comments/:id/like', async (req: AuthRequest, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
      const { id } = req.params;
   ...
   * Unlike a comment
   */
  router.delete('/comments/:id/like', async (req: AuthRequest, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
      const { id } = req.params;
   ...
   */
  router.get('/mentions', async (req: AuthRequest, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
   ...
   */
  router.post('/mentions/mark-read', async (req: AuthRequest, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
      const { mention_ids } = req.body as { mention_ids?: string[] };
      ...
      if (mention_ids && mention_ids.length > 0) {
        ...
        await pool.query(
          `UPDATE post_mentions
           SET is_read = TRUE
           WHERE id = ANY($1) AND mentioned_user_id = $2`,
          [mention_ids, userId]
        );
      } else {
