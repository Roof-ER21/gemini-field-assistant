...
function getMessagePreview(content: MessageContent, maxLength = 100): string {
  if (content.text) {
...
export function createMessagingRoutes(pool: pg.Pool) {
  ...
  router.post('/conversations', async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      if (initial_message && initial_message.content) {
        await client.query(
          `INSERT INTO team_messages (conversation_id, sender_id, message_type, content)
           VALUES ($1, $2, $3, $4)`,
          [
            conversationId,
            userId,
  ...
  router.get('/conversations/:id/messages', async (req: AuthRequest, res: Response) => {
    ...
    const beforeId = req.query.before_message_id as string;
  ...
  router.post('/conversations/:id/messages', async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const messageResult = await client.query(
        `INSERT INTO team_messages (conversation_id, sender_id, message_type, content, parent_message_id)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING *`,
        [conversationId, userId, message_type, JSON.stringify(content), parent_message_id || null]
      );
      ...
      if (message_type === 'shared_chat' || message_type === 'shared_email') {
        ...
        await client.query(
          `INSERT INTO shared_ai_content (message_id, content_type, original_session_id, shared_by)
           VALUES ($1, $2, $3, $4)`,
          [message.id, contentType, sessionId || null, userId]
        );
      }
      ...
      if (pushService) {
        ...
        if (conversationInfo.rows[0].type === 'direct') {
          ...
          const dmRecipient = await pool.query(
            `SELECT user_id FROM conversation_participants
          ...
          if (dmRecipient.rows.length > 0) {
            pushService.sendToUser(
              ...
              'team_message'
            ).catch((err: any) => console.error('Push error (DM):', err.message));
          }
        } else {
          ...
          for (const p of groupParticipants.rows) {
            pushService.sendToUser(
              ...
              'team_message'
            ).catch((err: any) => console.error('Push error (group):', err.message));
          }
        }
        ...
        if (content.mentioned_users && Array.isArray(content.mentioned_users)) {
          for (const mentionedUserId of content.mentioned_users) {
            ...
            pushService.sendMentionNotification(
              mentionedUserId,
              senderName,
              typeof content.text === 'string' ? content.text : preview,
              message.id
            ).catch((err: any) => console.error('Push error (mention):', err.message));
          }
        }
      }
   ...
   */
  router.post('/attachments', async (req: AuthRequest, res: Response) => {
    const userId = req.userId;
  ...
  router.get('/search', async (req: AuthRequest, res: Response) => {
    ...
    try {
      const params: any[] = [userId, `%${query.toLowerCase()}%`];
      let sql = `
        SELECT
          m.id,
          m.conversation_id,
   ...
   */
  router.get('/conversations/:id/pins', async (req: AuthRequest, res: Response) => {
    const userId = req.userId;
    const conversationId = req.params.id;
   ...
   */
  router.post('/conversations/:id/pins/:messageId', async (req: AuthRequest, res: Response) => {
    const userId = req.userId;
    const conversationId = req.params.id;
    ...
    try {
      ...
      const existingPin = await client.query(
        `SELECT id FROM message_pins WHERE conversation_id = $1 AND message_id = $2`,
   ...
   */
  router.post('/polls/:messageId/vote', async (req: AuthRequest, res: Response) => {
    const userId = req.userId;
    const { messageId } = req.params;
    ...
    try {
      ...
      await client.query(
        `INSERT INTO message_poll_votes (message_id, user_id, option_index)
         VALUES ($1, $2, $3)
         ON CONFLICT (message_id, user_id) DO UPDATE SET option_index = $3, created_at = NOW()`,
        [messageId, userId, option_index]
      );
   ...
   */
  router.post('/events/:messageId/rsvp', async (req: AuthRequest, res: Response) => {
    const userId = req.userId;
    const { messageId } = req.params;
    ...
    try {
      ...
      await client.query(
        `INSERT INTO message_event_rsvps (message_id, user_id, status)
         VALUES ($1, $2, $3)
         ON CONFLICT (message_id, user_id) DO UPDATE SET status = $3, created_at = NOW()`,
    ...
    try {
      ...
      if (conversation_id) {
        ...
        await client.query(
          `UPDATE message_mentions
           SET is_read = true
           WHERE mentioned_user_id = $1 AND message_id = ANY($2::uuid[])`,
          [userId, message_ids]
        );
      } else {
        await client.query('ROLLBACK');
  ...
  router.get('/notifications', async (req: AuthRequest, res: Response) => {
    ...
    try {
      ...
      const countResult = await pool.query(
        'SELECT COUNT(*)::integer as unread_count FROM team_notifications WHERE user_id = $1 AND is_read = false',
        [userId]
      );
