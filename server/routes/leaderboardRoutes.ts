...
function getNextSyncTime(timeZone: string): { iso: string; local: string } {
  const now = new Date();
  const formatter = new Intl.DateTimeFormat('en-US', {
    timeZone,
    hour12: false,
  ...
  const targetDate = makeDateInTimeZone(
  ...
  const localLabel = `${baseDate.getUTCFullYear()}-${String(baseDate.getUTCMonth() + 1).padStart(2, '0')}-${String(
    baseDate.getUTCDate()
  ).padStart(2, '0')} ${String(targetHour).padStart(2, '0')}:00 ${timeZone}`;
...
export function createLeaderboardRoutes(pool: Pool) {
  const router = Router();
  ...
  router.get('/stats', async (_req: Request, res: Response) => {
    try {
      const sortBy = (_req.query.sortBy as string) || 'monthly_signups';
  ...
  router.get('/sync-status', async (_req: Request, res: Response) => {
    try {
      const userEmail = _req.headers['x-user-email'] as string;
      if (!userEmail || !(await isAdminUser(userEmail))) {
        return res.status(403).json({
          success: false,
          error: 'Admin access required'
