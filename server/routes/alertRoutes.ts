...
import { Pool } from 'pg';
import { twilioService, StormAlert } from '../services/twilioService.js';
 ...
 * }
 */
router.post('/test-sms', requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 * }
 */
router.post('/storm-notification', async (req: Request, res: Response) => {
  try {
    const userEmail = req.headers['x-user-email'] as string;
    if (!userEmail) {
      return res.status(401).json({ error: 'No user email provided' });
 ...
 * }
 */
router.post('/batch-storm-notifications', requireAdmin, async (req: Request, res: Response) => {
  try {
 ...
 */
router.get('/sms-stats', async (req: Request, res: Response) => {
  try {
    const userEmail = req.headers['x-user-email'] as string;
    if (!userEmail) {
      return res.status(401).json({ error: 'No user email provided' });
 ...
 */
router.get('/recent-notifications', requireAdmin, async (req: Request, res: Response) => {
  try {
    const pool: Pool = req.app.get('pool');
    const limit = parseInt(req.query.limit as string) || 50;

    const result = await pool.query(
      `SELECT * FROM recent_sms_notifications
       LIMIT $1`,
      [limit]
    );
