...
router.post('/checkout', async (req: Request, res: Response) => {
  try {
    ...
    const session = await checkinService.endCheckin(
      activeSession.id,
      userId,
      {
      ...
      activeSession.checkInLat || 0,
      activeSession.checkInLng || 0
    );
...
router.put('/:id/notes', async (req: Request, res: Response) => {
  try {
    ...
    // Update notes
    const result = await pool.query(
      'UPDATE territory_checkins SET notes = $1 WHERE id = $2 RETURNING *',
      [notes || null, id]
    );
