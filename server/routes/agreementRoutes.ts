...
interface AgreementData {
  agreementType: 'claim_authorization' | 'contingency';
...
router.post('/', async (req: Request, res: Response) => {
  try {
    ...
    const data: AgreementData = req.body;
    ...
    await pool.query(
      `SELECT log_agreement_action($1, 'signed', 'customer', $2, $3, $4, $5)`,
      [
        agreementId,
        userId,
        ipAddress,
...
router.get('/:id', async (req: Request, res: Response) => {
  try {
    ...
    const userAgent = req.headers['user-agent'] || null;
    await pool.query(
      `SELECT log_agreement_action($1, 'viewed', 'user', NULL, $2, $3, NULL)`,
...
router.get('/', async (req: Request, res: Response) => {
  try {
    ...
    if (presentationId) {
      ...
      params.push(presentationId as string);
    }
    if (inspectionId) {
      query += ` AND a.inspection_id = $${paramIndex++}`;
      params.push(inspectionId as string);
    }
    if (jobId) {
...
router.post('/:id/void', async (req: Request, res: Response) => {
  try {
    ...
    // Void the agreement
    await pool.query(
      `UPDATE agreements SET status = 'voided', updated_at = NOW() WHERE id = $1`,
      [id]
    );
    ...
    const userAgent = req.headers['user-agent'] || null;
    await pool.query(
      `SELECT log_agreement_action($1, 'voided', 'user', $2, $3, $4, $5)`,
      [id, userId, ipAddress, userAgent, JSON.stringify({ reason })]
    );
...
router.get('/presentation/:presentationId', async (req: Request, res: Response) => {
  try {
