...
export function repReportRoutes(pool: Pool): Router {
  const router = Router();
  ...
  // ─── REP SUBMISSION ───
  router.post('/rep-report', async (req: Request, res: Response) => {
    if (!enabled()) return res.status(503).json({ error: 'rep reports disabled' });

    try {
      const {
        latitude, longitude, event_date,
        hail_size_inches, wind_mph, tornado_ef_rank,
        photo_urls, notes, state,
      } = req.body;
  ...
  router.post('/customer-report', async (req: Request, res: Response) => {
    if (!enabled()) return res.status(503).json({ error: 'customer reports disabled' });
    ...
    try {
      const {
        ...
        photo_urls, notes, state,
        email, phone,                          // customer contact info (stored in source_details)
      } = req.body;
  ...
  // ─── ADMIN QUEUE ───
  router.get('/rep-report/pending', async (req: Request, res: Response) => {
    if (!enabled()) return res.status(503).json({ error: 'rep reports disabled' });
  ...
  router.post('/rep-report/:id/approve', async (req: Request, res: Response) => {
    if (!enabled()) return res.status(503).json({ error: 'rep reports disabled' });
  ...
  router.post('/rep-report/:id/reject', async (req: Request, res: Response) => {
    if (!enabled()) return res.status(503).json({ error: 'rep reports disabled' });
