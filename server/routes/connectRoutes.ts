...
export function signState(userId: string, now = Date.now()): string | null {
  const key = stateKey();
  if (!key) return null;
  const payload = Buffer.from(
    JSON.stringify({ u: userId, n: crypto.randomBytes(12).toString('base64url'), e: now + STATE_TTL_MS }),
  ).toString('base64url');
  const sig = crypto.createHmac('sha256', key).update(payload).digest('base64url');
  return `${payload}.${sig}`;
}

export function verifyState(state: unknown, now = Date.now()): { userId: string | null; error?: string } {
  const key = stateKey();
 ...
 */
export function buildConnectStartUrl(req: Request, userId: string): string | null {
  if (!encryptionConfigured() || !appSecret()) return null;
...
export function createConnectRoutes(pool: pg.Pool): Router {
  ...
  // ---- GET /roofhr/start ----
  router.get('/roofhr/start', requireVerifiedSession, (req: VerifiedRequest, res: Response) => {
    ...
    if (!url) return res.status(503).json({ error: 'Roof HR connections are not switched on yet.' });
  ...
  router.get('/roofhr/callback', (req: Request, res: Response) => {
  ...
  router.post('/roofhr/complete', requireVerifiedSession, async (req: VerifiedRequest, res: Response) => {
    ...
    try {
      ...
      exchanged = (await response.json().catch(() => ({}))) as typeof exchanged;
      if (!response.ok) {
      ...
      console.error('[connect] exchange failed:', (err as Error).message);
  ...
  // ---- GET /roofhr/status ----
  router.get('/roofhr/status', requireVerifiedSession, async (req: VerifiedRequest, res: Response) => {
  ...
  // ---- DELETE /roofhr ----
  router.delete('/roofhr', requireVerifiedSession, async (req: VerifiedRequest, res: Response) => {
