...
import {
  ...
  encryptionKey,
  saveConnection,
} from '../services/roofhrConnection.js';
 ...
 */
export function callbackUrl(req: Request, app: ConnectedApp): { url: string | null; error?: string } {
  ...
  for (const origin of origins) {
    ...
    if (registered.includes(url)) return { url };
  }
  return {
    url: null,
    error:
      `This server's return address (${origins[0]}/api/connect/${app.slug}/callback) is not registered ` +
...
export function signState(userId: string, appSlug: string, now = Date.now()): string | null {
  const key = stateKey();
  if (!key) return null;
  const payload = Buffer.from(
    JSON.stringify({ u: userId, a: appSlug, n: crypto.randomBytes(12).toString('base64url'), e: now + STATE_TTL_MS }),
  ).toString('base64url');
  const sig = crypto.createHmac('sha256', key).update(payload).digest('base64url');
  return `${payload}.${sig}`;
}

export function verifyState(state: unknown, now = Date.now()): { userId: string | null; appSlug?: string; error?: string } {
  const key = stateKey();
  if (!key) return { userId: null, error: 'Roof HR connections are not configured.' };
  const raw = String(state ?? '');
  const dot = raw.lastIndexOf('.');
  if (dot <= 0) return { userId: null, error: 'That connection request is not valid.' };
  ...
  let parsed: { u?: unknown; a?: unknown; e?: unknown };
  try {
 ...
 */
export function buildConnectStartUrl(req: Request, userId: string, app: ConnectedApp): string | null {
  if (!encryptionConfigured() || !appSecret(app)) return null;
  const cb = callbackUrl(req, app);
  if (!cb.url) return null;
...
type VerifiedRequest = SessionRequest & { connectUserId?: string };
 ...
 */
function requireVerifiedSession(req: VerifiedRequest, res: Response, next: express.NextFunction): void {
...
export function createConnectRoutes(pool: pg.Pool): Router {
   ...
   */
  function peerOr404(req: Request, res: Response): ConnectedApp | null {
  ...
  router.get('/:app/start', requireVerifiedSession, (req: VerifiedRequest, res: Response) => {
    ...
    const cb = callbackUrl(req, app);
    if (!cb.url) return res.status(500).json({ error: cb.error });

    const url = buildConnectStartUrl(req, req.connectUserId!, app);
    if (!url) return res.status(503).json({ error: `${app.displayName} connections are not switched on yet.` });
  ...
  router.get('/:app/callback', (req: Request, res: Response) => {
    const app = peerOr404(req, res);
    if (!app) return;
  ...
  router.post('/:app/complete', requireVerifiedSession, async (req: VerifiedRequest, res: Response) => {
    ...
    if (!verified.userId) return res.status(400).json({ error: verified.error });
    ...
    if (verified.userId !== req.connectUserId) {
      console.warn(
        `[connect] state/session mismatch — state=${verified.userId} session=${req.connectUserId}; refusing`,
      );
    ...
    if (!code) return res.status(400).json({ error: 'That connection is missing its code. Start again.' });
    ...
    try {
      ...
      exchanged = (await response.json().catch(() => ({}))) as Record<string, unknown>;
      if (!response.ok) {
      ...
      console.error(`[connect] ${app.slug} exchange failed:`, (err as Error).message);
    ...
    const saved = await saveConnection(pool, {
      ...
      remoteUserId,
      token,
      scopes: Array.isArray(exchanged.scopes) ? (exchanged.scopes as string[]) : [],
      endpoint,
      expiresAt,
    });
  ...
  router.get('/status', requireVerifiedSession, async (req: VerifiedRequest, res: Response) => {
  ...
  // ---- DELETE /:app ----
  router.delete('/:app', requireVerifiedSession, async (req: VerifiedRequest, res: Response) => {
    const app = peerOr404(req, res);
    if (!app) return;
