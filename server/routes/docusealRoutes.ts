...
export function createDocuSealRoutes(dbPool: Pool) {
  const router = Router();
  const pool = dbPool;
  ...
  router.post('/submissions', async (req: Request, res: Response) => {
    try {
      ...
      if (customerName) fields.push({ name: 'Customer Name', default_value: customerName });
      if (customerAddress) fields.push({ name: 'Property Address', default_value: customerAddress });
      if (insuranceCompany) fields.push({ name: 'Insurance Company', default_value: insuranceCompany });
      if (claimNumber) fields.push({ name: 'Claim Number', default_value: claimNumber });
      if (agentName) fields.push({ name: 'Agent Name', default_value: agentName });
      ...
      if (agreementId) {
        await pool.query(
          `UPDATE agreements SET
            ...
            updated_at = NOW()
          WHERE id = $3`,
          [submission.id, submission.submitters?.[0]?.slug || null, agreementId]
        );
      }
    ...
    try {
      ...
      if (event.eventType === 'form.completed' || event.status === 'completed') {
        ...
        if (result.rows.length > 0) {
          ...
          if (doc) {
            await pool.query(
              'UPDATE agreements SET signed_pdf_url = $1 WHERE id = $2',
              [doc.url, agreementId]
            );
          }
          ...
          await pool.query(
            `INSERT INTO agreement_logs (agreement_id, action, actor_type, details)
             VALUES ($1, 'signed', 'customer', $2)`,
            [agreementId, JSON.stringify({ source: 'docuseal', submissionId: event.submissionId })]
          );
   ...
   */
  router.get('/submissions/:id/document', async (req: Request, res: Response) => {
    try {
      if (!docusealService.isConfigured()) {
        return res.status(503).json({ error: 'DocuSeal not configured' });
      }
