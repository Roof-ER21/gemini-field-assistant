...
export function createAgentNetworkRoutes(pool: pg.Pool): Router {
  const router = Router();
    ...
    try {
      ...
      const result = await pool.query(
        `INSERT INTO agent_network_messages (author_user_id, intel_type, content, state, insurer)
         VALUES ($1, $2, $3, $4, $5)
         RETURNING *`,
        [userId, intel_type, content, state || null, insurer || null]
      );
  ...
  router.put('/:id/reject', async (req: Request, res: Response) => {
    try {
      ...
      const result = await pool.query(
        `UPDATE agent_network_messages SET status = 'rejected' WHERE id = $1 RETURNING *`,
        [req.params.id]
      );
  ...
  router.post('/:id/vote', async (req: Request, res: Response) => {
    try {
      ...
      const existing = await pool.query(
        'SELECT id, vote_type FROM agent_network_votes WHERE message_id = $1 AND user_id = $2',
        [messageId, userId]
      );
      ...
      if (existing.rows.length > 0) {
        ...
        if (oldVote === vote_type) {
          ...
          await pool.query(
            `UPDATE agent_network_messages SET ${col} = GREATEST(${col} - 1, 0) WHERE id = $1`,
            [messageId]
          );
          ...
          await pool.query(
            'UPDATE agent_network_votes SET vote_type = $1 WHERE id = $2',
            [vote_type, existing.rows[0].id]
          );
          const addCol = vote_type === 'up' ? 'upvotes' : 'downvotes';
          const subCol = vote_type === 'up' ? 'downvotes' : 'upvotes';
          await pool.query(
            `UPDATE agent_network_messages SET ${addCol} = ${addCol} + 1, ${subCol} = GREATEST(${subCol} - 1, 0) WHERE id = $1`,
            [messageId]
          );
      ...
      // New vote
      await pool.query(
        'INSERT INTO agent_network_votes (message_id, user_id, vote_type) VALUES ($1, $2, $3)',
        [messageId, userId, vote_type]
      );
      const col = vote_type === 'up' ? 'upvotes' : 'downvotes';
      await pool.query(
        `UPDATE agent_network_messages SET ${col} = ${col} + 1 WHERE id = $1`,
        [messageId]
      );
  ...
  router.post('/:id/promote', async (req: Request, res: Response) => {
    try {
      ...
      const intel = await pool.query(
        'SELECT * FROM agent_network_messages WHERE id = $1',
        [req.params.id]
      );
      ...
      const learning = await pool.query(
        `INSERT INTO global_learnings (title, content, category, state, submitted_by, status, approved_by, approved_at)
      ...
      // Link back
      await pool.query(
        'UPDATE agent_network_messages SET promoted_to_global_learning_id = $1 WHERE id = $2',
        [learning.rows[0].id, req.params.id]
      );
