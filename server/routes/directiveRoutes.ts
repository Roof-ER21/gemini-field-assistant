...
export function createDirectiveRoutes(pool: pg.Pool): Router {
  ...
  router.post('/', async (req: Request, res: Response) => {
    ...
    try {
      const body = req.body as DirectiveBody;
  ...
  router.put('/:id', async (req: Request, res: Response) => {
    ...
    try {
      const { id } = req.params;
      const body = req.body as Partial<DirectiveBody> & { is_active?: boolean };
  ...
  router.delete('/:id', async (req: Request, res: Response) => {
    ...
    try {
      const result = await pool.query(
        'DELETE FROM manager_directives WHERE id = $1 RETURNING id',
        [req.params.id]
      );
