  ...
  router.post('/api/livekit/sessions', async (req: Request, res: Response) => {
    try {
      ...
      const existing = await pool.query(
        `SELECT id FROM live_sessions WHERE host_user_id = $1 AND status = 'active'`,
        [userId]
      );
      ...
      const result = await pool.query(
        `INSERT INTO live_sessions (room_name, title, host_user_id, status, participant_count)
         VALUES ($1, $2, $3, 'active', 1)
         RETURNING *`,
        [roomName, sessionTitle, userId]
      );
  ...
  router.post('/api/livekit/sessions/:id/join', async (req: Request, res: Response) => {
    try {
      ...
      const session = await pool.query(
        `SELECT * FROM live_sessions WHERE id = $1 AND status = 'active'`,
        [id]
      );
