...
export function createProfileLeadsRoutes(pool: Pool) {
  const router = Router();
  ...
  router.get('/', async (req: Request, res: Response) => {
    try {
      ...
      params.push(Math.min(parseInt(limit as string), 100), parseInt(offset as string));
  ...
  router.put('/:id', async (req: Request, res: Response) => {
    try {
      ...
      const accessCheck = await pool.query(
        'SELECT profile_id FROM profile_leads WHERE id = $1',
        [id]
      );
  ...
  router.delete('/:id', async (req: Request, res: Response) => {
    try {
      ...
      const result = await pool.query(
        'DELETE FROM profile_leads WHERE id = $1 RETURNING id',
        [id]
      );
