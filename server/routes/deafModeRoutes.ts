...
const getPool = (req: Request): Pool => req.app.get('pool');
...
router.post('/sessions', async (req: Request, res: Response) => {
  try {
    ...
    if (!(await requireTable(pool, 'deaf_mode_sessions', res))) return;
    ...
    const result = await pool.query(
      `INSERT INTO deaf_mode_sessions
       ...
       RETURNING id, rep_user_id, lead_id, started_at`,
      [rep_user_id, lead_id]
    );
...
router.post('/transcript', async (req: Request, res: Response) => {
  try {
    ...
    if (!(await requireTable(pool, 'deaf_mode_transcript', res))) return;
    ...
    const result = await pool.query(
      `INSERT INTO deaf_mode_transcript
         (session_id, timestamp, speaker, input_method,
          content, sign_confidence, recognition_source)
       VALUES ($1, NOW(), $2, $3, $4, $5, $6)
       RETURNING id`,
      [session_id, speaker, input_method, content, sign_confidence, recognition_source]
    );
...
router.get('/sessions/:id/transcript', async (req: Request, res: Response) => {
  try {
    ...
    const sessionResult = await pool.query(
      `SELECT * FROM deaf_mode_sessions WHERE id = $1`,
      [sessionId]
    );
...
// ---------------------------------------------------------------------------
router.get('/quick-responses', async (req: Request, res: Response) => {
  try {
    const pool = getPool(req);

    if (!(await requireTable(pool, 'deaf_mode_quick_responses', res))) return;
    ...
    for (const row of result.rows) {
      const scope: string = row.scope || 'universal';
...
router.get('/sessions', async (req: Request, res: Response) => {
  try {
    ...
    const parsedLimit  = Math.min(parseInt(limit as string, 10)  || 20, 100); // cap at 100
    ...
    const [countResult, dataResult] = await Promise.all([
      ...
      pool.query(
        `SELECT
         ...
         LIMIT $2 OFFSET $3`,
        [rep_user_id, parsedLimit, parsedOffset]
      ),
    ]);
