...
export function createLeadGenRoutes(pool: Pool) {
  const router = Router();
  ...
  async function getUserProfileId(email: string): Promise<string | null> {
    try {
      const result = await pool.query(
  ...
  async function calculateLeadScore(params: {
    ...
    referralCode?: string;
    zipCode?: string;
  }): Promise<{ score: number; scoreFactors: ScoreFactors }> {
    const {
      homeownerPhone,
      homeownerEmail,
      address,
    ...
    if (factors.highValueService) score += 20;
    if (factors.hasPreferredDate) score += 15;
    if (factors.stormSource)      score += 15;
    if (factors.referralSource)   score += 10;

    // Check active storm zone for this zip (+15)
    if (zipCode) {
      try {
        const stormResult = await pool.query(
          `SELECT id FROM storm_zones
           WHERE zip_code = $1
             AND is_active = true
  ...
  function notifyTelegram(params: {
    ...
    const msg =
      `${scoreEmoji} New Lead (Score: ${score}/100)\n` +
      `Name: ${homeownerName}\n` +
  ...
  router.post('/intake', async (req: Request, res: Response) => {
    try {
      ...
      const resolvedSource: LeadSource = validSources.includes(source as LeadSource)
      ...
      const leadId: string = insertResult.rows[0].id;
   ...
   */
  router.get('/storm-zones', async (req: Request, res: Response) => {
    try {
      const result = await pool.query(
   ...
   */
  router.get('/storm-zone/:zip', async (req: Request, res: Response) => {
    try {
  ...
  router.post('/validate-referral', async (req: Request, res: Response) => {
    try {
      ...
      const result = await pool.query(
        `SELECT rc.code, ep.name as rep_name
  ...
  router.get('/dashboard', async (req: Request, res: Response) => {
    try {
      ...
      const topZipsResult = await pool.query(
        `SELECT
           zip_code,
           COUNT(*) as lead_count
         FROM profile_leads pl
   ...
   */
  router.post('/storm-zones', async (req: Request, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
   ...
   */
  router.put('/storm-zones/:id', async (req: Request, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
      const { id } = req.params;
      ...
      const existing = await pool.query(
        'SELECT id FROM storm_zones WHERE id = $1',
        [id]
      );
   ...
   */
  router.delete('/storm-zones/:id', async (req: Request, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
      const { id } = req.params;
   ...
   */
  router.post('/referral-codes', async (req: Request, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
   ...
   */
  router.get('/referral-codes', async (req: Request, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
...
<Response>
  ...
  router.post('/elevenlabs-webhook', async (req: Request, res: Response) => {
    try {
      ...
      const transcript: Array<{ role: string; message: string; time_in_call_secs?: number }> =
        convData.transcript || [];
      ...
      emailService.sendCustomEmail(adminEmail, emailTemplate).catch((err: unknown) => {
   ...
   */
  router.post('/sms-followups/process', async (req: Request, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
   ...
   */
  router.post('/sms-followups/opt-out', async (req: Request, res: Response) => {
    try {
   ...
   */
  router.get('/sms-followups/stats', async (req: Request, res: Response) => {
    try {
      const userEmail = req.headers['x-user-email'] as string;
...
function parseTranscriptForLeadData(transcript: string): {
  name: string;
  phone: string;
  address: string;
  date: string;
  ...
  const phoneMatch = transcript.match(
    /(?:phone|number|reach|call|contact).*?(\(?\d{3}\)?[\s.-]?\d{3}[\s.-]?\d{4})/i
  ...
  const addressMatch = transcript.match(
    /(\d{1,5}\s+[A-Z][a-zA-Z]+(?:\s+[A-Z][a-zA-Z]+)*\s+(?:St|Street|Rd|Road|Ave|Avenue|Dr|Drive|Blvd|Boulevard|Ln|Lane|Ct|Court|Way|Place|Pl|Circle|Cir|Pike|Highway|Hwy)[.,]?\s*(?:[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)?(?:[.,]?\s*(?:VA|MD|PA|Virginia|Maryland|Pennsylvania))?\s*(?:\d{5})?)/i
  );
  ...
  const dateMatch = transcript.match(
    /(?:appointment|schedule|come out|inspection|visit).*?((?:this |next )?(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday)|\d{1,2}\/\d{1,2}(?:\/\d{2,4})?|tomorrow|today)/i
  ) || transcript.match(
    /((?:this |next )?(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday))/i
  );
...
function generateCallLeadEmail(data: {
  ...
  const {
    callerName, callerPhone, callerEmail, callerAddress,
    appointmentSummary, hasAppointment,
    damageType, callDuration, callSuccessful,
    leadScore, leadId, conversationId, transcript,
  } = data;
