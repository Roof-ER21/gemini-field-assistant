...
function sourceListFromRow(row: any): string[] {
  const out: string[] = [];
  if (row.source_noaa_ncei) out.push('NOAA NCEI');
  if (row.source_iem_lsr) out.push('NWS LSR (IEM)');
  if (row.source_ncei_swdi) out.push('NCEI Radar (NX3HAIL)');
  if (row.source_mrms) out.push('MRMS');
  if (row.source_nws_alert) out.push('NWS Alert');
  if (row.source_iem_vtec) out.push('NWS Warning Archive');
  if (row.source_cocorahs) out.push('CoCoRaHS Observer');
  if (row.source_mping) out.push('mPING');
  if (row.source_synoptic) out.push('Synoptic Mesonet');
  if (row.source_spc_wcm) out.push('SPC WCM');
  if (row.source_rep_report) out.push('Rep Self-Report (verified)');
  if (row.source_customer_report) out.push('Customer Report (verified)');
  if (row.source_groupme) out.push('GroupMe Auto-Parse');
  if (row.source_hailtrace) out.push('HailTrace');
  if (row.source_ihm) out.push('IHM');
  return out;
}

export function hailSearchV2Routes(pool: Pool): Router {
  const router = Router();
   ...
   */
  router.get('/search-v2', async (req: Request, res: Response) => {
    try {
      ...
      const formatted = events.map((r: any) => ({
        id: r.id,
      ...
      // Summary stats
      const hailEvents = formatted.filter((e: any) => e.hail_size_inches != null);
      const windEvents = formatted.filter((e: any) => e.wind_mph != null);
      ...
      res.json({
        ...
        totals: {
          ...
          actionable_hail: formatted.filter((e: any) => e.hail_size_inches && e.hail_size_inches >= 0.5).length,
          severe_wind: formatted.filter((e: any) => e.wind_mph && e.wind_mph >= 58).length,
          tornado: formatted.filter((e: any) => e.tornado_ef_rank != null).length,
          multi_source_confirmed: formatted.filter((e: any) => (e.verification_count || 0) >= 2).length,
        },
        events: formatted,
   ...
   */
  router.get('/address-history-v2', async (req: Request, res: Response) => {
    try {
      ...
      const history = await pool.query(
        `SELECT
         ...
         GROUP BY year
         ORDER BY year DESC`,
        [geo.lat, geo.lng, radiusMiles]
      );

      const latestImpact = await pool.query(
        `SELECT event_date, hail_size_inches, wind_mph, tornado_ef_rank, confidence_tier,
