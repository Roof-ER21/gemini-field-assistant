...
export function createGeminiProxyRouter(upstreamFetch: typeof fetch = fetch) {
  ...
  router.post('/v1beta/models/:operation', async (req: SessionRequest, res) => {
    ...
    if (!key) return res.status(503).json({ error: { message: 'AI is not configured.' } });
    ...
    try {
      ...
      const upstream = await upstreamFetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${req.params.operation}${streaming ? '?alt=sse' : ''}`,
        {
          method: 'POST',
      ...
      if (!res.headersSent) res.status(502).json({ error: { message: 'AI request failed. Please retry.' } });
      else res.end();
    } finally {
